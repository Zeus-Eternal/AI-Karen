...
class SecureCredentialManager:
    ...
    def get_credential(
        self, name: str, environment: Optional[str] = None
    ) -> Optional[str]:
        ...
        try:
            ...
            if (
                ...
                and credential.environment != environment
            ):
                return None
                ...
                result.append(
                    {
                        ...
                        and datetime.utcnow() > credential.expires_at,
                    }
                )
    ...
    def _log_rotation_event(
        self, credential_name: str, action: str, environment: Optional[str]
    ):
                    ...
                    if time_until_rotation.total_seconds() > 0:
                        ...
                        task = asyncio.create_task(
                            self._schedule_rotation(
                                name, time_until_rotation.total_seconds()
                            )
                        )
                        self._rotation_tasks[name] = task
                        logger.info(
                            f"Scheduled rotation for credential '{name}' in {time_until_rotation}"
                        )
        ...
        try:
            ...
            with self._rotation_lock:
                ...
                if success:
                    ...
                    logger.error(
                        f"Failed to auto-rotate credential '{credential_name}'"
                    )
...
class ConfigFileWatcher(FileSystemEventHandler):
    ...
    def _debounced_reload(self):
        ...
        self._reload_timer = threading.Timer(self.debounce_delay, self._trigger_reload)
...
class ExtensionEnvironmentConfigManager:
    ...
    def __init__(
        self,
        config_dir: str = "config_assets/extensions",
        credentials_dir: str = "config_assets/extensions/credentials",
        ...
        self.config_validators: List[
            Callable[[ExtensionEnvironmentConfig], List[str]]
        ] = []
    ...
    def _detect_environment(self) -> Environment:
        ...
        env_var = (
            os.getenv("EXTENSION_ENVIRONMENT")
    ...
    def _load_all_configurations(self):
        ...
        try:
            ...
            logger.info(
                f"Loaded configurations for {len(self.configurations)} environments"
            )
    ...
    def _load_environment_config(
        ...
        try:
            if config_file.exists():
                ...
                # Load credentials
                config_data = self._load_credentials_for_config(
                    environment, config_data
                )

                return ExtensionEnvironmentConfig(
                    environment=environment, **config_data
                )
            else:
                logger.info(
                    f"No config file found for {environment.value}, using defaults"
                )
    ...
    def _apply_environment_defaults(
        self, environment: Environment, config_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Apply environment-specific defaults."""
        defaults = {
    ...
    def _load_credentials_for_config(
        self, environment: Environment, config_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            secret_key = self.credentials_manager.get_credential(
                f"extension_secret_key_{environment.value}", environment.value
            )
            if secret_key:
            ...
            api_key = self.credentials_manager.get_credential(
                f"extension_api_key_{environment.value}", environment.value
            )
            if api_key:
    ...
    def get_current_config(self) -> ExtensionEnvironmentConfig:
        ...
        return self.configurations.get(
            ...
            self._create_default_config(self.current_environment),
        )

    def get_config(self, environment: Environment) -> ExtensionEnvironmentConfig:
        """Get configuration for a specific environment."""
        return self.configurations.get(
            environment, self._create_default_config(environment)
        )
        ...
        try:
            ...
            for key, value in updates.items():
                if hasattr(config, key):
                    ...
                    logger.warning(
                        f"Unknown config key '{key}' for environment {environment.value}"
                    )
            ...
            if validation_errors:
                logger.error(
                    f"Configuration validation failed for {environment.value}:"
                )
    ...
    def _save_environment_config(
        self, environment: Environment, config: ExtensionEnvironmentConfig
    ):
        ...
        try:
            ...
            logger.debug(
                f"Saved configuration for {environment.value} to {config_file}"
            )
    ...
    def validate_config(self, config: ExtensionEnvironmentConfig) -> List[str]:
        ...
        try:
            ...
            if config.jwt_algorithm not in [
                "HS256",
                "HS384",
                "HS512",
                "RS256",
    ...
    def add_config_validator(
        self, validator: Callable[[ExtensionEnvironmentConfig], List[str]]
    ):
        """Add a custom configuration validator."""
        self.config_validators.append(validator)

    def add_reload_callback(
        self, callback: Callable[[ExtensionEnvironmentConfig], None]
    ):
    ...
    async def reload_configuration(self):
        ...
        try:
            ...
            if not old_current_config or asdict(current_config) != asdict(
                old_current_config
            ):
                logger.info(
                    f"Configuration changed for {self.current_environment.value}"
                )
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "healthy"
                if not validation_errors and not expired_credentials
                else "degraded",
    ...
    def export_config(
        self, environment: Environment, format: ConfigFormat = ConfigFormat.YAML
    ) -> str:
