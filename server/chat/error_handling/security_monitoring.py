...
from chat.security import SecurityEvent, ThreatLevel, get_security_monitor
...
class ChatMonitoringService:
    ...
    def __init__(self):
        ...
        self.suppression_rules = {
            "same_ip_same_alert": {"window_minutes": 5, "max_alerts": 3},
            "same_user_same_alert": {"window_minutes": 10, "max_alerts": 5},
        }
    ...
    async def end_session(self, session_id: str):
        ...
        logger.info(
            f"Session ended: {session_id}, duration: {duration}s, messages: {session.message_count}"
        )

    async def _evaluate_event_for_alert(
    ...
    async def _check_auth_failure_alert(
        self, event: SecurityEvent
    ) -> Optional[SecurityAlert]:
    ...
    async def _check_brute_force_alert(
        self, event: SecurityEvent
    ) -> Optional[SecurityAlert]:
    ...
    async def _check_rate_limit_alert(
        self, event: SecurityEvent
    ) -> Optional[SecurityAlert]:
    ...
    async def _check_content_alert(
        self, event: SecurityEvent
    ) -> Optional[SecurityAlert]:
    ...
    async def _check_unauthorized_alert(
        self, event: SecurityEvent
    ) -> Optional[SecurityAlert]:
    ...
    async def _check_session_limit_alert(
        self, event: SecurityEvent
    ) -> Optional[SecurityAlert]:
        ...
        if alert.source_ip:
            recent_ip_alerts = [
                ...
                if (
                    ...
                    and (current_time - a.timestamp).total_seconds() < 300
                )  # 5 minutes
            ]

            if (
                len(recent_ip_alerts)
                >= self.suppression_rules["same_ip_same_alert"]["max_alerts"]
            ):
                return True
        ...
        if alert.user_id:
            recent_user_alerts = [
                ...
                if (
                    ...
                    and a.alert_type == alert.alert_type
                    and a.status == AlertStatus.ACTIVE
                    and (current_time - a.timestamp).total_seconds() < 600
                )  # 10 minutes
            ]

            if (
                len(recent_user_alerts)
                >= self.suppression_rules["same_user_same_alert"]["max_alerts"]
            ):
                return True
    ...
    async def _metric_collection_loop(self):
        ...
        while True:
            try:
                ...
                active_users = len(
                    [s for s in self.active_sessions.values() if s.end_time is None]
                )
                ...
                recent_metrics = [
                    ...
                    if (
                        m.metric_type == MetricType.ERROR_RATE
                        and (datetime.utcnow() - m.timestamp).total_seconds() < 300
                    )  # 5 minutes
                ]

                if recent_metrics:
                    avg_error_rate = sum(m.value for m in recent_metrics) / len(
                        recent_metrics
                    )
                    await self.record_metric(
                        MetricType.ERROR_RATE, avg_error_rate, "ratio"
                    )
            ...
            try:
                ...
                for session_id, session in list(self.active_sessions.items()):
                    if (
                        ...
                        logger.info(
                            f"Session cleaned up due to inactivity: {session_id}"
                        )
    ...
    ) -> List[ChatSessionMetrics]:
        """Get active chat sessions."""
        sessions = [s for s in self.active_sessions.values() if s.end_time is None]
    ...
    def get_session_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        avg_response_time = 0
        sessions_with_response = [
            s
            for s in recent_sessions
            if s.total_response_time > 0 and s.message_count > 0
        ]
        ...
        for s in recent_sessions:
            if s.provider_used:
                provider_usage[s.provider_used] = (
                    provider_usage.get(s.provider_used, 0) + 1
                )
...
async def update_chat_session(
    ...
    await monitoring_service.update_session(
        session_id, message_count, response_time, error_count, provider_used
    )
