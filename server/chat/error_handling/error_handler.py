...
class GlobalErrorHandler:
    ...
    def _initialize_default_handlers(self):
        ...
        self.register_error_handler(
            ErrorCategory.CONNECTIVITY, self._handle_connectivity_error
        )

        # System errors
        self.register_error_handler(ErrorCategory.SYSTEM, self._handle_system_error)
        self.register_error_handler(
            ErrorCategory.INFRASTRUCTURE, self._handle_infrastructure_error
        )
        ...
        self.register_error_handler(
            ErrorCategory.AI_PROCESSING, self._handle_ai_processing_error
        )
        self.register_error_handler(
            ErrorCategory.MODEL_UNAVAILABLE, self._handle_model_unavailable_error
        )
        self.register_error_handler(
            ErrorCategory.LLM_PROVIDER, self._handle_llm_provider_error
        )
    ...
    async def _classify_error(
        ...
        resolution_steps, recovery_actions = self._generate_resolution_steps(
            category, error
        )
    ...
    def _determine_category_and_severity(
        ...
        if any(
            ...
            for keyword in ["connection", "network", "dns", "timeout", "unreachable"]
        ):
        ...
        if any(
            keyword in error_message
            for keyword in ["database", "sql", "connection pool", "transaction"]
        ):
        ...
        if any(
            keyword in error_message
            for keyword in ["model", "ai", "llm", "inference", "generation"]
        ):
        ...
        if any(
            keyword in error_message
            for keyword in ["memory", "disk", "resource", "system"]
        ):
    ...
    async def _log_error(self, error_info: ErrorInfo):
        ...
        log_message = (
            f"Error {error_info.id}: {error_info.title} - {error_info.message}"
        )
        ...
        if error_info.category in [
            ...
            await log_security_event(
                "security_error",
                {
                    ...
                    "ip_address": error_info.context.get("ip_address")
    ...
    def _create_error_response(
        self, error_info: ErrorInfo, recovery_result: Optional[RecoveryResult] = None
    ) -> ErrorResponse:
        """Create standardized error response."""
        response = ErrorResponse(error=error_info, request_id=error_info.request_id)
    ...
    def _create_circuit_breaker_response(self, error_info: ErrorInfo) -> ErrorResponse:
        ...
        return ErrorResponse(
            error=circuit_breaker_error, request_id=error_info.request_id
        )
    ...
    def _create_fallback_response(
        ...
        return ErrorResponse(
            error=fallback_error, request_id=context.request_id if context else None
        )
