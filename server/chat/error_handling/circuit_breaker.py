...
class CircuitBreaker:
    ...
    def _should_open_circuit(self) -> bool:
        ...
        if self.config.policy == CircuitBreakerPolicy.FAILURE_COUNT:
            ...
            if (
                self.metrics.total_requests >= self.config.min_samples
                and self.metrics.get_failure_rate()
    ...
    def _get_state_change_reason(
        ...
        if (
            ...
            and new_state == CircuitBreakerState.HALF_OPEN
        ):
            return "Timeout elapsed, attempting recovery"
        elif (
            old_state == CircuitBreakerState.HALF_OPEN
            and new_state == CircuitBreakerState.CLOSED
        ):
    ...
    def _add_to_history(
        self, timestamp: datetime, success: bool, exception: Optional[Exception] = None
    ) -> None:
        ...
        self.metrics.request_history = [
            entry
            for entry in self.metrics.request_history
...
class CircuitBreakerManager:
            ...
            if name not in self.circuit_breakers:
                if config is None:
                    config = self.config_templates.get(
                        "network_service", CircuitBreakerConfig(name=name)
                    )
    ...
    def create_from_template(self, name: str, template_name: str) -> CircuitBreaker:
        ...
        config = CircuitBreakerConfig(
            ...
            **{
                k: v
                for k, v in self.config_templates[template_name].__dict__.items()
                if k != "name"
            },
        )
    ...
    def create_from_error_classification(
        self, name: str, classification: ErrorClassification
    ) -> CircuitBreaker:
        ...
        if classification.user_action_required:
            config.failure_threshold = min(
                config.failure_threshold, 3
            )  # Lower threshold for user-action errors
