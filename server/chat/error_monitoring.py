...
class ErrorMonitoringService:
    ...
    async def _detect_trend_patterns(self) -> List[ErrorPattern]:
        ...
        for i in range(5):  # Last 5 windows
            window_end = current_time - timedelta(
                minutes=i * self.config.trend_window.total_seconds() / 60
            )
    ...
    async def _detect_correlation_patterns(self) -> List[ErrorPattern]:
        ...
        for i in range(len(time_sorted_errors)):
            ...
            correlated_errors = [
                error
                for error in time_sorted_errors[i + 1 : i + 10]
                if (error["timestamp"] - current_error["timestamp"]).total_seconds()
                <= 60
            ]
                ...
                if len(components) >= 2:  # At least 2 different components
                    patterns.append(
                        ErrorPattern(
                            ...
                            last_seen=max(
                                error["timestamp"] for error in correlated_errors
                            ),
                            ...
                            metadata={
                                ...
                                "correlated_errors": [
                                    error["id"] for error in correlated_errors
                                ],
    ...
    async def _detect_anomalies(self) -> List[ErrorPattern]:
        ...
        intervals = [
            (
                time_sorted_errors[i + 1]["timestamp"]
                - time_sorted_errors[i]["timestamp"]
            ).total_seconds()
            ...
            if intervals[i] > threshold:
                patterns.append(
                    ErrorPattern(
                        ...
                        confidence=min(
                            1.0, (intervals[i] - mean_interval) / stdev_interval / 2
                        ),
    ...
    async def calculate_metrics(
        ...
        recurring_errors = len(
            [count for count in error_type_counts.values() if count >= 3]
        )

        # Estimate cascading errors (simplified detection)
        cascading_errors = sum(
            1
            for error in recent_errors
            if "cascade" in error.get("message", "").lower()
        )

        # Estimate user impact (simplified)
        user_impacted_count = len(
            set(error.get("user_id") for error in recent_errors if error.get("user_id"))
        )
        ...
        top_errors = [
            ...
            for error in recent_errors
            if f"{error['type']}_{error.get('component', 'unknown')}" == error_type
        ]
            ...
            try:
                ...
                for pattern in patterns:
                    ...
                    existing_similar = [
                        ...
                        if (
                            p.type == pattern.type
                            and p.affected_components == pattern.affected_components
                            and (
                                datetime.now(timezone.utc) - p.last_seen
                            ).total_seconds()
                            < 300
                        )  # 5 minutes
                    ]
        ...
        while True:
            try:
                ...
                if len(self.error_history) < original_count:
                    logger.debug(
                        f"Cleaned up {original_count - len(self.error_history)} old error records"
                    )
                ...
                if len(self.patterns) < original_pattern_count:
                    logger.debug(
                        f"Cleaned up {original_pattern_count - len(self.patterns)} old patterns"
                    )
                ...
                self.alerts = [
                    alert for alert in self.alerts if alert.created_at > alert_cutoff
                ]

                if len(self.alerts) < original_alert_count:
                    logger.debug(
                        f"Cleaned up {original_alert_count - len(self.alerts)} old alerts"
                    )
    ...
    def get_active_patterns(self) -> List[ErrorPattern]:
        ...
        return [pattern for pattern in self.patterns if pattern.last_seen > cutoff_time]
    ...
    def get_error_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        return {
            ...
            "top_error_components": dict(
                sorted(
                    metrics.errors_by_component.items(),
                    key=lambda x: x[1],
                    reverse=True,
                )[:5]
            ),
            "top_error_severities": dict(
                sorted(
                    metrics.errors_by_severity.items(), key=lambda x: x[1], reverse=True
                )[:5]
            ),
