...
class TestPasswordResetFeatures:
    ...
    async def test_create_password_reset_token_success(self, auth_service, sample_user):
        ...
        auth_service.core_auth.token_manager.create_password_reset_token_with_storage = AsyncMock(return_value="reset-token-123")
        ...
        auth_service.core_auth.get_user_by_email.assert_called_once_with("test@example.com")
    ...
    async def test_create_password_reset_token_user_not_found(self, auth_service):
        ...
        auth_service.core_auth.get_user_by_email.assert_called_once_with("nonexistent@example.com")
        auth_service.security_layer.log_auth_event.assert_called_once()
    ...
    async def test_create_password_reset_token_rate_limited(self, auth_service, sample_user):
        """Test password reset token creation when rate limited."""
        # Setup mocks
        auth_service.core_auth.get_user_by_email = AsyncMock(return_value=sample_user)
        auth_service.security_layer.check_rate_limit = AsyncMock(side_effect=RateLimitExceededError())
    ...
    async def test_verify_password_reset_token_success(self, auth_service, sample_user):
        ...
        # Setup mocks
        auth_service.core_auth.token_manager.verify_password_reset_token_with_storage.return_value = sample_user
...
class TestEmailVerificationFeatures:
    ...
    async def test_create_email_verification_token_success(self, auth_service, unverified_user):
        """Test successful email verification token creation."""
        # Setup mocks
        auth_service.core_auth.get_user_by_id.return_value = unverified_user
        auth_service.core_auth.token_manager.create_email_verification_token_with_storage.return_value = "verify-token-123"
    ...
    async def test_create_email_verification_token_already_verified(self, auth_service, verified_user):
    ...
    async def test_verify_email_address_success(self, auth_service, unverified_user):
        ...
        # Setup mocks
        auth_service.core_auth.token_manager.verify_email_verification_token_with_storage.return_value = unverified_user
...
class TestUserManagementFeatures:
    ...
    async def test_update_user_preferences_success(self, auth_service, sample_user):
        ...
        updated_user = auth_service.core_auth.db_client.update_user.call_args[0][0]
        expected_preferences = {"theme": "light", "language": "en", "notifications": True}
...
class TestTokenManagerEnhancements:
    ...
    async def test_create_password_reset_token_with_storage(self, token_manager, sample_user, mock_db_client):
    ...
    async def test_verify_password_reset_token_with_storage_success(self, token_manager, sample_user, mock_db_client):
    ...
    async def test_verify_password_reset_token_with_storage_invalid_token(self, token_manager, mock_db_client):
        """Test password reset token verification with invalid token."""
    ...
    async def test_create_email_verification_token_with_storage(self, token_manager, sample_user, mock_db_client):
...
class TestDatabaseTokenStorage:
    ...
    async def test_store_and_retrieve_password_reset_token(self, db_client, sample_user_id):
    ...
    async def test_cleanup_expired_password_reset_tokens(self, db_client, sample_user_id):
    ...
    async def test_store_and_retrieve_email_verification_token(self, db_client, sample_user_id):
