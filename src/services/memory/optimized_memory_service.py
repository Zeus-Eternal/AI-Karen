...
from src.services.vector_optimization import (
    SearchResult,
...
class OptimizedMemoryService(UnifiedMemoryService):
    ...
    def __init__(
        ...
        super().__init__(
            db_client, milvus_client, embedding_manager, redis_client, policy_manager
        )
    ...
    async def query(
        ...
        try:
            ...
            search_k = (
                int(request.top_k * self.vector_config.rerank_factor)
                if self.vector_config.rerank_enabled
                else request.top_k
            )
            ...
            if (
                search_metrics.total_latency_ms
                > self.vector_config.target_p95_latency_ms
            ):
    ...
    async def _ensure_vectors_loaded(
        self, tenant_id: Union[str, uuid.UUID], collection_name: str
    ):
    ...
    async def _load_vectors_from_milvus(
        self, tenant_id: Union[str, uuid.UUID], collection_name: str
    ):
    ...
    async def _get_memory_data_by_id(
        ...
        try:
            ...
            return {"content": f"Memory content for {memory_id}", "metadata": {}}
        except Exception as e:
            logger.error(f"Failed to get memory data for {memory_id}: {e}")
            return None

    async def _apply_memory_policy_ranking(
        self, context_hits: List[ContextHit]
    ) -> List[ContextHit]:
    ...
    async def _record_recall_metrics(
        self, results: List[ContextHit], query: str, correlation_id: str
    ):
