...
class ProfileManager:
    ...
    def _load_profiles(self) -> None:
        ...
        if self.profiles_path.exists():
            try:
                ...
                logger.info(
                    f"Loaded {len(self._profiles)} profiles from {self.profiles_path}"
                )
    ...
    def _deserialize_profile(self, data: Dict[str, Any]) -> Optional[LLMProfile]:
        ...
        try:
            ...
            logger.error(
                f"Failed to deserialize profile {data.get('name', 'unknown')}: {e}"
            )
            return None
    ...
    def validate_profile(self, profile: LLMProfile) -> Dict[str, List[str]]:
        ...
        # Validate guardrails
        if (
            profile.guardrails.max_tokens_per_request
            > profile.memory_budget.max_context_length
        ):
    ...
    def get_routing_decision(
        ...
        elif policy.privacy_level == "low" and context.get(
        ...
        if preferred_provider not in available_providers:
            ...
            if policy.fallback_strategy == "graceful":
                ...
                reason = (
                    f"Fallback to {preferred_provider} (original provider unavailable)"
                )
            elif policy.fallback_strategy == "aggressive":
                preferred_provider = (
                    available_providers[0] if available_providers else "local"
                )
