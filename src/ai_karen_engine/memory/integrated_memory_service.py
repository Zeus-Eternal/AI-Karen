...
from ai_karen_engine.memory.enhanced_memory_service import (
    EnhancedMemoryService,
    MemoryServiceError,
)
from ai_karen_engine.memory.internal.conversation_tracker import (
    ConversationSession,
    ConversationTracker,
    ConversationTurn,
)
...
class IntegratedMemoryService:
    ...
    def __init__(
        self, base_memory_manager: MemoryManager, db_client: MultiTenantPostgresClient
    ):
    ...
    async def query_memories_with_context(
        self, tenant_id: Union[str, uuid.UUID], query: ContextualMemoryQuery
        ...
        try:
            ...
            if query.include_conversation_context and query.session_id:
                try:
                    conversation_context = (
                        await self.conversation_tracker.get_conversation_context(
                            query.session_id,
            ...
            enhanced_query_text = self._enhance_query_with_context(
                query.text, context_summary
            )
            ...
            used_fallback = False
            try:
                memories = await self.enhanced_memory_service.query_memories(
                    tenant_id, web_ui_query
                )
            ...
            logger.info(
                ...
                extra={
                    ...
                    "context_enhanced": bool(
                        conversation_context and not conversation_context.get("error")
                    ),
                },
            )
    ...
    async def store_conversation_memory(
        ...
        try:
            ...
            try:
                conversation_turn = (
                    await self.conversation_tracker.add_conversation_turn(
                        session_id=session_id,
                        user_message=user_message,
                        assistant_response=assistant_response,
                        memory_references=memory_references,
    ...
    async def get_conversation_history_with_memories(
        ...
        try:
            ...
            turns = await self.conversation_tracker.get_session_history(
                session_id, limit
            )
    ...
    def _enhance_query_with_context(
        self, original_query: str, context_summary: str
    ) -> str:
            ...
            for memory in memories:
                if memory.id in context_memory_refs:
                    ...
                    if memory.similarity_score:
                        memory.similarity_score = min(
                            memory.similarity_score * 1.2, 1.0
                        )
...
def create_integrated_memory_service(
    base_memory_manager: MemoryManager, db_client: MultiTenantPostgresClient
