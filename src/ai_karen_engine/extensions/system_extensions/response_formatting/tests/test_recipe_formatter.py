...
class TestRecipeResponseFormatter(unittest.TestCase):
    ...
    def test_extract_recipe_info_comprehensive(self):
        ...
        preheat_found = any(
            "preheat" in inst.lower() for inst in recipe_info.instructions
        )
        self.assertTrue(
            preheat_found,
            f"Preheat instruction not found in: {recipe_info.instructions}",
        )
        self.assertIn(
            "Bake for 9-11 minutes until golden brown.", recipe_info.instructions
        )

    def test_extract_recipe_info_minimal(self):
        """Test recipe information extraction with minimal data."""
        content = (
            "Here's how to make pasta: boil water, add pasta, cook for 10 minutes."
        )
    ...
    def test_parse_instructions_list(self):
        ...
        self.assertTrue(
            preheat_found, f"Preheat instruction not found in: {instructions}"
        )
    ...
    def test_get_confidence_score(self):
        ...
        measurement_content = (
            "You need 2 cups flour and 1 tablespoon vanilla to bake cookies."
        )
        context = ResponseContext(
            user_query="baking question",
    ...
    def test_measurement_pattern_detection(self):
        ...
        measurement_matches = sum(
            1
            for pattern in self.formatter._measurement_patterns
