...
class ExtensionRunner:
    ...
    def __init__(self, registry: ExtensionRegistry, default_timeout: float = 30.0):
        """
    ...
    async def execute_hook(
        ...
        logger.info(
            f"Executing {len(extensions)} extensions for hook point: {hook_point}"
        )

        # Update Prometheus metrics for hook execution count
        if "hook_execution_count" in self._prometheus_metrics:
            self._prometheus_metrics["hook_execution_count"].labels(
        ...
        for extension in extensions:
            ...
            if not self._check_rbac_permissions(extension, user_role):
                logger.warning(
                    f"Extension {extension.manifest.name} not allowed for role: {user_role}"
                )
                continue
            ...
            try:
                ...
                if "extension_execution_count" in self._prometheus_metrics:
                    self._prometheus_metrics["extension_execution_count"].labels(
                        extension_name=extension_name, hook_point=hook_point.value
                    ).inc()
        ...
        if "hook_execution_duration" in self._prometheus_metrics:
            self._prometheus_metrics["hook_execution_duration"].labels(
                hook_point=hook_point.value
    ...
    async def _execute_extension_with_timeout(
        self,
    ...
    async def _execute_extension_safely(
        self, extension: ExtensionBase, hook_point: HookPoint, context: HookContext
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not extension.is_initialized():
                logger.warning(
                    f"Extension {extension.manifest.name} is not initialized, initializing now"
                )
            ...
            if not extension.supports_hook_point(hook_point):
                logger.warning(
                    f"Extension {extension.manifest.name} does not support hook point: {hook_point}"
                )
                return {"error": "Extension does not support this hook point"}

            # Execute the hook
            logger.debug(
                f"Executing hook {hook_point} in extension {extension.manifest.name}"
            )
            ...
            if not isinstance(result, dict):
                logger.warning(
                    f"Extension {extension.manifest.name} returned non-dict result: {type(result)}"
                )
                result = {"data": result}
    ...
    def _check_rbac_permissions(
        self, extension: ExtensionBase, user_role: Optional[str]
    ) -> bool:
        """
        ...
        if user_role is None:
            ...
            return "guest" in [
                role.value for role in extension.manifest.rbac.allowed_roles
            ]
        ...
        for hook_name, hook_stats in self._execution_stats.items():
            if extension_name:
                if extension_name in hook_stats.get("extension_execution_count", {}):
                    metrics[hook_name] = {
                        "execution_count": hook_stats["extension_execution_count"].get(
                            extension_name, 0
                        ),
                        "error_count": hook_stats["extension_errors"].get(
                            extension_name, 0
                        ),
                    }
            else:
            ...
            if extension_name:
                ...
                for metric_name, samples in prometheus_metrics.items():
                    ...
                    filtered_samples = [
                        ...
                        if sample.get("labels", {}).get("extension_name")
                        == extension_name
                    ]
                    if filtered_samples:
    ...
    def get_hook_metrics(self, hook_point: HookPoint) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "average_execution_time": execution_stats.get(
                "average_execution_time", 0.0
            ),
