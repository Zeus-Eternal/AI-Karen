...
class WebSearchStockMarketHandler(ExtensionBase):
    """
    Stock market search intent handler.

    Responsibilities:
    - Prepare a normalized stock-market-oriented retrieval strategy
    - Implement the required ExtensionBase contract
    - Return structured intent payloads for orchestrator/runtime use
    - Do NOT fetch market data, crawl pages, or generate final answers
    """
    ...
    DEFAULTS: Dict[str, Any] = {
        ...
        "include_analyst_sentiment": False,
    }
    ...
    async def initialize(self) -> None:
        self.logger.debug(
            "Initializing stock market search extension",
            extra={
    ...
    async def shutdown(self) -> None:
        self.logger.debug(
            "Shutting down stock market search extension",
            extra={
                "extension_name": getattr(self.manifest, "name", self.__class__.__name__),
                "mode": self.MODE,
            },
    ...
    async def prepare(
        ...
        prefer_financial_sources = self._coerce_bool(
            safe_context.get("prefer_financial_sources"),
            self._read_manifest_setting("prefer_financial_sources"),
            self.DEFAULTS["prefer_financial_sources"],
        )
        ...
        include_price_action = self._coerce_bool(
            safe_context.get("include_price_action"),
            self._read_manifest_setting("include_price_action"),
            self.DEFAULTS["include_price_action"],
        )
        include_company_news = self._coerce_bool(
            safe_context.get("include_company_news"),
            self._read_manifest_setting("include_company_news"),
            self.DEFAULTS["include_company_news"],
        )
        include_earnings = self._coerce_bool(
            safe_context.get("include_earnings"),
            self._read_manifest_setting("include_earnings"),
            self.DEFAULTS["include_earnings"],
        )
        include_analyst_sentiment = self._coerce_bool(
            safe_context.get("include_analyst_sentiment"),
            self._read_manifest_setting("include_analyst_sentiment"),
            self.DEFAULTS["include_analyst_sentiment"],
        )
        ...
        response = {
            ...
            "search_profile": {
                ...
                "prefer_financial_sources": prefer_financial_sources,
                "allow_forum_results": allow_forum_results,
            },
            "market_profile": {
                "ticker": self._normalize_string(safe_context.get("ticker")),
                "company_name": self._normalize_string(safe_context.get("company_name")),
                "exchange": self._normalize_string(safe_context.get("exchange")),
                "sector": self._normalize_string(safe_context.get("sector")),
                "include_price_action": include_price_action,
                "include_company_news": include_company_news,
                "include_earnings": include_earnings,
                "include_analyst_sentiment": include_analyst_sentiment,
            },
            "time_policy": {
                ...
                "market_session": self._normalize_string(safe_context.get("market_session")),
            },
            "domain_policy": {
        ...
        self.logger.debug(
            "Prepared stock market search intent strategy",
            extra={
    ...
    async def execute(self, config: Dict[str, Any], query: str) -> Dict[str, Any]:
        ...
        prepared.setdefault("query_strategy", "market_news_biased")
        prepared.setdefault(
            "search_profile",
            {
                "prefer_recent": self.DEFAULTS["prefer_recent"],
                "prefer_official_sources": self.DEFAULTS["prefer_official_sources"],
                "prefer_financial_sources": self.DEFAULTS["prefer_financial_sources"],
        ...
        prepared.setdefault(
            "market_profile",
            {
                ...
                "exchange": None,
                "sector": None,
                "include_price_action": self.DEFAULTS["include_price_action"],
                "include_company_news": self.DEFAULTS["include_company_news"],
                "include_earnings": self.DEFAULTS["include_earnings"],
                "include_analyst_sentiment": self.DEFAULTS["include_analyst_sentiment"],
            },
        )
        prepared.setdefault(
            "time_policy",
...
class MainExtension(WebSearchStockMarketHandler):
