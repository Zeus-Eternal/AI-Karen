...
class WebSearchDocsHandler(ExtensionBase):
    """
    Documentation search intent handler.

    Responsibilities:
    - Prepare a normalized retrieval strategy for docs-oriented web search
    ...
    async def initialize(self) -> None:
        ...
        """
        self.logger.debug(
            "Initializing docs search extension",
            extra={
    ...
    async def shutdown(self) -> None:
        ...
        """
        self.logger.debug(
            "Shutting down docs search extension",
            extra={
                "extension_name": getattr(self.manifest, "name", self.__class__.__name__),
                "mode": self.MODE,
            },
    ...
    async def prepare(
        ...
        """
        Prepare a normalized docs search strategy payload.
        """
        ...
        prefer_api_reference = self._coerce_bool(
            safe_context.get("prefer_api_reference"),
            self._read_manifest_setting("prefer_api_reference"),
            self.DEFAULTS["prefer_api_reference"],
        )
        prefer_changelogs = self._coerce_bool(
            safe_context.get("prefer_changelogs"),
            self._read_manifest_setting("prefer_changelogs"),
            self.DEFAULTS["prefer_changelogs"],
        )
        ...
        response = {
            ...
            "search_profile": {
                ...
                "prefer_api_reference": prefer_api_reference,
                "prefer_changelogs": prefer_changelogs,
            ...
            "docs_hints": {
                "product": self._normalize_string(safe_context.get("product")),
                "version": self._normalize_string(safe_context.get("version")),
                "language": self._normalize_string(safe_context.get("language")),
                "target_doc_types": self._normalize_list(safe_context.get("target_doc_types")),
            },
        ...
        self.logger.debug(
            "Prepared docs search intent strategy",
            extra={
    ...
    async def execute(self, config: Dict[str, Any], query: str) -> Dict[str, Any]:
        ...
        prepared.setdefault("query_strategy", "official_docs_biased")
        prepared.setdefault(
            "search_profile",
            {
                ...
                "prefer_news_publishers": False,
                "prefer_api_reference": self.DEFAULTS["prefer_api_reference"],
                "prefer_changelogs": self.DEFAULTS["prefer_changelogs"],
...
class MainExtension(WebSearchDocsHandler):
