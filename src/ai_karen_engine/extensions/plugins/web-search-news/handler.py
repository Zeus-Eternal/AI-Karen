...
class WebSearchNewsHandler(ExtensionBase):
    """
    News search intent handler.

    Responsibilities:
    - Prepare a normalized retrieval strategy for news-oriented web search
    ...
    async def initialize(self) -> None:
        ...
        This intent extension is lightweight and does not create external resources,
        but we still validate required manifest state and log readiness.
        """
        self.logger.debug(
            "Initializing news search extension",
            extra={
    ...
    async def shutdown(self) -> None:
        ...
        """
        self.logger.debug(
            "Shutting down news search extension",
            extra={
                "extension_name": getattr(self.manifest, "name", self.__class__.__name__),
                "mode": self.MODE,
            },
    ...
    async def execute_hook(
        ...
        """
        Execute a supported hook by converting the incoming host context into
    ...
    async def prepare(
        ...
        """
        Prepare a normalized news search strategy payload.
        """
...
class MainExtension(WebSearchNewsHandler):
