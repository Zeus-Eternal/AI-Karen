...
class WorkflowEngine:
    ...
    async def execute_workflow(
        ...
        asyncio.create_task(self._execute_workflow_async(workflow, execution, user_context or {}))
    ...
    async def _execute_step(
        ...
        try:
            ...
            if step.type == StepType.PLUGIN:
                result = await self._execute_plugin_step(workflow, execution, step, step_execution, user_context)
            elif step.type == StepType.CONDITION:
                result = await self._execute_condition_step(workflow, execution, step, step_execution, user_context)
            elif step.type == StepType.PARALLEL:
                result = await self._execute_parallel_step(workflow, execution, step, step_execution, user_context)
            elif step.type == StepType.LOOP:
                result = await self._execute_loop_step(workflow, execution, step, step_execution, user_context)
            elif step.type == StepType.WAIT:
                result = await self._execute_wait_step(workflow, execution, step, step_execution, user_context)
            elif step.type == StepType.TRANSFORM:
                result = await self._execute_transform_step(workflow, execution, step, step_execution, user_context)
            elif step.type == StepType.MCP_TOOL:
                result = await self._execute_mcp_tool_step(workflow, execution, step, step_execution, user_context)
            else:
            ...
            for next_step_id in step.next_steps:
                await self._execute_step(workflow, execution, next_step_id, user_context)
            ...
            if step.retry_config and step_execution.retry_count < step.retry_config.get("max_retries", 0):
                ...
                return await self._execute_step(workflow, execution, step_id, user_context)
            
            # Check if error handler is configured
            if step.error_handler:
                self.logger.info(f"Executing error handler {step.error_handler} for step {step_id}")
                return await self._execute_step(workflow, execution, step.error_handler, user_context)
    ...
    async def _execute_parallel_step(
        ...
        for parallel_step_id in parallel_steps:
            ...
            task = asyncio.create_task(
                self._execute_step(workflow, branch_execution, parallel_step_id, user_context)
            )
            tasks.append(task)
    ...
    async def _execute_loop_step(
        ...
        if loop_type == "items":
            ...
            for i, item in enumerate(items):
                ...
                # Execute target step
                result = await self._execute_step(workflow, execution, target_step, user_context)
                results.append(result)
                
                # Check for break condition
                break_condition = step.config.get("break_condition")
                if break_condition and self._evaluate_condition(break_condition, execution.variables):
                    self.logger.debug(f"Loop step {step.id} break condition met after {i+1} iterations")
                    break
    ...
    async def _execute_transform_step(
        ...
        if transform_type == "function":
            ...
            if not function_name or function_name not in self.function_registry:
                raise ValueError(f"Transform step {step.id} missing or unknown function: {function_name}")
            ...
            resolved_args = [self._resolve_variables(arg, execution.variables) for arg in args]
            resolved_kwargs = {k: self._resolve_variables(v, execution.variables) for k, v in kwargs.items()}
    ...
    async def _execute_mcp_tool_step(
        ...
        if not service_name or not tool_name:
            raise ValueError(f"MCP tool step {step.id} missing service_name or tool_name")
        
        # Check if tool is registered
        if (service_name not in self.mcp_tool_registry or 
            tool_name not in self.mcp_tool_registry[service_name]):
    ...
    def _evaluate_condition(self, condition: str, variables: Dict[str, Any]) -> bool:
        ...
        for var_name, var_value in variables.items():
            ...
            if placeholder in resolved_condition:
                resolved_condition = resolved_condition.replace(placeholder, str(var_value))
