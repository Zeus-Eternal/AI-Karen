...
class ExtensionConfig(BaseService):
    ...
    async def update_extension_config(
        self, extension_id: str, config_updates: Dict[str, Any]
    ) -> bool:
        """
        ...
        async with self._lock:
            ...
            if extension_id in self._extension_configs:
                ...
                self.logger.error(
                    f"No configuration found for extension {extension_id}"
                )
                return False
            ...
            if not await self._validate_config(extension_id, current_config):
                self.logger.error(
                    f"Invalid configuration update for extension {extension_id}"
                )
                return False
        ...
        async with self._lock:
            if extension_id in self._extension_configs:
                ...
                config_file = os.path.join(
                    "config", "extensions", f"{extension_id}.json"
                )
    ...
    async def register_extension_config(
        self, extension_id: str, schema: Dict[str, Any], defaults: Dict[str, Any]
    ) -> bool:
        """
    ...
    async def _load_config_files(self) -> None:
        """Load configuration files from the config_assets/extensions directory."""
        configs = load_extension_configs()
        for config in configs:
            extension_id = config.get("id", "unknown")
            self._extension_configs[extension_id] = config
            self.logger.info(f"Loaded configuration for extension {extension_id}")

    async def _load_config_schemas(self) -> None:
        """Load configuration schemas from the config_assets/schemas directory."""
        schema_dir = os.path.join("config_assets", "schemas")
                ...
                if filename.endswith(".json"):
                    ...
                    try:
                        with open(filepath, "r") as f:
                            ...
                            self.logger.info(
                                f"Loaded configuration schema for extension {extension_id} from {filename}"
                            )
                    except Exception as e:
                        self.logger.error(
                            f"Failed to load configuration schema from {filename}: {e}"
                        )
    ...
    async def _validate_config(self, extension_id: str, config: Dict[str, Any]) -> bool:
        ...
        for field in required_fields:
            if field not in config:
                self.logger.error(
                    f"Missing required field '{field}' in configuration for extension {extension_id}"
                )
                return False
    ...
    async def _save_config(self, extension_id: str, config: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Saved configuration for extension {extension_id} to {config_file}"
            )
        except Exception as e:
            self.logger.error(
                f"Failed to save configuration for extension {extension_id}: {e}"
            )
