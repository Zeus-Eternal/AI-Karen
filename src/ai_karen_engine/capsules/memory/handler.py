"""Memory maintenance capsule implementation."""
...
try:
    ...
    CAPSULE_SUCCESS = Counter(
        "capsule_memory_success_total", "Successful memory capsule executions"
    )
    CAPSULE_FAILURE = Counter(
        "capsule_memory_failure_total", "Failed memory capsule executions"
    )
except ImportError:
    CAPSULE_SUCCESS = CAPSULE_FAILURE = None


class MemoryCapsule(BaseCapsule):
    """BaseCapsule-backed memory maintenance capsule."""

    _instance: Optional["MemoryCapsule"] = None
    ...
    def _execute_core(self, context):
        ...
        try:
            ...
            logger.info("Memory task succeeded for user %s", context.user_ctx.get("sub"))
...
def get_capsule_handler() -> MemoryCapsule:
    """Return the process-local Memory capsule singleton."""
