...
class DevOpsCapsule(BaseCapsule):
    """BaseCapsule-backed DevOps capsule."""

    _instance: Optional["DevOpsCapsule"] = None
    ...
    def __init__(self, capsule_dir: Optional[Path] = None):
        if getattr(self, "_devops_initialized", False):
            return

        super().__init__(capsule_dir or Path(__file__).parent)
        self._devops_initialized = True
        logger.info("DevOps capsule initialized in secure mode")
    ...
    def _execute_core(self, context):
        ...
        try:
            ...
            logger.info(
                "Task succeeded for user %s", context.user_ctx.get("sub")
            )
...
def get_capsule_handler() -> DevOpsCapsule:
    """Return the process-local DevOps capsule singleton."""
