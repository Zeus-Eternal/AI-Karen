...
class HealthMonitor:
    ...
    def _update_service_health(self, name: str, result: HealthResult) -> None:
        ...
        recent_checks = [
            check for check in service_health.checks
    ...
    def get_health_history(self, service_name: str, hours: int = 24) -> List[HealthResult]:
        ...
        return [
            result for result in self._results[service_name]
...
async def setup_default_health_checks() -> None:
    ...
    async def check_ai_orchestrator():
        try:
            ...
            return {"status": "unhealthy", "message": f"AI Orchestrator error: {str(e)}"}
