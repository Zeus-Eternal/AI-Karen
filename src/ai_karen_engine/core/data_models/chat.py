...
class ChatConversation(Base):
    ...
    sessions = relationship(
        "ChatSession", back_populates="conversation", cascade="all, delete-orphan"
    )

    __table_args__ = (
        Index(
            "idx_chat_conversations_user_id_updated_at", "user_id", "updated_at DESC"
        ),
...
class ChatMessage(Base):
    ...
    __table_args__ = (
        Index(
            "idx_chat_messages_conversation_id_created_at",
            "conversation_id",
            "created_at ASC",
        ),
...
class ChatProviderConfiguration(Base):
    ...
    )  # Security level for this config
    access_count = Column(
        Integer, default=0
    )  # How many times this config has been accessed
    last_accessed_at = Column(DateTime(timezone=True))
    created_by_ip = Column(String(45))
    last_modified_by = Column(String(255))
