...
class LangGraphOrchestrator:
    ...
    # --- Salvaged Utility Methods ---

    async def _format_response_with_engine(
        self, response_text: str, state: LangGraphOrchestrationState
        ...
        pretty_layer = PrettyOutputLayer()

        # 3. Build context
        formatting_ctx = await self._build_formatting_context(state)

        # 4. Apply primary formatting
        formatted_result = await formatting_engine.format_response(
        ...
        # 6. Apply pretty output layer
        final_text = await pretty_layer.render(
        ...
        # 7. Extract specialized payload (for UI components)
    ...
    async def _auth_gate(self, state: LangGraphOrchestrationState) -> LangGraphOrchestrationState:
    ...
    async def _safety_gate(self, state: LangGraphOrchestrationState) -> LangGraphOrchestrationState:
    ...
    async def _memory_fetch(self, state: LangGraphOrchestrationState) -> LangGraphOrchestrationState:
        ...
        try:
            ...
            session_state_manager = await self._ensure_session_state_manager()
            session_id = state.get("session_id")
    ...
    async def _intent_detect(self, state: LangGraphOrchestrationState) -> LangGraphOrchestrationState:
    ...
    async def _planner(self, state: LangGraphOrchestrationState) -> LangGraphOrchestrationState:
    ...
    async def _router_select(self, state: LangGraphOrchestrationState) -> LangGraphOrchestrationState:
    ...
    async def _tool_exec(self, state: LangGraphOrchestrationState) -> LangGraphOrchestrationState:
    ...
    async def _response_synth(self, state: LangGraphOrchestrationState) -> LangGraphOrchestrationState:
        """Response synthesis and generation with salvaged formatting and policy enforcement."""
        ...
        try:
            ...
            # Salvaged: Run the full formatting and policy enforcement pipeline
            (
                formatted_text,
                formatting_payload,
            ) = await self._format_response_with_engine(response_text, state)

            state["response"] = formatted_text
            state["formatted_response"] = formatted_text
            state["formatting_payload"] = formatting_payload
            ...
            state.setdefault("messages", []).append(AIMessage(content=formatted_text))
    ...
    async def _approval_gate(self, state: LangGraphOrchestrationState) -> LangGraphOrchestrationState:
    ...
    async def _memory_write(self, state: LangGraphOrchestrationState) -> LangGraphOrchestrationState:
    ...
    async def run_dry_run_analysis(
        ...
        *,
        ...
        if provider_selection:
            ...
            routing_reason = (
                f"Selected via {self._llm_router.routing_policy.value} policy"
            )
    ...
    async def process(
        ...
        initial_state: LangGraphOrchestrationState = {
            "messages": messages,
            "user_id": user_id,
            "session_id": session_id,
            "tenant_id": None,
            ...
            "auth_context": cast(
                Dict[str, Any], runtime_config.get("auth_context") or {}
            ),
            "user_profile": None,
            "safety_status": None,
            "safety_flags": None,
            ...
            "streaming_enabled": bool(
                runtime_config.get("streaming_enabled", self.config.streaming_enabled)
            ),
            "stream_chunks": None,
...
def create_orchestrator(config: LangGraphOrchestrationConfig = None) -> LangGraphOrchestrator:
