...
class ServiceConfig:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ServiceConfig":
        ...
        if "resource_requirements" in data and isinstance(
            data["resource_requirements"], dict
        ):
            data["resource_requirements"] = ResourceRequirements(
...
class ServiceConfigurationLoader:
    ...
    def __init__(self, config_paths: Optional[List[Union[str, Path]]] = None):
        ...
        self.config_paths = config_paths or [
            Path("config_assets/services.yml"),
            Path("config_assets/services.yaml"),
            Path("config_assets/services.json"),
        ...
        optional_services = {
            ...
            "langgraph_orchestrator": ServiceConfig(
                ...
                resource_requirements=ResourceRequirements(
                    memory_mb=512, gpu_memory_mb=1024
                ),
                gpu_compatible=True,
                idle_timeout=600,
            ),
    ...
    def save_configuration(self, output_path: Union[str, Path]) -> None:
        ...
        data = {
            "services": {
                name: config.to_dict() for name, config in self.services.items()
            },
            "deployment_profiles": {
                mode.value: profile
    ...
    def get_services_by_classification(
        self, classification: ServiceClassification
    ) -> List[ServiceConfig]:
    ...
    def get_services_for_deployment_mode(
        ...
        enabled_classifications = profile.get(
            "enabled_classifications", [ServiceClassification.ESSENTIAL]
        )
...
class DependencyGraphAnalyzer:
        ...
        for service_name, config in self.services.items():
            ...
            for dep_name in config.dependencies:
                if dep_name in self.graph:
                    ...
                    logger.warning(
                        f"Service {service_name} depends on unknown service: {dep_name}"
                    )
        ...
        for service_name, config in self.services.items():
            if config.classification == ServiceClassification.ESSENTIAL:
                for dep_name in config.dependencies:
                    ...
                    if (
                        dep_config
                        and dep_config.classification != ServiceClassification.ESSENTIAL
                    ):
                        errors.append(
    ...
    def get_consolidation_groups(self) -> Dict[str, List[str]]:
        ...
        return {
            group: services for group, services in groups.items() if len(services) > 1
        }
    ...
    def analyze_resource_requirements(self) -> Dict[str, Any]:
        ...
        all_enabled_services = [
            config for config in self.services.values() if config.enabled
        ]
        analysis["total"] = ResourceRequirements(
            memory_mb=sum(
                (config.resource_requirements.memory_mb or 0)
                for config in all_enabled_services
            ),
            cpu_cores=sum(
                (config.resource_requirements.cpu_cores or 0)
                for config in all_enabled_services
            ),
            gpu_memory_mb=sum(
                (config.resource_requirements.gpu_memory_mb or 0)
                for config in all_enabled_services
            ),
        )
...
class ServiceConfigurationValidator:
    ...
    def _validate_essential_services(self) -> List[str]:
        ...
        warnings = []
        essential_services = self.loader.get_services_by_classification(
            ServiceClassification.ESSENTIAL
        )
        ...
        if missing_essential:
            warnings.append(
                f"Missing recommended essential services: {', '.join(missing_essential)}"
            )
        ...
        for service in essential_services:
            if service.startup_priority > 50:
                warnings.append(
                    f"Essential service {service.name} has low startup priority ({service.startup_priority})"
                )
    ...
    def _validate_resource_requirements(self) -> List[str]:
        ...
        for service_name, config in self.loader.services.items():
            ...
            if req.memory_mb and req.memory_mb > 1024:
                warnings.append(
                    f"Service {service_name} has high memory requirement: {req.memory_mb}MB"
                )

            # Check for GPU requirements on non-GPU compatible services
            if req.gpu_memory_mb and not config.gpu_compatible:
                warnings.append(
                    f"Service {service_name} has GPU memory requirement but is not GPU compatible"
                )
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        optional_services = self.loader.get_services_by_classification(
            ServiceClassification.OPTIONAL
        )
        for service in optional_services:
            if service.idle_timeout is None:
                recommendations.append(
                    f"Consider adding idle timeout to optional service: {service.name}"
                )

        # Check for background services with high priority
        background_services = self.loader.get_services_by_classification(
            ServiceClassification.BACKGROUND
        )
        for service in background_services:
            if service.startup_priority < 100:
                recommendations.append(
                    f"Background service {service.name} has high startup priority"
                )
