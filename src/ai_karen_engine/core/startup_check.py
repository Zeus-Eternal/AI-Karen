...
class StartupChecker:
    ...
    def __init__(self):
        ...
        self.required_files = [
            "config_assets/config.json",
            "config_assets/llm_profiles.yml",
            "config_assets/memory.yml",
        ]
        self.required_dirs = [
            "data",
            "logs",
        ...
        for config_file in self.required_files:
            ...
            if not config_path.exists():
                if auto_fix:
                    ...
                    try:
                        from ai_karen_engine.core.initialization import (
                            initialize_system,
                        )

                        self.logger.info(
                            f"🔧 Initializing system to create missing config: {config_file}"
                        )
                        await initialize_system()
                        break  # initialization will create all configs
                    except Exception as e:
                        issues.append(
                            f"Failed to initialize system for config {config_file}: {e}"
                        )
                else:
    ...
    async def _check_models_availability(self, auto_fix: bool) -> List[str]:
        ...
        has_transformers = transformers_cache.exists() and any(
            transformers_cache.iterdir()
        )
        ...
        if not model_files and not has_transformers:
            if auto_fix:
                try:
                    ...
                    self.logger.info(
                        "📥 No models found, initializing system to download default models..."
                    )
                    await initialize_system()
                except Exception as e:
                ...
                issues.append(
                    "No models available - run system initialization to download default models"
                )
        else:
            self.logger.info(
                f"✅ Found {len(model_files)} model files and transformers cache: {has_transformers}"
            )
    ...
    async def _check_python_dependencies(self) -> List[str]:
        ...
        for module_name, display_name in critical_deps:
            try:
                ...
                issues.append(
                    f"Missing critical dependency: {display_name} ({module_name})"
                )
        ...
        if missing_optional:
            self.logger.info(
                f"ℹ️ Optional dependencies not available: {', '.join(missing_optional)}"
            )
    ...
    async def get_system_status(self) -> Dict[str, Any]:
        ...
        # Models status
        if self.models_dir.exists():
            model_files = list(self.models_dir.rglob("*.gguf")) + list(
                self.models_dir.rglob("*.bin")
            )
            status["models"] = {
