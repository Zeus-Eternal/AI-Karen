...
async def setup_default_health_checks() -> None:
    ...
    async def check_langgraph_orchestrator():
        try:
            from ai_karen_engine.core.services.service_registry import (
                get_langgraph_orchestrator,
            )

            service = await get_langgraph_orchestrator()
            if hasattr(service, "health_check"):
                return await service.health_check()
            return {"status": "healthy", "message": "LangGraph Orchestrator OK"}
        except Exception as e:
            return {
                "status": "unhealthy",
                "message": f"LangGraph Orchestrator error: {str(e)}",
            }
    ...
    async def check_extensions():
        try:
            from ai_karen_engine.extensions.platform.core.manager import get_plugin_manager as get_extension_manager
    ...
    monitor.register_health_check(
        "langgraph_orchestrator", check_langgraph_orchestrator, interval=30, critical=True
    )
    monitor.register_health_check(
