    ...
    def get_memory_usage(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "pool_blocks": sum(
                    len(blocks) for blocks in self._memory_pool.values()
                ),
...
class GPUComputeOffloader:
    ...
    def _bind_runtime_configuration(self) -> None:
        ...
        try:
            ...
            logger.debug(
                f"GPU offloader could not bind optimization config manager: {exc}"
            )
    ...
    def _apply_environment_overrides(self) -> None:
        ...
        if os.getenv("KARI_ENABLE_CUDA"):
            self.cuda_enabled = os.getenv("KARI_ENABLE_CUDA", "true").lower() in (
                "1",
                "true",
                "yes",
            )
        if os.getenv("KARI_CUDA_MEMORY_FRACTION"):
            try:
                self.max_gpu_memory_usage = float(
                    os.getenv(
                        "KARI_CUDA_MEMORY_FRACTION", str(self.max_gpu_memory_usage)
                    )
                )
            except ValueError:
                pass
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if self._gpu_info.backend != GPUBackend.NONE:
                logger.info(
                    f"GPU detected: {self._gpu_info.backend.value} "
    ...
    async def detect_gpu_availability(self) -> GPUInfo:
        ...
        try:
            ...
            if self.cuda_enabled:
                ...
                logger.info(
                    "CUDA detection disabled by optimization/admin configuration"
                )
    ...
    async def _detect_cuda(self) -> GPUInfo:
                ...
                try:
                    result = await asyncio.wait_for(
                        asyncio.create_subprocess_exec(
                            "nvidia-smi",
                            "--query-gpu=count,name,memory.total,memory.free",
        ...
        return GPUInfo(
            backend=GPUBackend.NONE, device_count=0, total_memory=0, available_memory=0
        )
    ...
    async def _detect_metal(self) -> GPUInfo:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if "Metal" in output and (
                    "AMD" in output or "Intel" in output or "Apple" in output
                ):
    ...
    async def offload_to_gpu(
        ...
        if self._gpu_info.backend != GPUBackend.NONE and self._can_allocate_memory(
            memory_requirement
        ):
            try:
    ...
    async def _execute_on_gpu(
        ...
        if memory_requirement:
            ...
            if not memory_block:
                raise RuntimeError(
                    f"Failed to allocate {memory_requirement}MB GPU memory"
                )
    ...
    async def fallback_to_cpu(
        self, computation: Callable, data: Any, timeout: Optional[float] = None
    ) -> Any:
        ...
        if timeout:
            return await asyncio.wait_for(
                loop.run_in_executor(self._executor, cpu_computation), timeout=timeout
            )
        else:
    ...
    async def manage_gpu_memory(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self._gpu_info and self._gpu_info.backend != GPUBackend.NONE:
                ...
                estimated_available = (
                    self._gpu_info.total_memory - memory_stats["total_allocated_mb"]
                )
    ...
    async def batch_offload_to_gpu(
        self,
        ...
        for i, (computation, data) in enumerate(tasks):
            ...
            coroutines.append(
                self.offload_to_gpu(computation, data, priority, task_timeout)
            )
        ...
        try:
            ...
            utilization = {
                ...
                "memory_utilization_percent": (
                    ...
                    / self._gpu_info.total_memory
                    * 100
                    if self._gpu_info.total_memory > 0
                    else 0
                ),
    ...
    async def shutdown(self):
        ...
        try:
            ...
            if self._memory_manager:
                ...
                memory_stats = self._memory_manager.get_memory_usage()
                logger.info(
                    f"Cleaning up {memory_stats['active_blocks']} GPU memory blocks"
                )
...
async def create_gpu_offloader(
    max_workers: int = 4,
    memory_strategy: GPUMemoryStrategy = GPUMemoryStrategy.LAZY,
    enable_cpu_fallback: bool = True,
) -> GPUComputeOffloader:
...
def is_gpu_computation_beneficial(
    data_size_mb: float, computation_complexity: str = "medium"
) -> bool:
    """
...
def image_processing_gpu_optimized(image_data: Dict[str, Any]) -> Any:
    ...
    try:
        ...
        try:
            ...
            return {
                "processed_image": result_image.tolist()
