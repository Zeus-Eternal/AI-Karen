...
class SystemMonitor:
    ...
    def _collect_system_metrics(self):
        ...
        try:
            ...
            self.metrics_collector.record_metric(
                Metric(
                    "system.memory.used_mb", memory.used / 1024 / 1024, MetricType.GAUGE
                )
            )
            self.metrics_collector.record_metric(
                Metric(
...
async def langgraph_orchestrator_health_check() -> HealthCheck:
    """LangGraph Orchestrator health check"""
    try:
        # This would typically check LangGraph orchestrator functionality
        return HealthCheck(
            name="langgraph_orchestrator",
            status=HealthStatus.HEALTHY,
            message="LangGraph Orchestrator OK",
        )
    except Exception as e:
        return HealthCheck(
            name="langgraph_orchestrator",
            status=HealthStatus.UNHEALTHY,
            message=f"LangGraph Orchestrator failed: {str(e)}",
        )
...
class AnalyticsService:
    ...
    def get_usage_report(self, hours: int) -> Dict[str, Any]:
        ...
        if relevant_performance:
            success_rate = sum(
                1 for metric in relevant_performance if metric.success
    ...
    def get_user_metrics(self, user_id: str, hours: int = 24) -> Dict[str, Any]:
        ...
        for session in sessions_snapshot.values():
            ...
            session_durations.append(
                max((last_activity - start_time).total_seconds() / 60.0, 0.0)
            )
        ...
        average_session_minutes = (
            round(total_session_minutes / session_count, 2) if session_count else 0.0
        )
    ...
    def _register_default_health_checks(self):
        ...
        self.health_checker.register_health_check(
            "memory_service", memory_service_health_check
        )
        self.health_checker.register_health_check(
            "langgraph_orchestrator", langgraph_orchestrator_health_check
        )
    ...
    def set_alert_threshold(
        ...
        """Set alert threshold for a metric"""
        self.alert_manager.set_threshold(
            metric_name, threshold_value, level, comparison
        )
    ...
    def get_metric_stats(self, metric_name: str) -> Dict[str, Any]:
        ...
        if metric_name in self.metrics_collector.counters:
            return {
                "type": "counter",
                "value": self.metrics_collector.get_counter(metric_name),
            }
            ...
            return {
                "type": "histogram",
                **self.metrics_collector.get_histogram_stats(metric_name),
            }
