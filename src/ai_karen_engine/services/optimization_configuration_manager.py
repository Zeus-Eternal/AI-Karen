...
class ModelDiscoveryConfig:
    ...
    supported_formats: List[str] = field(
        default_factory=lambda: ["gguf", "safetensors", "bin", "pt", "pth"]
    )
...
class ModelRoutingConfig:
    ...
    routing_strategy: str = (
        "capability_based"  # capability_based, performance_based, round_robin
    )
...
class OptimizationConfiguration:
    ...
    content_optimization: ContentOptimizationConfig = field(
        default_factory=ContentOptimizationConfig
    )
    streaming: StreamingConfig = field(default_factory=StreamingConfig)
    cuda: CudaConfig = field(default_factory=CudaConfig)
    monitoring: MonitoringConfig = field(default_factory=MonitoringConfig)
    reasoning_preservation: ReasoningPreservationConfig = field(
        default_factory=ReasoningPreservationConfig
    )
...
class OptimizationConfigurationManager:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        ...
        self.config_path = config_path or Path("config_assets/optimization_config.json")
        ...
        self.logger.info(
            f"Optimization Configuration Manager initialized with config: {self.config_path}"
        )
    ...
    def _load_configuration(self):
        ...
        try:
            if self.config_path.exists():
                ...
                self.logger.info(
                    f"Loaded optimization configuration from {self.config_path}"
                )
            else:
    ...
    def _deserialize_config(
        ...
        try:
            ...
            if "model_discovery" in config_data:
                config.model_discovery = ModelDiscoveryConfig(
                    **config_data["model_discovery"]
                )

            if "model_routing" in config_data:
                config.model_routing = ModelRoutingConfig(
                    **config_data["model_routing"]
                )
            ...
            if "content_optimization" in config_data:
                config.content_optimization = ContentOptimizationConfig(
                    **config_data["content_optimization"]
                )
            ...
            if "monitoring" in config_data:
                ...
                if "alert_thresholds" not in monitoring_data:
                    monitoring_data["alert_thresholds"] = (
                        MonitoringConfig().alert_thresholds
                    )
                config.monitoring = MonitoringConfig(**monitoring_data)

            if "reasoning_preservation" in config_data:
                config.reasoning_preservation = ReasoningPreservationConfig(
                    **config_data["reasoning_preservation"]
                )
    ...
    def _setup_validation_rules(self):
        ...
        self.validation_rules = {
            "cpu_threshold": lambda config: 0
            < config.performance.cpu_threshold_percent
            <= 100,
            "memory_threshold": lambda config: config.performance.memory_threshold_mb
            > 0,
            "cache_size": lambda config: config.cache.cache_size_mb > 0,
            "ttl_positive": lambda config: config.cache.default_ttl_seconds > 0,
            "model_size_limit": lambda config: config.model_discovery.max_model_size_gb
            > 0,
            "scan_interval": lambda config: config.model_discovery.scan_interval_minutes
            > 0,
            "timeout_positive": lambda config: config.model_routing.connection_timeout_seconds
            > 0,
            "retry_attempts": lambda config: config.model_routing.max_retry_attempts
            >= 0,
            "chunk_size": lambda config: config.streaming.chunk_size_bytes > 0,
            "cuda_memory_fraction": lambda config: 0
            < config.cuda.memory_fraction
            <= 1.0,
            "metrics_retention": lambda config: config.monitoring.metrics_retention_hours
            > 0,
        }

    def validate_configuration(
        self, config: Optional[OptimizationConfiguration] = None
    ) -> Dict[str, List[str]]:
        ...
        try:
            for rule_name, rule_func in self.validation_rules.items():
                try:
                    ...
                    validation_errors["validation"].append(
                        f"Rule {rule_name} error: {str(e)}"
                    )
            ...
            if config.performance.cpu_threshold_percent > 50:
                ...
                validation_errors["performance"].append(
                    "CPU threshold above 50% may impact system performance"
                )

            if config.cache.cache_size_mb > 2000:
                if "cache" not in validation_errors:
                    validation_errors["cache"] = []
                validation_errors["cache"].append(
                    "Cache size above 2GB may consume excessive memory"
                )
    ...
    def update_configuration(
        ...
        try:
            with self._lock:
                ...
                new_config = self._deserialize_config(
    ...
    def _apply_updates(
        self, config: OptimizationConfiguration, updates: Dict[str, Any]
    ):
    ...
    def _notify_config_change(
        self,
        old_config: OptimizationConfiguration,
            ...
            with self._lock:
                if component:
                    ...
                    if component == "model_discovery":
                        ...
                        self.config.reasoning_preservation = (
                            ReasoningPreservationConfig()
                        )
                    else:
                ...
                self.logger.info(
                    f"Reset configuration {'component ' + component if component else 'to defaults'}"
                )
                return True
    ...
    def export_configuration(self, export_path: Optional[Path] = None) -> bool:
        ...
        try:
            export_path = export_path or Path(
                f"optimization_config_export_{int(time.time())}.json"
            )
...
def get_optimization_config_manager(
    config_path: Optional[Path] = None,
) -> OptimizationConfigurationManager:
    ...
    if _optimization_config_manager is None:
        with _config_lock:
            if _optimization_config_manager is None:
                _optimization_config_manager = OptimizationConfigurationManager(
                    config_path
                )
...
def build_gpu_runtime_policy(
    ...
    return GPURuntimePolicy(
        ...
        enable_memory_optimization=bool(
            getattr(cuda_config, "enable_memory_optimization", True)
        ),
        enable_batch_processing=bool(
            getattr(cuda_config, "enable_batch_processing", True)
        ),
