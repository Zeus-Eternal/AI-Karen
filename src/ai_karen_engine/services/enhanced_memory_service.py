...
from .memory_service import (
    MemoryType, UISource, WebUIMemoryEntry, WebUIMemoryQuery, 
    MemoryContextBuilder, WebUIMemoryService
)
...
class MemoryServiceError(Exception):
    ...
    def __init__(self, message: str, error_code: str = "MEMORY_ERROR", details: Dict[str, Any] = None):
...
class EnhancedMemoryService(WebUIMemoryService):
            ...
            try:
                ...
                if e.error_code == "CIRCUIT_BREAKER_OPEN":
                    ...
                    logger.error(
                        ...
                        extra={"correlation_id": correlation_id, "error_code": e.error_code}
                    )
                    self.error_stats["vector_failures"] += 1
                    self._record_error(e, correlation_id)
                
                # Fall back to SQL search
                return await self._fallback_to_sql_search(tenant_id, query, correlation_id)
    ...
    async def _sql_text_search(
        ...
        try:
            async with self.db_client.get_async_session() as session:
                ...
                sql_query = select(TenantMemoryEntry).where(
                    and_(
                        TenantMemoryEntry.user_id == uuid.UUID(query.user_id) if query.user_id else True,
                        TenantMemoryEntry.content.ilike(f"%{query.text}%")
                    )
                ).order_by(desc(TenantMemoryEntry.created_at)).limit(query.top_k)
                ...
                for db_memory in db_memories:
                    try:
                        memory = WebUIMemoryEntry(
                            ...
                            ui_source=UISource(db_memory.ui_source) if db_memory.ui_source else UISource.API,
                            conversation_id=str(db_memory.conversation_id) if db_memory.conversation_id else None,
                            memory_type=MemoryType(db_memory.memory_type) if db_memory.memory_type else MemoryType.GENERAL,
                        ...
                        logger.warning(
                            ...
                            extra={"correlation_id": correlation_id, "memory_id": str(db_memory.id)}
                        )
                        continue
    ...
    def _update_performance_stats(self, query_time: float, operation_type: str):
        ...
        self.performance_stats["avg_query_time"] = (current_avg * (count - 1) + query_time) / count
        
        # Update operation-specific averages
        if operation_type == "vector":
            current_avg = self.performance_stats["avg_vector_time"]
            self.performance_stats["avg_vector_time"] = (current_avg * 0.9 + query_time * 0.1)
        elif operation_type == "sql":
            current_avg = self.performance_stats["avg_sql_time"]
            self.performance_stats["avg_sql_time"] = (current_avg * 0.9 + query_time * 0.1)
        ...
        return {
            ...
            "circuit_breakers": {
                "vector_store": {
                    ...
                    "last_failure": self.vector_circuit_breaker.stats.last_failure_time.isoformat() if self.vector_circuit_breaker.stats.last_failure_time else None
                },
                "sql_fallback": {
                    "state": self.sql_circuit_breaker.stats.state.value,
                    "failure_count": self.sql_circuit_breaker.stats.failure_count,
                    "last_failure": self.sql_circuit_breaker.stats.last_failure_time.isoformat() if self.sql_circuit_breaker.stats.last_failure_time else None
                }
            },
...
def create_enhanced_memory_service(base_memory_manager: MemoryManager) -> EnhancedMemoryService:
