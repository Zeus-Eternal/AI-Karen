    ...
    async def process_e2e_request(
        ...
        try:
            ...
            try:
                ...
                metrics.context_tokens_estimate = len(str(context)) // 4  # Rough estimate
                ...
                async for chunk in self._optimized_llm_generation(
                    ...
                    if first_token_time is None:
                        ...
                        metrics.first_token_latency_ms = (first_token_time - llm_start) * 1000
    ...
    async def _optimized_memory_query(
        ...
        if self.memory_service:
            try:
                ...
                from .unified_memory_service import MemoryQueryRequest
                ...
                response = await self.memory_service.query(tenant_id, memory_request, correlation_id)
                ...
                logger.error(f"Memory service error: {e}", extra={"correlation_id": correlation_id})
                return [], False
    ...
    async def _optimized_llm_generation(
        ...
        if self.llm_service:
            try:
                ...
                logger.error(f"LLM service error: {e}", extra={"correlation_id": correlation_id})
...
Please provide a helpful and accurate response based on the context provided."""
        ...
        try:
            ...
            for cache_type, stats in cache_stats["by_type"].items():
                if stats["hit_rate"] < 0.2:  # Less than 20% hit rate
                    logger.info(f"Low hit rate for {cache_type}: {stats['hit_rate']:.2%}")
    ...
    async def _check_performance_targets(self):
        ...
        total_latencies = [m.total_latency_ms for m in recent_metrics if m.error is None]
        memory_latencies = [m.memory_query_latency_ms for m in recent_metrics if m.error is None]
        first_token_latencies = [m.first_token_latency_ms for m in recent_metrics if m.error is None and m.first_token_latency_ms > 0]
    ...
    def _monitor_connection_pool(self):
        ...
        if stats["utilization"] > 0.8:
            logger.warning(f"High connection pool utilization: {stats['utilization']:.1%}")
        
        # Log errors if there are timeouts
        if stats["metrics"]["timeout_count"] > 0:
            logger.warning(f"Connection pool timeouts: {stats['metrics']['timeout_count']}")
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        total_latencies = [m.total_latency_ms for m in recent_metrics if m.error is None]
        memory_latencies = [m.memory_query_latency_ms for m in recent_metrics if m.error is None]
        context_latencies = [m.context_build_latency_ms for m in recent_metrics if m.error is None]
        llm_latencies = [m.llm_generation_latency_ms for m in recent_metrics if m.error is None]
        first_token_latencies = [m.first_token_latency_ms for m in recent_metrics if m.error is None and m.first_token_latency_ms > 0]
        
        # Cache hit rates
        memory_cache_hits = sum(1 for m in recent_metrics if m.cache_hits.get("memory", False))
        context_cache_hits = sum(1 for m in recent_metrics if m.cache_hits.get("context", False))
    ...
    async def benchmark_e2e_performance(
        ...
        if successful_results:
            ...
            results["summary"] = {
                ...
                "slo_compliance": sorted_latencies[int(len(sorted_latencies) * 0.95)] <= self.performance_targets["total_latency_p95_ms"]
            }
