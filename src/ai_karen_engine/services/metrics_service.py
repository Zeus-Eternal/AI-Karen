...
class FallbackMetricsCollector:
    ...
    def inc_counter(self, name: str, labels: Dict[str, str] = None, value: float = 1):
        ...
        self.recent_values[name].append(MetricValue(
            value=value,
...
class MetricsService:
    ...
    def record_copilot_request(self, status: str, user_id: str = "", org_id: str = "", 
                              correlation_id: Optional[str] = None):
        ...
        logger.debug(f"Recorded copilot request: {labels}", extra={
            "correlation_id": correlation_id,
            "metric_type": "counter"
        })
    
    def record_memory_query(self, operation: str, status: str, user_id: str = "", 
                           org_id: str = "", correlation_id: Optional[str] = None):
        """Record memory query metrics"""
        labels = {"operation": operation, "status": status, "user_id": user_id, "org_id": org_id}
        ...
        logger.debug(f"Recorded memory query: {labels}", extra={
            "correlation_id": correlation_id,
            "metric_type": "counter"
        })
    
    def record_memory_commit(self, status: str, decay_tier: str = "", user_id: str = "", 
                            org_id: str = "", correlation_id: Optional[str] = None):
        """Record memory commit metrics"""
        labels = {"status": status, "decay_tier": decay_tier, "user_id": user_id, "org_id": org_id}
        ...
        logger.debug(f"Recorded memory commit: {labels}", extra={
            "correlation_id": correlation_id,
            "metric_type": "counter"
        })
    
    def record_llm_latency(self, duration: float, provider: str = "", model: str = "", 
                          status: str = "success", correlation_id: Optional[str] = None):
        ...
        if PROMETHEUS_AVAILABLE:
            ...
            self.fallback_collector.observe_histogram("llm_latency_seconds", duration, labels)
        
        # Record for SLO monitoring
        self._record_for_slo_monitoring("llm_latency_seconds", duration)
        
        logger.debug(f"Recorded LLM latency: {duration:.3f}s {labels}", extra={
            "correlation_id": correlation_id,
            "metric_type": "histogram"
        })
    
    def record_vector_latency(self, duration: float, operation: str = "search", 
                             status: str = "success", correlation_id: Optional[str] = None):
        ...
        if PROMETHEUS_AVAILABLE:
            ...
            self.fallback_collector.observe_histogram("vector_latency_seconds", duration, labels)
        
        # Record for SLO monitoring
        self._record_for_slo_monitoring("vector_latency_seconds", duration)
        
        logger.debug(f"Recorded vector latency: {duration:.3f}s {labels}", extra={
            "correlation_id": correlation_id,
            "metric_type": "histogram"
        })
    
    def record_total_turn_time(self, duration: float, endpoint: str, status: str = "success",
                              correlation_id: Optional[str] = None):
        ...
        if PROMETHEUS_AVAILABLE:
            ...
            self.fallback_collector.observe_histogram("total_turn_time_seconds", duration, labels)
        
        # Record for SLO monitoring
        self._record_for_slo_monitoring("total_turn_time_seconds", duration)
        
        logger.debug(f"Recorded total turn time: {duration:.3f}s {labels}", extra={
            "correlation_id": correlation_id,
            "metric_type": "histogram"
        })
    
    def update_memory_quality_metrics(self, context_usage_rate: Optional[float] = None,
        ...
        if PROMETHEUS_AVAILABLE:
            ...
            if context_usage_rate is not None:
                ...
                self.fallback_collector.set_gauge("memory_context_usage_rate", context_usage_rate, labels)
            if ignored_top_hit_rate is not None:
                updates["ignored_top_hit_rate"] = ignored_top_hit_rate
                self.fallback_collector.set_gauge("memory_ignored_top_hit_rate", ignored_top_hit_rate, labels)
            if used_shard_rate is not None:
                updates["used_shard_rate"] = used_shard_rate
                self.fallback_collector.set_gauge("memory_used_shard_rate", used_shard_rate, labels)
            if avg_relevance_score is not None:
                updates["avg_relevance_score"] = avg_relevance_score
                self.fallback_collector.set_gauge("memory_avg_relevance_score", avg_relevance_score, labels)
            
            self.fallback_collector.update_memory_quality(**updates)
        
        logger.debug(f"Updated memory quality metrics: {labels}", extra={
            "correlation_id": correlation_id,
            "metric_type": "gauge"
        })
    
    def record_model_performance(self, score: float, model: str, task_type: str,
                                correlation_id: Optional[str] = None):
        ...
        if PROMETHEUS_AVAILABLE:
            ...
            self.fallback_collector.observe_histogram("model_performance_score", score, labels)
        
        logger.debug(f"Recorded model performance: {score:.3f} {labels}", extra={
            "correlation_id": correlation_id,
            "metric_type": "summary"
        })
    
    def update_system_health(self, active_connections: Optional[int] = None,
                            memory_usage: Optional[int] = None,
                            service: str = "api", memory_type: str = "heap",
                            correlation_id: Optional[str] = None):
        ...
        if PROMETHEUS_AVAILABLE:
            ...
            if memory_usage is not None:
                self.memory_usage.labels(service=service, type=memory_type).set(memory_usage)
        else:
            if active_connections is not None:
                self.fallback_collector.set_gauge("active_connections_total", active_connections, {"service": service})
            if memory_usage is not None:
                self.fallback_collector.set_gauge("memory_usage_bytes", memory_usage, {"service": service, "type": memory_type})
        
        logger.debug(f"Updated system health: service={service}", extra={
            "correlation_id": correlation_id,
            "metric_type": "gauge"
        })
    
    def update_turn_health(self, success_rate: Optional[float] = None,
                          error_rate: Optional[float] = None,
                          endpoint: str = "", error_type: str = "",
                          time_window: str = "5m",
                          correlation_id: Optional[str] = None):
        """Update turn health metrics"""
        if PROMETHEUS_AVAILABLE:
            if success_rate is not None:
                self.turn_success_rate.labels(endpoint=endpoint, time_window=time_window).set(success_rate)
            if error_rate is not None:
                self.turn_error_rate.labels(endpoint=endpoint, error_type=error_type, time_window=time_window).set(error_rate)
        else:
            if success_rate is not None:
                self.fallback_collector.set_gauge("turn_success_rate", success_rate, {"endpoint": endpoint, "time_window": time_window})
            if error_rate is not None:
                self.fallback_collector.set_gauge("turn_error_rate", error_rate, {"endpoint": endpoint, "error_type": error_type, "time_window": time_window})
        
        logger.debug(f"Updated turn health: endpoint={endpoint}", extra={
            "correlation_id": correlation_id,
            "metric_type": "gauge"
        })
    ...
    def time_operation(self, operation_name: str, labels: Dict[str, str] = None,
        ...
        try:
            ...
            if "llm" in operation_name.lower():
                self.record_llm_latency(duration, correlation_id=correlation_id, **labels)
            elif "vector" in operation_name.lower() or "search" in operation_name.lower():
                self.record_vector_latency(duration, correlation_id=correlation_id, **labels)
            elif "turn" in operation_name.lower():
                self.record_total_turn_time(duration, correlation_id=correlation_id, **labels)
