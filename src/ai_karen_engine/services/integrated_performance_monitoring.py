...
from ai_karen_engine.monitoring.metrics_service import (
    MetricsService,
    get_metrics_service,
)
...
class IntegratedPerformanceMonitor:
        ...
        try:
            ...
            if self.performance_metrics_service:
                try:
                    ...
                    response_metrics = {
                        "avg_response_time_ms": response_stats.get(
                            "avg_response_time_ms", 0
                        ),
                        "max_response_time_ms": response_stats.get(
                        ...
                        "resource_pressure_count": response_stats.get(
            ...
            try:
                ...
                if isinstance(metrics_stats, dict):
                    reasoning_metrics = {
                        "llm_requests": metrics_stats.get("counters", {}).get(
                            "copilot_requests_total", 0
                        ),
                        "memory_queries": metrics_stats.get("counters", {}).get(
                            "memory_queries_total", 0
                        ),
                        "avg_llm_latency_ms": self._extract_histogram_avg(
                            metrics_stats.get("histograms", {}).get(
                                "llm_latency_seconds", {}
                            )
                        )
                        * 1000,
    ...
    async def _send_to_existing_metrics(self, integrated: IntegratedMetrics):
        ...
        try:
            ...
            if integrated.response_metrics:
                ...
                if "avg_cpu_percent" in integrated.response_metrics:
                    self.metrics_service.update_system_health(
                        memory_usage=int(
                            integrated.system_metrics.get("memory_mb", 0) * 1024 * 1024
                        ),
            ...
            if (
                integrated.model_metrics
                and "routing_decisions" in integrated.model_metrics
            ):
    ...
    async def _check_alert_conditions(self):
        ...
        try:
            ...
            response_time = latest_metrics.response_metrics.get(
                "avg_response_time_ms", 0
            )
            ...
            cache_hit_rate = (
                latest_metrics.cache_metrics.get("cache_hit_rate", 100) * 100
            )
    ...
    async def _update_aggregated_stats(self):
        ...
        try:
            with self._lock:
                ...
                recent_metrics = [
                    m for m in self.integrated_metrics if m.timestamp >= cutoff_time
                ]
            ...
            with self._lock:
                ...
                if recent_metrics:
                    ...
                    dashboard_data["component_health"] = {
                        ...
                        "cache_system": latest.cache_metrics.get("cache_hit_rate", 0.5)
                        * 100,
                        "system_resources": 100
                        - latest.system_metrics.get("cpu_percent", 0),
                    }
                ...
                if len(recent_metrics) >= 10:
                    ...
                    cpu_usage = [
                        m.system_metrics.get("cpu_percent", 0) for m in trend_metrics
                    ]
    ...
    def get_integration_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "performance_service_available": self.performance_metrics_service
            is not None,
