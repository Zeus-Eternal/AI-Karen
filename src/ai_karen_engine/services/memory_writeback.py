...
class MemoryWritebackSystem:
    ...
    async def link_response_to_shards(
        ...
        try:
            ...
            for i, context_hit in enumerate(source_context_hits[:self.max_shard_links_per_response]):
    ...
    async def process_writeback_batch(self) -> int:
        ...
        try:
            ...
            self._pending_writebacks = self._pending_writebacks[self.writeback_batch_size:]
            ...
            for entry in batch:
                try:
                    ...
                    from .unified_memory_service import MemoryCommitRequest
                    
                    # Determine decay tier based on interaction type and importance
                    decay_tier = self._determine_decay_tier(entry.interaction_type, entry.importance)
            ...
            logger.info(f"Processed {processed_count} writeback entries in {processing_time_ms:.2f}ms")
    ...
    async def calculate_feedback_metrics(
        ...
        try:
            ...
            used_shards = len([link for link in relevant_links if link.usage_type == ShardUsageType.USED_IN_RESPONSE])
            ignored_top_hits = len([
                link for link in relevant_links 
                if link.usage_type == ShardUsageType.IGNORED_TOP_HIT and link.position_in_results == 0
            ])
            
            used_shard_rate = used_shards / total_retrievals if total_retrievals > 0 else 0.0
            ignored_top_hit_rate = ignored_top_hits / total_retrievals if total_retrievals > 0 else 0.0
            
            # Calculate average relevance score
            avg_relevance = sum(link.relevance_score for link in relevant_links) / total_retrievals
    ...
    def _extract_content_snippet(self, shard_content: str, response_content: str) -> str:
        """Extract relevant snippet from shard content"""
        try:
            # Simple extraction - first 100 characters
            return shard_content[:100] + "..." if len(shard_content) > 100 else shard_content
        except Exception:
            return ""
    ...
    def _determine_decay_tier(self, interaction_type: InteractionType, importance: int) -> str:
        """Determine decay tier based on interaction type and importance"""
        ...
        elif interaction_type in [InteractionType.USER_QUERY, InteractionType.USER_FEEDBACK]:
            if importance >= 7:
                return "medium"
            else:
                return "short"
    ...
    async def _update_feedback_metrics(
        ...
        try:
            ...
            if metrics.total_retrievals > 0:
                metrics.used_shard_rate = metrics.total_used_shards / metrics.total_retrievals
                metrics.ignored_top_hit_rate = metrics.total_ignored_top_hits / metrics.total_retrievals
    ...
    def get_system_metrics(self) -> Dict[str, Any]:
        ...
        return {
            **self.metrics,
            ...
            "active_shard_links": sum(len(links) for links in self._shard_links.values()),
