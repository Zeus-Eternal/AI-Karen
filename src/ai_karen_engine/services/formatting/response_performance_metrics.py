...
class ResponsePerformanceCollector:
    ...
    async def get_recent_performance_summary(self, window_seconds: int = 300) -> Dict[str, Any]:
        """Return a lightweight summary of recent response performance."""
        ...
        response_times = [m.response_time * 1000.0 for m in recent]
        cpu_values = [max(0.0, float(m.cpu_usage)) for m in recent]
        memory_values = [max(0, int(m.memory_usage)) for m in recent]
        resource_pressure_count = sum(
            1 for m in recent if m.cpu_usage > 80 or m.memory_usage > 1_000_000_000
        )
        ...
        return {
            ...
            "resource_pressure_count": resource_pressure_count,
        }
...
async def get_performance_metrics_service() -> ResponsePerformanceCollector:
    """Return the shared performance collector instance."""
