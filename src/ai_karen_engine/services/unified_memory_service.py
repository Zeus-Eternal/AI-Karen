...
from .memory_policy import MemoryPolicy, MemoryPolicyManager, DecayTier, ImportanceLevel
from .memory_writeback import MemoryWritebackSystem, InteractionType, ShardUsageType
...
class UnifiedMemoryService:
    ...
    async def commit(
        ...
        try:
            ...
            if decay_tier == DecayTier.SHORT and request.importance >= self.policy.importance_long_threshold:
                # Auto-promote to long-term based on importance
                decay_tier = DecayTier.LONG
                logger.info(f"Auto-promoted memory to long-term based on importance {request.importance}")
            ...
            if not memory_id:
                raise ValueError("Failed to store memory - content not surprising enough")
        ...
        try:
            ...
            if new_importance is not None:
                ...
                if abs(new_importance - old_importance) >= 2:
                    ...
                    # Update expiry date
                    created_at = datetime.fromisoformat(current_metadata.get("created_at", datetime.utcnow().isoformat()))
                    new_expiry = self.policy.calculate_expiry_date(new_decay_tier, created_at)
                    if new_expiry:
            ...
            if new_content is not None:
                ...
                vector_metadata = {
                    ...
                    "kind": "memory",
                    "timestamp": time.time(),
                    **{k: v for k, v in updated_metadata.items() if k not in ["content", "embedding"]}
                }
    ...
    async def _apply_policy_filtering(self, memories: List[MemoryEntry]) -> List[MemoryEntry]:
    ...
    async def get_feedback_metrics(self, tenant_id: Union[str, uuid.UUID]) -> Dict[str, float]:
        ...
        total_retrievals = sum(stats.total_retrievals for stats in self._usage_stats.values())
        
        used_shard_rate = total_usage / total_retrievals if total_retrievals > 0 else 0.0
    ...
    async def run_policy_adjustment(self, tenant_id: Union[str, uuid.UUID]) -> Dict[str, Any]:
        ...
        try:
            async with self.db_client.get_async_session() as session:
                ...
                memory_entry = MemoryEntry(
                    ...
                    timestamp=memory_record.created_at.timestamp() if memory_record.created_at else time.time()
                )
    ...
    async def read(
        ...
        try:
            ...
            context_hits = await self._convert_to_context_hits([memory], user_id, org_id)
        ...
        try:
            async with self.db_client.get_async_session() as session:
                ...
                if not include_deleted:
                    conditions.append(
                        or_(
                            TenantMemoryItem.metadata.op('->>')('soft_deleted').is_(None),
                ...
                for record in memory_records:
                    memory_entry = MemoryEntry(
                        ...
                        scope=record.scope,
                        kind=record.kind,
                        timestamp=record.created_at.timestamp() if record.created_at else time.time()
                    )
                    memories.append(memory_entry)
                
                # Convert to ContextHit format
                context_hits = await self._convert_to_context_hits(memories, user_id, org_id)
        ...
        try:
            async with self.db_client.get_async_session() as session:
                ...
                for tier in ["short", "medium", "long", "pinned"]:
                    ...
                    tier_query = select(func.count(TenantMemoryItem.id)).where(and_(*tier_conditions))
                ...
                recent_query = select(func.count(TenantMemoryItem.id)).where(and_(*recent_conditions))
                ...
                deleted_query = select(func.count(TenantMemoryItem.id)).where(and_(*deleted_conditions))
                ...
                return {
                    ...
                    "usage_stats": {
                        ...
                        "total_retrievals": sum(s.total_retrievals for s in self._usage_stats.values()),
                        "total_usage": sum(s.usage_count for s in self._usage_stats.values())
                    },
