...
from .deduplication import get_audit_deduplication_service
                        ...
                        if file_modified < cutoff_date:
                            ...
                            if compress_before_delete:
                                ...
                                if compressed_file.exists():
                                    ...
                                    stats["errors"].append(
                                        f"Failed to compress {log_file.name}"
                                    )
                            else:
        ...
        try:
            ...
            for log_file in self.log_directory.iterdir():
                if log_file.is_file() and log_file.suffix in [".log", ".gz"]:
                    ...
                    # Track file types
                    file_type = (
                        "compressed" if log_file.suffix == ".gz" else "uncompressed"
                    )
    ...
    def cleanup_all(
        ...
        try:
            ...
            cleanup_stats["file_cleanup"] = self.cleanup_old_log_files(
                max_age_days=max_age_days, compress_before_delete=compress_old_files
            )
...
__all__ = ["AuditLogCleanupService", "get_audit_cleanup_service"]
