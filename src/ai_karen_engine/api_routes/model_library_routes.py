...
from services.memory.model_library_service import (
    ModelLibraryService,
    ModelInfo,
    DownloadTask,
    ModelMetadata,
...
def get_model_library_service() -> ModelLibraryService:
    ...
    if _model_library_service is None:
        try:
            ...
            # The service expects a registry_path string parameter
            _model_library_service = ModelLibraryService("model_registry.json")
            ...
            _model_library_service = ModelLibraryService("fallback_model_registry.json")
    ...
    try:
        ...
        for model in models:
            if model.provider not in providers:
                providers[model.provider] = {"total": 0, "local": 0, "available": 0}
...
async def get_cache_info():
    ...
    try:
        ...
        return {"service_cache": service_cache_info, "api_cache": api_cache_info}
...
class CacheConfigRequest(BaseModel):
    ...
    ttl_seconds: int = Field(
        ge=30, le=3600, description="Cache TTL in seconds (30-3600)"
    )
...
async def configure_cache(request: CacheConfigRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to configure cache: {str(e)}"
        )
...
async def get_available_models(
    ...
    try:
        ...
        cache_key = (
            f"quick={quick}|provider={provider}|status={status}|capability={capability}"
        )
        now = time.time()
            ...
            if quick:
                ...
                try:
                    ...
                    discovery_cache_path = Path(
                        "models/.discovery_cache/discovery_cache.json"
                    )
                    if discovery_cache_path.exists():
                        with open(discovery_cache_path, "r") as f:
                            discovery_data = json.load(f)
                        logger.debug(
                            f"Found {len(discovery_data)} models in discovery cache"
                        )
                        ...
                        for model_id, model_data in discovery_data.items():
                            try:
                                ...
                                logger.debug(
                                    f"Failed to parse discovery model {model_id}: {e}"
                                )
                                continue
                    else:
                ...
                for entry in raw:
                    try:
                        ...
                        seen_ids.add(mid)
                        prov = str(
                            entry.get("provider") or entry.get("type") or "local"
                        )
                        ...
                        model_responses.append(
                            ModelInfoResponse(
                                ...
                                provider=prov,
                                size=0,
                                description=(entry.get("metadata") or {}).get(
                                    "description", f"Local model: {mid}"
                                )
                                if entry.get("metadata")
                                else f"Local model: {mid}",
                                ...
                                download_date=(entry.get("downloadInfo") or {}).get(
                                    "downloadDate"
                                ),
                            )
                        )
                    ...
                    try:
                        ...
                        model_responses.append(
                            ModelInfoResponse(
                                ...
                                description=discovered_model.metadata.description
                                if discovered_model.metadata
                                else f"Discovered model: {discovered_model.name}",
                                capabilities=list(discovered_model.capabilities or []),
                                status=discovered_model.status,
                                download_progress=None,
                                metadata={
                                    "category": discovered_model.category.value
                                    if hasattr(discovered_model.category, "value")
                                    else str(discovered_model.category),
                                    "specialization": [
                                        s.value if hasattr(s, "value") else str(s)
                                        for s in (discovered_model.specialization or [])
                                    ],
                                    "tags": discovered_model.tags or [],
                                    "requirements": discovered_model.requirements.__dict__
                                    if discovered_model.requirements
                                    else {},
                                    "modalities": [
                                        m.__dict__ if hasattr(m, "__dict__") else m
                                        for m in (discovered_model.modalities or [])
                                    ],
                                }
                                if discovered_model.metadata
                                else {},
                ...
                try:
                    models = await asyncio.wait_for(
                        asyncio.to_thread(
                            service.get_available_models_fast, force_refresh
                        ),
                    ...
                    logger.warning(
                        "Model library service timed out, falling back to quick mode"
                    )
        ...
        try:
            ...
            models_data = await asyncio.wait_for(
                get_models_with_timeout(), timeout=timeout_seconds
            )

            if (
                isinstance(models_data, list)
                and len(models_data) > 0
                and hasattr(models_data[0], "id")
            ):
            ...
            return ModelLibraryResponse(
                models=[], total_count=0, local_count=0, available_count=0
            )
        ...
        if isinstance(filtered_models, list) and len(filtered_models) > 0:
            if hasattr(filtered_models[0], "id"):
                ...
                if capability:
                    filtered_models = [
                        m for m in filtered_models if capability in m.capabilities
                    ]
                ...
                for model in filtered_models:
                    try:
                        ...
                        logger.warning(
                            f"Skipping malformed model entry: {getattr(model, 'name', None)} ({map_err})"
                        )
                        continue
            else:
                ...
                if provider:
                    model_responses = [
                        m for m in model_responses if m.provider == provider
                    ]
                if status:
        ...
        logger.error(
            f"Model library error getting available models: {e.error_info.message}"
        )
...
async def initiate_model_download(request: DownloadRequest):
    ...
    try:
        ...
        logger.error(
            f"Disk space error initiating download for {request.model_id}: {e.error_info.message}"
        )
        error_handler = ErrorHandler()
        error_response = error_handler.create_error_response(e.error_info)
        raise HTTPException(
            status_code=507, detail=error_response
        )  # 507 Insufficient Storage

    except PermissionError as e:
        logger.error(
            f"Permission error initiating download for {request.model_id}: {e.error_info.message}"
        )
        error_handler = ErrorHandler()
        error_response = error_handler.create_error_response(e.error_info)
        ...
        logger.error(
            f"Validation error initiating download for {request.model_id}: {e.error_info.message}"
        )
        error_handler = ErrorHandler()
        error_response = error_handler.create_error_response(e.error_info)
        raise HTTPException(status_code=400, detail=error_response)
        ...
        logger.error(
            f"Network error initiating download for {request.model_id}: {e.error_info.message}"
        )
        error_handler = ErrorHandler()
        error_response = error_handler.create_error_response(e.error_info)
        ...
        logger.error(
            f"Model library error initiating download for {request.model_id}: {e.error_info.message}"
        )
        error_handler = ErrorHandler()
        error_response = error_handler.create_error_response(e.error_info)
        raise HTTPException(status_code=500, detail=error_response)
        ...
        logger.error(
            f"Unexpected error initiating download for {request.model_id}: {e}"
        )
        error_info = handle_download_error(
            "unexpected",
...
async def delete_local_model(model_id: str):
    ...
    try:
        ...
        if model_info.status != "local":
            raise HTTPException(
                status_code=400, detail=f"Model {model_id} is not a local model"
            )
...
async def get_model_metadata(model_id: str):
    ...
    try:
        ...
        if not metadata:
            raise HTTPException(
                status_code=404, detail=f"Metadata not found for model {model_id}"
            )
...
async def get_model_usage_stats(model_id: str):
    ...
    try:
        ...
        response = {
            ...
            "disk_usage_mb": round(stats.get("disk_usage", 0) / (1024**2), 2)
...
async def get_model_status_history(model_id: str):
    ...
    try:
        ...
        return {"model_id": model_id, "history": history, "total_events": len(history)}
...
async def cleanup_orphaned_files():
    ...
    try:
        ...
        return {"message": "Orphaned files cleanup completed", "result": cleanup_result}
...
async def validate_model_download(model_id: str):
    ...
    try:
        ...
        if model_id not in predefined:
            raise HTTPException(
                status_code=404,
                detail=f"Model {model_id} not found in predefined models",
            )
        ...
        if not download_url:
            raise HTTPException(
                status_code=400,
                detail=f"No download URL available for model {model_id}",
            )

        validation_result = service.validate_model_before_download(
    ...
    try:
        ...
        for model in models:
            if model.status == "quarantined":
                ...
                quarantine_info = (
                    model_data.get("quarantine_info", {}) if model_data else {}
                )
            ...
            if model_data:
                ...
                if last_scan:
                    ...
                    summary["last_scan_times"].append(
                        {"model_id": model.id, "last_scan": last_scan}
                    )
