...
try:
    from ai_karen_engine.services.slo_monitoring import get_slo_monitor, SLOStatus, AlertSeverity
    ...
    try:
        if not SLO_MONITORING_AVAILABLE:
            ...
            return {
                "fallback": SLOTargetStatus(
                    ...
                    target_value=1.0,
                    status=SLOStatus.UNKNOWN.value if SLO_MONITORING_AVAILABLE else "unknown",
                    thresholds=[]
                )
            }
        ...
        for slo_name, status_data in slo_status.items():
            ...
            for threshold_data in status_data.get("thresholds", []):
                thresholds.append(SLOThresholdStatus(
        ...
        logger.info(f"Retrieved SLO status for {len(response)} targets", extra={
            "correlation_id": correlation_id,
            "slo_count": len(response)
        })
        
        return response
        
    except Exception as e:
        logger.error(f"Failed to get SLO status: {e}", extra={"correlation_id": correlation_id})
    ...
    try:
        if not SLO_MONITORING_AVAILABLE:
            ...
            return SLODashboardResponse(
                ...
                system_health={"status": "unknown", "message": "SLO monitoring not available"}
            )
        ...
        logger.info(f"Retrieved SLO dashboard data", extra={
            "correlation_id": correlation_id,
        ...
        logger.error(f"Failed to get SLO dashboard: {e}", extra={"correlation_id": correlation_id})
...
async def export_metrics(request: Request):
    ...
    try:
        ...
        logger.debug(f"Exported metrics", extra={
            "correlation_id": correlation_id,
        ...
        logger.error(f"Failed to export metrics: {e}", extra={"correlation_id": correlation_id})
...
async def resolve_alert(alert_id: str, request: Request):
    ...
    try:
        if not SLO_MONITORING_AVAILABLE:
            raise HTTPException(
                status_code=503,
                detail="SLO monitoring not available"
            )
        
        slo_monitor = get_slo_monitor()
        slo_monitor.resolve_alert(alert_id)
        
        logger.info(f"Resolved alert: {alert_id}", extra={
            "correlation_id": correlation_id,
            "alert_id": alert_id
        })
        
        return {"success": True, "message": f"Alert {alert_id} resolved", "correlation_id": correlation_id}
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Failed to resolve alert {alert_id}: {e}", extra={"correlation_id": correlation_id})
