...
async def process_flow(
    ...
    langgraph_orchestrator: AIOrchestrator = Depends(
        get_langgraph_orchestrator_service
    ),
):
    ...
    try:
        ...
        result = await langgraph_orchestrator.process_flow(
            request.flow_type, flow_input
        )
...
async def conversation_processing(
    ...
    try:
        ...
        start_time = datetime.utcnow()
        result = await langgraph_orchestrator.conversation_processing_flow(
            flow_input
        )
        processing_time = (datetime.utcnow() - start_time).total_seconds() * 1000
...
async def get_flow_metrics(
    ...
    try:
        metrics = langgraph_orchestrator.get_metrics()
