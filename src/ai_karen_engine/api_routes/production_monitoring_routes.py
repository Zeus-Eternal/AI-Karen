...
from ai_karen_engine.services.auth_service import get_current_user
...
async def get_response_formatting_metrics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to get response formatting metrics"
        )
...
async def get_database_consistency_metrics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to get database consistency metrics"
        )
...
async def get_authentication_anomaly_metrics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to get authentication anomaly metrics"
        )
...
async def get_performance_degradation_metrics(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to get performance degradation metrics"
        )
...
async def resolve_production_alert(
    request: AlertRequest, current_user: dict = Depends(get_current_user)
) -> Dict[str, Any]:
    """
...
async def update_production_metrics(
    request: MetricUpdateRequest, current_user: dict = Depends(get_current_user)
) -> Dict[str, Any]:
    """
...
async def get_prometheus_metrics() -> str:
    ...
    try:
        ...
        if not PROMETHEUS_ENABLED:
            raise HTTPException(
                status_code=503, detail="Prometheus metrics not available"
            )
