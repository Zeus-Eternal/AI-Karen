...
async def get_provider_recommendations(
    provider_name: str, limit: int = Query(10, ge=1, le=50)
):
...
async def get_provider_suggestions(provider_name: str):
    ...
    try:
        suggestions = compatibility_service.get_provider_model_suggestions(
            provider_name
        )
        return suggestions
...
async def get_provider_capabilities():
    ...
    try:
        ...
        for (
            provider_name,
            provider_caps,
        ) in compatibility_service.provider_capabilities.items():
        ...
        return {"providers": capabilities, "total_providers": len(capabilities)}
    ...
    try:
        ...
        try:
            if provider_name == "llama-cpp":
                ...
                error_message = (
                    f"Model loading not implemented for provider: {provider_name}"
                )
