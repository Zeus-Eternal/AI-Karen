...
from ai_karen_engine.services.tooling.tool_service import BaseTool, ToolInput, ToolService
...
async def list_tools(
    ...
    try:
        ...
        return ToolListResponse(
            tools=tools, total_count=len(tools), categories=sorted(list(categories))
        )
...
async def get_tool_info(
    tool_name: str, tool_service: ToolService = Depends(get_tool_service)
):
...
async def get_tool_schema(
    ...
    try:
        ...
        if not tool_instance:
            raise HTTPException(
                status_code=404, detail=f"Tool {tool_name} not available"
            )
...
async def get_tool_categories(tool_service: ToolService = Depends(get_tool_service)):
...
async def get_tool_metrics(tool_service: ToolService = Depends(get_tool_service)):
...
@router.get("/health")
async def health_check(tool_service: ToolService = Depends(get_tool_service)):
...
async def _get_tool_info(
    tool_service: ToolService, tool_name: str
) -> Optional[ToolInfoResponse]:
