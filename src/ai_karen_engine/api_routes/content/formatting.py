"""
API routes for formatting and structure optimization system.
"""
...
# Global formatting engine instances
formatting_engine = ResponseFormattingEngine()
...
class FormatRequest(BaseModel):
    ...
    display_context: str = Field(
        default="desktop",
        description="Display context (desktop, mobile, tablet, terminal, api, print)",
    )
    accessibility_level: str = Field(
        default="basic", description="Accessibility level (basic, enhanced, full)"
    )
    user_preferences: Dict[str, Any] = Field(
        default_factory=dict, description="User formatting preferences"
    )
    technical_level: str = Field(
        default="intermediate", description="User technical level"
    )
...
async def format_content(request: FormatRequest) -> FormatResponse:
    """
    Format content with formatting and structure optimization.

    This endpoint applies intelligent formatting including:
    - Automatic format selection
    - Hierarchical content organization
    - Specialized content-aware cards (Weather, Movie, Recipe, etc.)
    ...
    try:
        ...
        # Try specialized formatting first
        try:
            specialized_response = await specialized_integration.format_response(
                user_query=request.content,  # Use content as query for detection if query not provided
                response_content=request.content,
                user_preferences=request.user_preferences,
            )

            # If we got a specialized format (not just standard markdown), return it
            if specialized_response.format_type != FormatType.STANDARD_MARKDOWN:
                return FormatResponse(
                    formatted_content=specialized_response.content,
                    format_type=specialized_response.format_type.value,
                    sections_count=0,  # Specialized cards don't use standard sections yet
                    navigation_aids_count=0,
                    accessibility_features={},
                    metadata=specialized_response.metadata,
                    estimated_reading_time=None,
                )
        except Exception as e:
            logger.warning(
                f"Specialized formatting failed, falling back to standard: {e}"
            )

        # Fallback to standard hierarchical formatting
        ...
        raise HTTPException(
            status_code=500, detail="Internal server error during formatting"
        )
...
async def analyze_content(request: AnalysisRequest) -> AnalysisResponse:
    ...
    try:
        logger.info(
            f"Analyzing content structure for {len(request.content)} characters"
        )
...
async def get_optimal_format_type(request: FormatRequest) -> Dict[str, Any]:
    ...
    try:
        logger.info(
            f"Determining optimal format type for {len(request.content)} characters"
        )
        ...
        format_type = await formatting_engine.select_optimal_format(
            request.content, context
        )
...
async def organize_content_hierarchically(request: AnalysisRequest) -> Dict[str, Any]:
    ...
    try:
        logger.info(
            f"Organizing content hierarchically for {len(request.content)} characters"
        )

        # Organize content
        sections = await formatting_engine.organize_content_hierarchically(
            request.content
        )
            ...
            "sections": [
                {
                    ...
                    "format_hint": section.format_hint.value
        ...
        raise HTTPException(
            status_code=500, detail="Internal server error during organization"
        )


@router.post("/syntax-highlight")
async def apply_syntax_highlighting(
    code: str, language: str = "text"
) -> Dict[str, Any]:
    """
    ...
    try:
        logger.info(
            f"Applying syntax highlighting for {language} code ({len(code)} characters)"
        )

        # Apply syntax highlighting
        highlighted_code = await formatting_engine.apply_syntax_highlighting(
            code, language
        )
        ...
        raise HTTPException(
            status_code=500, detail="Internal server error during syntax highlighting"
        )
...
async def get_accessibility_features(request: FormatRequest) -> Dict[str, Any]:
    ...
    try:
        logger.info(
            f"Getting accessibility features for {len(request.content)} characters"
        )
        ...
        return {
            ...
            "estimated_reading_time": formatted_response.estimated_reading_time,
            "content_warnings": formatted_response.accessibility_features.get(
                "content_warning"
            ),
            "screen_reader_optimized": "screen_reader_text"
            in formatted_response.accessibility_features,
...
async def health_check() -> Dict[str, str]:
    """Health check endpoint for the formatting service."""
    try:
