...
router = APIRouter(prefix="/api/auth", tags=["production-auth"])
...
async def register(
    ...
    try:
        # Create user
        user = await production_auth_service.create_user(
            email=req.email,
...
async def update_credentials(
    ...
    try:
        ...
        if req.new_password:
            ...
            auth_result = await production_auth_service.authenticate_user(
            ...
            # Update password
            success = await production_auth_service.update_user_password(
                user_id=user_data["user_id"],
        ...
        if req.preferences:
            success = await production_auth_service.update_user_preferences(
                user_id=user_data["user_id"],
                preferences=req.preferences
            )
        ...
        updated_user = await production_auth_service.validate_session(
...
async def logout(request: Request, response: Response) -> Dict[str, str]:
    ...
    if session_token:
        # Invalidate session
        await production_auth_service.invalidate_session(session_token)
...
async def request_password_reset(
    ...
    try:
        token = await production_auth_service.create_password_reset_token(
            email=req.email,
            ip_address=request.client.host if request.client else "unknown",
            user_agent=request.headers.get("user-agent", "")
        )
