...
class QueryMemoryRequest(BaseModel):
    ...
    session_id: Optional[str] = Field(None, description="Session ID")
    conversation_id: Optional[str] = Field(None, description="Conversation ID")
    ui_source: Optional[UISource] = Field(
        None, description="Filter by UI source (web, streamlit, desktop, api, ag_ui)"
    )
    memory_types: List[MemoryType] = Field(
        default_factory=list, description="Filter by memory types"
    )
    tags: List[str] = Field(default_factory=list, description="Filter by tags")
    importance_range: Optional[Tuple[int, int]] = Field(
        None, description="Importance score range"
    )
    ...
    top_k: int = Field(
        10, ge=1, le=100, description="Maximum results from the vector search"
    )
    result_limit: Optional[int] = Field(
        None, ge=1, description="Override default result cap"
    )
...
async def delete_memory(
    ...
    try:
        success = await memory_service.base_manager.delete_memory(
            tenant_id=current_user["tenant_id"], memory_id=memory_id
        )
...
async def get_memory_stats(
    ...
    try:
        stats = await memory_service.base_manager.get_memory_stats(
...
async def prune_expired_memories(
    ...
    try:
        pruned_count = await memory_service.base_manager.prune_expired_memories(
            current_user["tenant_id"]
        )
