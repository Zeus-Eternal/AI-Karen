...
class ValidationUtils:
    ...
    @staticmethod
    def validate_text_content(text: str, min_length: int = 1, max_length: int = 16000) -> str:
        """Validate text content"""
        if not text or not text.strip():
            raise ValueError(f"Text content cannot be empty")
...
class ErrorHandler:
    ...
    def create_service_error_response(
        ...
        status_code = 503 if not FASTAPI_AVAILABLE else status.HTTP_503_SERVICE_UNAVAILABLE
    ...
    def create_internal_error_response(
        ...
        status_code = 500 if not FASTAPI_AVAILABLE else status.HTTP_500_INTERNAL_SERVER_ERROR
