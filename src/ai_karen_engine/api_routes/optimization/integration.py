...
from ai_karen_engine.services.optimization.integration_orchestrator import (
...
async def initialize_integration(
    background_tasks: BackgroundTasks, orchestrator=Depends(get_orchestrator)
):
...
async def update_configuration(
    request: ConfigurationUpdateRequest, config_manager=Depends(get_config_manager)
):
    """Update optimization configuration."""
    try:
        success = config_manager.update_configuration(
            request.updates, validate=request.validate
        )
...
async def reset_configuration(
    component: Optional[str] = None, config_manager=Depends(get_config_manager)
):
    ...
    try:
        ...
        if success:
            ...
            return {"message": "Configuration reset failed", "status": "failed"}
    except Exception as e:
...
async def refresh_model_discovery(
    ...
    try:
        ...
        return {"message": "Model discovery refresh started", "status": "refreshing"}
    except Exception as e:
...
async def get_integrated_models(orchestrator=Depends(get_orchestrator)):
    ...
    try:
        ...
        models = await orchestrator.model_manager.get_models_for_profile(
            active_profile.name
        )
            ...
            formatted_models.append(
                {
                    ...
                    "requirements": {
                        "memory_mb": model_info.requirements.memory_mb
                        if model_info.requirements
                        else 0,
                        "gpu_required": model_info.requirements.gpu_required
                        if model_info.requirements
                        else False,
                        "min_context_length": model_info.requirements.min_context_length
                        if model_info.requirements
                        else 0,
                    },
...
async def toggle_model_routing(
    model_id: str, enable: bool, orchestrator=Depends(get_orchestrator)
):
...
async def optimize_cache_performance(
    ...
    try:
        ...
        return {"message": "Cache optimization started", "status": "optimizing"}
    except Exception as e:
...
async def get_performance_dashboard(orchestrator=Depends(get_orchestrator)):
    ...
    try:
        dashboard_data = (
            orchestrator.performance_monitor.get_performance_dashboard_data()
        )
...
async def resolve_performance_alert(
    alert_id: str, orchestrator=Depends(get_orchestrator)
):
...
async def shutdown_integration(
    ...
    try:
        ...
        return {"message": "Integration shutdown initiated", "status": "shutting_down"}
    except Exception as e:
