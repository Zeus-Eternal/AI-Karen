...
class FormattingRequest:
    def __init__(
        ...
        display_context: str = "desktop",
        theme_mode: str = "auto",
        accessibility_level: str = "basic",
...
async def format_response(request: FormattingRequest, background_tasks=None, user=None):
    ...
    """
    try:
        user = user or bypass_user_context_func()
...
async def format_streaming_chunk(
    request: StreamingFormattingRequest, background_tasks=None, user=None
):
    """
...
async def detect_content_type(
    request: ContentTypeDetectionRequest, background_tasks=None, user=None
):
    """
    ...
    try:
        user = user or bypass_user_context_func()
        logger.info(
            f"Content type detection request from user {user.get('user_id', 'unknown')}"
        )
        ...
        logger.info(
            f"Content type detected: {content_type} with confidence {confidence:.2f}"
        )
        return response
        ...
        raise Exception(
            f"Internal server error during content type detection: {str(e)}"
        )


@router.post("/user-profile")
async def save_user_profile(
    request: UserProfileRequest, background_tasks=None, user=None
):
    """
...
async def get_user_profile(user_id: str, background_tasks=None, current_user=None):
    ...
    """
    try:
        current_user = current_user or bypass_user_context_func()
    ...
    try:
        ...
        profiles = {
            "output_profiles": [
                {
                    "name": "plain",
                    "description": "Minimal formatting with no special styling",
                },
                {
                    "name": "pretty",
                    "description": "Enhanced formatting with markdown support",
                },
                {
                    "name": "dev_doc",
                    "description": "Developer documentation formatting",
                },
                {"name": "minimal", "description": "Ultra-minimal formatting"},
                {"name": "verbose", "description": "Detailed formatting with metadata"},
                {
                    "name": "accessible",
                    "description": "Accessibility-focused formatting",
                },
                {"name": "technical", "description": "Technical formatting for code"},
                {
                    "name": "conversational",
                    "description": "Friendly conversational formatting",
                },
            ],
            "layout_types": [
            ...
            "accessibility_levels": [
                ...
                {
                    "name": "full",
                    "description": "Full accessibility with screen reader",
                },
            ],
        }
