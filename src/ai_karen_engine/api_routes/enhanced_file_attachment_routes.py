...
class EnhancedFileUploadMetadata(BaseModel):
    ...
    ui_context: Optional[Dict[str, Any]] = Field(None, description="UI context for AG-UI integration")


class EnhancedFileListResponse(BaseModel):
    """Enhanced response for file listing with AG-UI grid data."""
    files: List[Dict[str, Any]] = Field(..., description="List of files with enhanced metadata")
    total_count: int = Field(..., description="Total number of files")
    has_more: bool = Field(..., description="Whether there are more files")
    grid_metadata: Dict[str, Any] = Field(default_factory=dict, description="AG-Grid metadata")
    statistics: Dict[str, Any] = Field(default_factory=dict, description="File statistics")
...
class FileAnalysisResponse(BaseModel):
    ...
    hook_results: Dict[str, Any] = Field(default_factory=dict, description="Hook execution results")
    multimedia_analysis: Optional[Dict[str, Any]] = Field(None, description="Multimedia analysis results")
    security_analysis: Optional[Dict[str, Any]] = Field(None, description="Security analysis results")
    plugin_results: Dict[str, Any] = Field(default_factory=dict, description="Plugin processing results")
    extension_results: Dict[str, Any] = Field(default_factory=dict, description="Extension processing results")
    ...
    try:
        ...
        enhanced_result["ui_metadata"] = {
            "grid_row_data": {
                ...
                "extracted_content_available": result.metadata.extracted_content is not None,
                "tags": file_metadata.tags,
    ...
    try:
        ...
        for file_id, metadata in file_service._file_metadata.items():
            ...
            if processing_status and metadata.processing_status != processing_status:
                continue
            if security_status and (not metadata.security_scan_result or metadata.security_scan_result.value != security_status):
                continue
            ...
            file_info = {
                ...
                "security_scan_result": metadata.security_scan_result.value if metadata.security_scan_result else None,
    ...
    try:
        ...
        return {
            ...
            "chart_data": {
                ...
                "file_types": [
                    {"type": k, "count": v} for k, v in storage_stats.get("files_by_type", {}).items()
                ]
            }
        }
...
def _calculate_upload_trends(files: List[Any]) -> List[Dict[str, Any]]:
    ...
    for i in range(30):
        ...
        trends.append({
            "date": date.isoformat(),
            "uploads": daily_uploads.get(date, 0)
        })
...
def _calculate_type_analysis(files: List[Any]) -> Dict[str, Any]:
    ...
    for file_metadata in files:
        ...
        if file_metadata.security_scan_result and file_metadata.security_scan_result.value in ["suspicious", "malicious"]:
    ...
    for file_type, stats in type_stats.items():
        if stats["count"] > 0:
            ...
            stats["processing_success_rate"] = (stats["processing_success_rate"] / stats["count"]) * 100
