...
async def get_error_summary(
    ...
    try:
        ...
        recent_errors = [
            error
            for error in error_service.error_occurrences
        ...
        affected_users = len(
            set(error.user_id for error in recent_errors if error.user_id)
        )
...
async def get_error_details(
    service: str, error_type: str, current_user: dict = Depends(get_current_user)
) -> Dict[str, Any]:
    """
...
async def search_errors(
    request: ErrorSearchRequest, current_user: dict = Depends(get_current_user)
) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        if request.pattern:
            try:
                ...
                raise HTTPException(
                    status_code=400, detail=f"Invalid pattern: {request.pattern}"
                )
...
async def get_error_trends(
    ...
    try:
        if period not in ["hourly", "daily"]:
            raise HTTPException(
                status_code=400, detail="Period must be 'hourly' or 'daily'"
            )
...
async def get_error_patterns(
    ...
    try:
        ...
        return {
            "patterns": {
                pattern.value: {**info, "pattern_id": pattern.value}
    ...
    try:
        ...
        critical_errors = len(
            [
                ...
                if error_service._determine_error_severity(
                    error_service._detect_error_pattern(
        ...
        return {
            ...
            "storage_usage": {
                ...
                "usage_percent": (
                    len(error_service.error_occurrences) / error_service.max_errors
                )
                * 100,
            },
        }
