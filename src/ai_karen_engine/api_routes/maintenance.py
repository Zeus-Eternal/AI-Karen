...
class CleanupRequest(BaseModel):
    ...
    dry_run: bool = Field(True, description="If True, only report changes without applying them")
    categories: Optional[List[str]] = Field(None, description="Specific categories to clean (files, users, cache, backups)")

class CleanupRecommendationResponse(BaseModel):
    """Response model for cleanup recommendations."""
    recommendations: List[str] = Field(..., description="List of cleanup recommendations")
...
async def run_system_cleanup(
    dry_run: bool = Query(True, description="If true, only simulate cleanup without actually deleting files"),
    user_ctx: Dict[str, Any] = Depends(get_current_user_context)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Maintenance cleanup failed: {str(exc)}"
        ) from exc
