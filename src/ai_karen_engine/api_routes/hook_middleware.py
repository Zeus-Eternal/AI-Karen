...
class HookMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> StarletteResponse:
        ...
        try:
            ...
            await self._trigger_post_response_hooks(request_info, response_info, pre_hook_results)
            ...
            # Trigger error hooks
            await self._trigger_error_hooks(request_info, str(e), processing_time, pre_hook_results)
    ...
    def _extract_response_info(self, response: StarletteResponse, processing_time: float) -> Dict[str, Any]:
    ...
    async def _trigger_pre_request_hooks(self, request_info: Dict[str, Any]) -> Dict[str, Any]:
        """Trigger pre-request hooks."""
        try:
            ...
            logger.debug(f"Pre-request hooks executed: {summary.successful_hooks}/{summary.total_hooks} for {request_info['path']}")
            ...
            return {"error": "timeout", "hook_type": hook_type if 'hook_type' in locals() else None}
        except Exception as e:
    ...
    async def _trigger_post_response_hooks(
        ...
        try:
            ...
            logger.debug(f"Post-response hooks executed: {summary.successful_hooks}/{summary.total_hooks} for {request_info['path']}")
    ...
    async def _trigger_error_hooks(
        ...
        try:
            ...
            logger.debug(f"Error hooks executed: {summary.successful_hooks}/{summary.total_hooks} for {request_info['path']}")
    ...
    def _determine_hook_type(self, path: str, phase: str) -> Optional[str]:
        ...
        path_mappings = {
            "/api/chat": HookTypes.PRE_MESSAGE if phase == "pre" else HookTypes.POST_MESSAGE,
            "/api/ws/chat": HookTypes.PRE_MESSAGE if phase == "pre" else HookTypes.POST_MESSAGE,
            "/api/ws/stream": HookTypes.PRE_MESSAGE if phase == "pre" else HookTypes.POST_MESSAGE,
            "/api/plugins": HookTypes.PLUGIN_EXECUTION_START if phase == "pre" else HookTypes.PLUGIN_EXECUTION_END,
            "/api/extensions": HookTypes.EXTENSION_ACTIVATED if phase == "pre" else HookTypes.EXTENSION_DEACTIVATED,
            "/api/memory": HookTypes.MEMORY_RETRIEVE if phase == "pre" else HookTypes.MEMORY_STORE,
            "/api/llm": HookTypes.LLM_REQUEST if phase == "pre" else HookTypes.LLM_RESPONSE,
            "/api/auth": HookTypes.USER_LOGIN if phase == "pre" else HookTypes.USER_LOGIN,
            "/api/files": HookTypes.FILE_UPLOADED if phase == "pre" else HookTypes.FILE_PROCESSED
        }
