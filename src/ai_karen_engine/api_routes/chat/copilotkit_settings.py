...
_copilotkit_status: CopilotKitStatus = CopilotKitStatus(
    status="unknown", message="Status not checked yet"
)
    ...
    try:
        ...
        if _copilotkit_config.enabled:
            ...
            _copilotkit_status = CopilotKitStatus(
                ...
                features_available=[
                    feature
                    for feature, enabled in _copilotkit_config.features.dict().items()
                    if enabled
                ],
...
async def test_copilotkit_connection(
    ...
    try:
        ...
        return ConnectionTestResponse(
            success=False, message="Connection test failed", error_details=str(e)
        )
...
async def update_copilotkit_features(
    features: CopilotKitFeatures,
) -> CopilotKitFeatures:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update features: {str(e)}"
        )
...
async def toggle_copilotkit_feature(feature_name: str, enabled: bool) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"CopilotKit feature '{feature_name}' {'enabled' if enabled else 'disabled'}"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to toggle feature: {str(e)}"
        )
        ...
        return {
            ...
            "configuration": {
                ...
                "features_enabled": sum(
                    1
                    for enabled in _copilotkit_config.features.dict().values()
                    if enabled
                ),
