...
def load_environment_specific_validation_config(settings: Settings) -> Settings:
    ...
    # Development environment - relaxed validation for debugging
        ...
        settings.enable_request_validation = True
        settings.enable_security_analysis = getattr(
            settings, "enable_security_analysis", True
        )
        settings.log_invalid_requests = True
        settings.validation_rate_limit_per_minute = 200  # More lenient for testing
        settings.max_invalid_requests_per_connection = (
            20  # Higher tolerance for debugging
        )
    ...
    # Staging environment - production-like but with more logging
        ...
        logger.info(
            "🎭 Staging validation config: production-like with enhanced logging"
        )
...
def validate_configuration_settings(settings: Settings) -> bool:
    ...
    if settings.validation_rate_limit_per_minute <= 0:
        ...
        issues.append(
            "validation_rate_limit_per_minute too high (>10000), potential resource exhaustion"
        )
...
def initialize_validation_framework(settings: Settings) -> None:
    ...
    try:
        ...
        # Import validation components (these should exist in the canonical server)
        try:
            ...
            blocked_agents = set(
                agent.strip().lower()
                for agent in settings.blocked_user_agents.split(",")
                if agent.strip()
            )
            suspicious_headers = set(
                header.strip().lower()
                for header in settings.suspicious_headers.split(",")
                if header.strip()
            )
            ...
            logger.info(
                f"   • Max request size: {settings.max_request_size / (1024 * 1024):.1f}MB"
            )
            logger.info(f"   • Max headers: {settings.max_headers_count}")
            logger.info(
                f"   • Security analysis: {'enabled' if settings.enable_security_analysis else 'disabled'}"
            )
            logger.info(
                f"   • Rate limiting: {settings.validation_rate_limit_per_minute} requests/minute"
            )
            ...
            logger.info(
                f"   • Protocol error handling: {'enabled' if settings.enable_protocol_error_handling else 'disabled'}"
            )

        except ImportError as e:
            logger.warning(f"⚠️ Some validation components not available: {e}")
