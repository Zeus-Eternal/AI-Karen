...
class ValidationConfig:
    ...
    blocked_user_agents: Set[str] = field(
        default_factory=lambda: {"sqlmap", "nikto", "nmap", "masscan", "zap"}
    )
    suspicious_headers: Set[str] = field(
        default_factory=lambda: {"x-forwarded-host", "x-cluster-client-ip", "x-real-ip"}
    )
...
class HTTPRequestValidator:
    ...
    async def validate_request(self, request: Request) -> ValidationResult:
        ...
        try:
            ...
            validation_details["content_length_valid"] = content_length_result[
                "is_valid"
            ]
    ...
    async def validate_headers(self, request: Request) -> Dict[str, Any]:
        ...
        try:
            ...
            blocked_agent = any(
                blocked in user_agent for blocked in self.config.blocked_user_agents
            )
            ...
            return {
                ...
                "error_message": "Blocked user agent detected"
                if blocked_agent
                else None,
    ...
    async def check_content_length(self, request: Request) -> Dict[str, Any]:
        ...
        try:
            ...
            if length < 0:
                return {"is_valid": False, "error_message": "Negative content-length"}
    ...
    def _record_validation_metrics(
        self, request: Request, result: ValidationResult, processing_time_ms: float
    ):
        ...
        try:
            ...
            if request and hasattr(request, "client") and request.client:
                ...
                client_ip = client_host if client_host is not None else "unknown"
            client_ip_hash = (
                self._hash_ip(client_ip) if client_ip != "unknown" else "unknown"
            )
            ...
            security_analysis = (
                security_analysis_raw if isinstance(security_analysis_raw, dict) else {}
            )
