...
class ProfileManager:
    ...
    def __init__(self, config_path: Optional[Path] = None, settings_manager=None):
        ...
        """
        self.config_path = config_path or Path("config_assets/llm_profiles.yml")
    ...
    def _load_profiles(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(self.profiles_config.profiles)} profiles from {self.config_path}"
            )
    ...
    def _parse_profile(self, profile_data: Dict[str, Any]) -> LLMProfile:
        ...
        if cloud_gating_data:
            cloud_gating = CloudGating(
                ...
                requires_policy_approval=cloud_gating_data.get(
        ...
        routing = RoutingConfig(
            strategy=RoutingStrategy(
                routing_data.get("strategy", "local_first_optional_cloud")
            ),
            allow_cloud_fallback=routing_data.get("allow_cloud_fallback", False),
            enforce_local_for_sensitive=routing_data.get(
                "enforce_local_for_sensitive", True
            ),
    ...
    def set_active_profile(
        self, profile_id: str, user_id: Optional[str] = None
    ) -> LLMProfile:
        """
        ...
        if self.active_profile_id:
            current_profile = self.profiles_config.get_profile_by_id(
