...
class DeploymentConfigManager:
    ...
    def __init__(
        ...
        """
        self.config_path = (
            Path(config_path) if config_path else Path("config_assets/services.yml")
        )
        ...
        self._service_listeners: List[
            Callable[[str, str], None]
        ] = []  # service_name, action
        self._hot_reload_task: Optional[asyncio.Task] = None
        self._executor = ThreadPoolExecutor(
            max_workers=2, thread_name_prefix="config-mgr"
        )
        ...
        logger.info(
            f"Deployment configuration manager initialized with config: {self.config_path}"
        )
        ...
        with self._config_lock:
            try:
                ...
                for service_name, service_data in services_data.items():
                    service_config = self._parse_service_config(
                ...
                try:
                    ...
                    logger.warning(
                        f"Invalid deployment mode: {mode_name}, using development"
                    )
                    self._current_mode = DeploymentMode.DEVELOPMENT

                # Set current profile
                self._current_profile = self._deployment_profiles.get(
                ...
                logger.info(
                    f"Configuration loaded: {len(self._services)} services, "
    ...
    async def get_services_for_current_mode(self) -> Dict[str, ServiceConfig]:
        ...
        for name, service in self._services.items():
            if service.classification in enabled_classifications and service.enabled:
        ...
        with self._config_lock:
            ...
            if service.enabled:
                ...
                if (
                    ...
                    and service.classification == ServiceClassification.ESSENTIAL
                    and ServiceClassification.ESSENTIAL
                    in self._current_profile.enabled_classifications
                ):
                    logger.warning(
                        f"Cannot stop essential service in current mode: {service_name}"
                    )
                    return False
                    ...
                    if key == "resource_requirements" and isinstance(value, dict):
                        ...
                        for res_key, res_value in value.items():
                            if hasattr(service.resource_requirements, res_key):
                                setattr(
                                    service.resource_requirements, res_key, res_value
                                )
                    else:
    ...
    async def get_resource_allocation(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "utilization": {
                "memory_percent": (
                    total_memory / self._current_profile.max_memory_mb * 100
                )
                if self._current_profile
                else 0,
                "cpu_percent": (total_cpu / self._current_profile.max_cpu_cores * 100)
                if self._current_profile
                else 0,
                "service_percent": (
                    service_count / self._current_profile.max_services * 100
                )
                if self._current_profile
                else 0,
            },
    ...
    async def save_configuration(self) -> None:
        ...
        with self._config_lock:
            try:
                config_data = {"services": {}, "deployment_profiles": {}}
                ...
                for name, profile in self._deployment_profiles.items():
                    config_data["deployment_profiles"][name] = {
                        "enabled_classifications": [
                            cls.value for cls in profile.enabled_classifications
                        ],
    ...
    def _parse_service_config(self, name: str, data: Dict[str, Any]) -> ServiceConfig:
        ...
        return ServiceConfig(
            name=name,
            classification=ServiceClassification(
                data.get("classification", "optional")
            ),
    ...
    def _parse_deployment_profile(
        self, name: str, data: Dict[str, Any]
    ) -> DeploymentProfile:
        """Parse deployment profile from data"""
        enabled_classifications = [
            ServiceClassification(cls)
            for cls in data.get("enabled_classifications", ["essential"])
        ]
    ...
    async def _validate_service_config(self, service: ServiceConfig) -> None:
        ...
        if service.resource_requirements.memory_mb <= 0:
            raise ConfigValidationError(
                f"Service {service.name}: memory_mb must be positive"
            )

        if service.resource_requirements.cpu_cores <= 0:
            raise ConfigValidationError(
                f"Service {service.name}: cpu_cores must be positive"
            )

        # Check timeouts
        if service.idle_timeout is not None and service.idle_timeout <= 0:
            raise ConfigValidationError(
                f"Service {service.name}: idle_timeout must be positive"
            )

        if service.health_check_interval <= 0:
            raise ConfigValidationError(
                f"Service {service.name}: health_check_interval must be positive"
            )

        if service.graceful_shutdown_timeout <= 0:
            raise ConfigValidationError(
                f"Service {service.name}: graceful_shutdown_timeout must be positive"
            )
    ...
    async def _validate_deployment_profile(self, profile: DeploymentProfile) -> None:
        ...
        if profile.max_memory_mb <= 0:
            raise ConfigValidationError(
                f"Profile {profile.name}: max_memory_mb must be positive"
            )

        if profile.max_services <= 0:
            raise ConfigValidationError(
                f"Profile {profile.name}: max_services must be positive"
            )

        if profile.max_cpu_cores <= 0:
            raise ConfigValidationError(
                f"Profile {profile.name}: max_cpu_cores must be positive"
            )
    ...
    async def _validate_service_dependencies(self) -> None:
        ...
        def has_circular_dependency(
            service_name: str, visited: Set[str], path: Set[str]
        ) -> bool:
