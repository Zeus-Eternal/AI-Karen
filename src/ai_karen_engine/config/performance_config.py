...
class PerformanceConfigManager:
    ...
    def __init__(self, config_file: Optional[str] = None):
        self.config_file = config_file or "config_assets/performance.yml"
    ...
    async def load_config(self) -> PerformanceConfig:
        ...
        try:
                ...
                try:
                    ...
                    if file_config:
                        ...
                        for field in config.__dataclass_fields__:
                            if getattr(config, field) == getattr(
                                PerformanceConfig(), field
                            ):
                                # Use file value if environment value is default
                                setattr(
                                    config, field, getattr(file_based_config, field)
                                )
            ...
            if not config.validate():
                logger.warning(
                    "Performance configuration validation failed, using defaults"
                )
            ...
            logger.info(
                f"  • Optimization enabled: {config.enable_performance_optimization}"
            )
            logger.info(f"  • Lazy loading: {config.enable_lazy_loading}")
            logger.info(f"  • GPU offloading: {config.enable_gpu_offloading}")
            logger.info(
                f"  • Service consolidation: {config.enable_service_consolidation}"
            )
