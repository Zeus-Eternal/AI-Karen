...
from .migration_validator import MigrationValidator, MigrationValidationReport
from .postgres_schema import PostgreSQLAuthSchema, User, UserSession, PasswordResetToken
...
class DatabaseConsolidationMigrator:
    ...
    def __init__(self, sqlite_paths: List[str], postgres_url: str, backup_dir: str = "migration_backups"):
        """
    ...
    async def migrate_all_data(self, create_backups: bool = True, validate_after: bool = True) -> MigrationResult:
        """
        ...
        try:
            ...
            if create_backups:
                ...
                backup_mapping = self.backup_manager.backup_sqlite_databases(self.sqlite_paths)
            ...
            if validate_after:
                ...
                if not validation_report.overall_success:
                    raise Exception(f"Migration validation failed: {validation_report.validation_timestamp}")
    ...
    async def _migrate_single_user(self, user_data: Dict) -> Optional[User]:
        ...
        try:
            with self.SessionLocal() as session:
                ...
                existing_user = session.query(User).filter(User.email == user_data.get('email')).first()
                if existing_user:
                    self.logger.warning(f"User {user_data.get('email')} already exists, skipping")
            ...
            try:
                ...
                for table_name in session_tables:
                    ...
                    for session_row in sqlite_sessions:
                        ...
                        migrated_session = await self._migrate_single_session(session_data)
    ...
    async def _migrate_single_session(self, session_data: Dict) -> Optional[UserSession]:
        """
        ...
        try:
            with self.SessionLocal() as session:
                ...
                if existing_session:
                    self.logger.warning(f"Session {session_data.get('session_token')} already exists, skipping")
                ...
                if not new_user_id:
                    self.logger.warning(f"No UUID mapping found for user_id {old_user_id}, skipping session")
                    return None
                ...
                new_session = UserSession(
                    ...
                    expires_at=self._parse_datetime(session_data.get('expires_at')) or self._parse_datetime(session_data.get('expires_in')),
                    created_at=self._parse_datetime(session_data.get('created_at')),
                    last_accessed=self._parse_datetime(session_data.get('last_accessed')),
                    ...
                    invalidated_at=self._parse_datetime(session_data.get('invalidated_at')),
            ...
            self.logger.error(f"Failed to migrate session {session_data.get('session_token')}: {e}")
            return None
    ...
    async def _migrate_single_token(self, token_data: Dict) -> Optional[PasswordResetToken]:
        """
        ...
        try:
            with self.SessionLocal() as session:
                ...
                existing_token = session.query(PasswordResetToken).filter(
                    PasswordResetToken.token == token_data.get('token')
                ).first()
                if existing_token:
                    self.logger.warning(f"Token {token_data.get('token')} already exists, skipping")
                ...
                if not new_user_id:
                    self.logger.warning(f"No UUID mapping found for user_id {old_user_id}, skipping token")
                    return None
