...
'''
    ...
    def setup_tenant_database(
        self, tenant_id: str, tenant_name: str, tenant_slug: str
    ) -> bool:
            ...
            if not success:
                ...
                with self.client.get_sync_session() as session:
                    tenant = (
                        ...
                        .filter(Tenant.id == uuid.UUID(tenant_id))
                        .first()
                    )
                    if tenant:
                ...
                for table in expected_tables:
                    ...
                    if not exists:
                        results["errors"].append(
                            f"Table {table} does not exist in schema {schema_name}"
                        )
    ...
    def get_database_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            status["alembic_initialized"] = os.path.exists(
                os.path.join(self.migrations_dir, "alembic.ini")
            )
