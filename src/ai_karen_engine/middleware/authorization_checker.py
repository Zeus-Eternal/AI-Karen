...
class AuthorizationChecker:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        logger.info(
            f"Authorization Checker initialized with {len(self._roles)} roles, {len(self._resources)} resources, and {len(self._access_rules)} access rules"
        )
    ...
    def _load_default_access_rules(self) -> None:
        ...
        default_access_rules = [
            ...
            AccessRule(
                rule_id="copilot_access",
                name="Copilot Access",
                description="Allow all users to access copilot routes",
                roles=["guest", "user", "admin", "agent", "security_officer"],
                resources=["copilot_api"],
                permissions=[Permission.READ, Permission.WRITE],
            ),
            AccessRule(
                rule_id="security_access",
        ...
        for resource_name, resource_data in custom_resources.items():
            try:
                resource = Resource(
                    ...
                    required_permissions=[
                        Permission(p)
                        for p in resource_data.get("required_permissions", [])
                    ],
        ...
        for rule_id, rule_data in custom_rules.items():
            try:
                rule = AccessRule(
                    ...
                    permissions=[
                        Permission(p) for p in rule_data.get("permissions", [])
                    ],
    ...
    async def check_authorization(
        ...
        try:
            ...
            is_authorized = all(
                perm in granted_permissions for perm in resource_permissions
            )
    ...
    def check_permission(
        self, user_roles: List[str], resource_type: ResourceType, permission: Permission
    ) -> bool:
        """
    ...
    def get_user_permissions(
        self, user_roles: List[str], resource_type: Optional[ResourceType] = None
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "admin_roles": [
                role.name for role in self._roles.values() if role.is_admin
            ],
            "roles_by_priority": {
                priority: len(roles) for priority, roles in roles_by_priority.items()
            },
            "resources_by_type": {
            ...
            "rules_by_role": {
                role: len(rules) for role, rules in rules_by_role.items()
            },
        }
