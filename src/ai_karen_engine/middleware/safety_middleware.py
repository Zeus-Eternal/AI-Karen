...
class SafetyMiddleware(BaseHTTPMiddleware):
    ...
    async def _check_content_safety(
        ...
        if not self.config.enable_content_safety:
            ...
            return ValidationResult(
                is_safe=True, confidence=1.0, risk_level=RiskLevel.SAFE
            )
        ...
        try:
            ...
            result = await agent_safety.check_content_safety(
                content=content_str, agent_id=agent_id or "unknown"
            )
            ...
            if self.config.strict_mode:
                ...
                return ValidationResult(
                    ...
                    violations=[
                        "Content safety check failed - allowed due to non-strict mode"
                    ],
                )

    async def _check_authorization(
        self, request: Request, resource: str, action: str
    ) -> bool:
    ...
    async def _determine_safety_action(
    ...
    async def _create_safety_response(
        ...
        if action == SafetyAction.BLOCK:
            ...
            return JSONResponse(
                status_code=status.HTTP_403_FORBIDDEN, content=response_data
            )
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            safety_result = await self._check_content_safety(
                content=request_content, user_id=user_id
            )
            ...
            safety_response = await self._create_safety_response(
                action=safety_action, validation_result=safety_result, request=request
            )
