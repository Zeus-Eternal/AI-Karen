...
from ..core.langgraph_orchestrator import LangGraphOrchestrator, CanonicalChatRequest
...
class ResponseCoreIntegrationMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        self, app, enable_response_core: bool = True, fallback_enabled: bool = True
    ):
        super().__init__(app)
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if (
            hasattr(request.state, "use_response_core")
            and request.state.use_response_core
        ):
...
class ResponseCoreCompatibilityLayer:
    ...
    def __init__(
        self, enable_response_core: bool = True, fallback_enabled: bool = True
    ):
        self.enable_response_core = enable_response_core
        self.fallback_enabled = fallback_enabled
    ...
    async def process_chat_request(
        ...
        **kwargs,
        ...
        try:
            if self.enable_response_core and (
                use_response_core
                or self._should_prefer_response_core(message, **kwargs)
            ):
    ...
    def _get_response_orchestrator(self, user_id: str):
        ...
        if user_id not in self._orchestrator_cache:
            try:
                self._orchestrator_cache[user_id] = get_global_orchestrator(
                    user_id=user_id
                )
            except Exception as e:
                logger.warning(
                    f"Failed to create global orchestrator, using local-only: {e}"
                )
                self._orchestrator_cache[user_id] = create_local_only_orchestrator(
                    user_id=user_id
                )
    ...
    def _get_chat_orchestrator(self) -> ChatOrchestrator:
        ...
        try:
            ...
            try:
                ...
                Exception
            ) as embed_err:  # pragma: no cover - best effort initialisation
            ...
            logger.warning(
                f"Failed to initialise memory manager for ChatOrchestrator: {db_err}"
            )
            memory_manager = None
    ...
    def _should_prefer_response_core(self, message: str, **kwargs) -> bool:
        ...
        if any(
            ...
            for keyword in ["analyze", "explain", "help", "guide"]
        ):
            return True

        # Check for persona-related requests
        if any(
            keyword in message.lower() for keyword in ["persona", "character", "role"]
        ):
            return True
...
def configure_response_core_integration(
    enable_response_core: bool = True, fallback_enabled: bool = True
) -> ResponseCoreCompatibilityLayer:
    """Configure Response Core integration"""
    global _compatibility_layer
    _compatibility_layer = ResponseCoreCompatibilityLayer(
        enable_response_core=enable_response_core, fallback_enabled=fallback_enabled
    )
    return _compatibility_layer
