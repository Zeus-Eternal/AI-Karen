...
class MemoryBudget:
    ...
    priority_memory_types: List[str] = field(
        default_factory=lambda: ["user_preferences", "important_facts"]
    )
...
class LLMProfile:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        for provider_pref in data["providers"].values():
            if "required_capabilities" in provider_pref:
                provider_pref["required_capabilities"] = list(
                    provider_pref["required_capabilities"]
                )
            if "excluded_capabilities" in provider_pref:
                provider_pref["excluded_capabilities"] = list(
                    provider_pref["excluded_capabilities"]
                )
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "LLMProfile":
        ...
        if "providers" in data:
            for provider_pref in data["providers"].values():
                if "required_capabilities" in provider_pref:
                    provider_pref["required_capabilities"] = set(
                        provider_pref["required_capabilities"]
                    )
                if "excluded_capabilities" in provider_pref:
                    provider_pref["excluded_capabilities"] = set(
                        provider_pref["excluded_capabilities"]
                    )
...
class LLMProfileManager:
    ...
    def switch_profile(self, profile_id: str) -> LLMProfile:
        ...
        if not validation_result["compatible"]:
            logger.warning(
                f"Profile {profile_id} has compatibility issues: {validation_result['issues']}"
            )
        ...
        try:
            ...
            for use_case, provider_pref in profile.providers.items():
                if provider_pref.provider not in available_providers:
                    issues.append(
                        f"Provider '{provider_pref.provider}' for {use_case} is not available"
                    )
                else:
                    ...
                    if health.get("status") == "unhealthy":
                        warnings.append(
                            f"Provider '{provider_pref.provider}' for {use_case} is currently unhealthy"
                        )
                    elif health.get("status") == "unavailable":
                        issues.append(
                            f"Provider '{provider_pref.provider}' for {use_case} is unavailable: {health.get('message', 'Unknown reason')}"
                        )
                    ...
                    if provider_info:
                        provider_capabilities = set(
                            provider_info.get("capabilities", [])
                        )

                        # Check required capabilities
                        missing_capabilities = (
                            provider_pref.required_capabilities - provider_capabilities
                        )
                        ...
                        conflicting_capabilities = (
                            provider_pref.excluded_capabilities & provider_capabilities
                        )
                        ...
                        if provider_pref.model:
                            try:
                                models = asyncio.run(
                                    self.provider_manager.discover_models(
                                        provider_pref.provider
                                    )
                                )
                                ...
                                if provider_pref.model not in model_ids:
                                    warnings.append(
                                        f"Model '{provider_pref.model}' not found in {provider_pref.provider}"
                                    )
            ...
            if profile.fallback_provider not in available_providers:
                issues.append(
                    f"Fallback provider '{profile.fallback_provider}' is not available"
                )
            else:
                ...
                if health.get("status") == "unhealthy":
                    warnings.append(
                        f"Fallback provider '{profile.fallback_provider}' is currently unhealthy"
                    )

            # Check memory budget constraints
            if profile.memory_budget.max_context_length > 128000:
                warnings.append(
                    "Very large context length may cause performance issues"
                )

            if profile.memory_budget.max_conversation_history > 200:
                warnings.append(
                    "Very large conversation history may impact performance"
                )
            ...
            if profile.router_policy == RouterPolicy.PRIVACY:
                ...
                if non_local_providers:
                    warnings.append(
                        f"Privacy policy with non-local providers: {non_local_providers}"
                    )
            ...
            if profile.guardrails.level == GuardrailLevel.CUSTOM:
                if not profile.guardrails.custom_rules:
                    warnings.append(
                        "Custom guardrail level specified but no custom rules defined"
                    )
    ...
    def _validate_profile(self, profile: LLMProfile) -> None:
        ...
        if not profile.is_valid:
            logger.warning(
                f"Profile {profile.id} has validation errors: {profile.validation_errors}"
            )
    ...
    def _create_default_profiles(self) -> None:
        ...
        performance_profile = self.create_profile(
            ...
            providers={
                "chat": ProviderPreference(
                    provider="local", priority=90, required_capabilities={"streaming"}
                ),
                ...
                "reasoning": ProviderPreference(provider="local", priority=85),
                "embedding": ProviderPreference(provider="local", priority=90),
            },
        ...
        quality_profile = self.create_profile(
            ...
            providers={
                ...
                "reasoning": ProviderPreference(
                    provider="openai", model="gpt-4o", priority=95
                ),
                "embedding": ProviderPreference(provider="openai", priority=85),
            },
        ...
        balanced_profile = self.create_profile(
            ...
            providers={
                "chat": ProviderPreference(
                    provider="gemini", priority=80, required_capabilities={"streaming"}
                ),
                "code": ProviderPreference(
                    provider="deepseek",
                    priority=85,
                ...
                "reasoning": ProviderPreference(provider="openai", priority=75),
                "embedding": ProviderPreference(provider="huggingface", priority=70),
            },
            fallback_provider="local",
            memory_budget=MemoryBudget(
    ...
    def get_routing_decision(
        self, use_case: str, requirements: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        ...
        if use_case in active_profile.providers:
            ...
            if provider_pref.provider in available_providers:
                ...
                logger.warning(
                    f"Preferred provider {provider_pref.provider} for {use_case} is not available"
                )
    ...
    def get_profile_statistics(self) -> Dict[str, Any]:
        ...
        for profile in self._profiles.values():
            ...
            stats["profiles_by_policy"][policy] = (
                stats["profiles_by_policy"].get(policy, 0) + 1
            )

            # Count provider usage
            for use_case, pref in profile.providers.items():
                provider = pref.provider
                stats["provider_usage"][provider] = (
                    stats["provider_usage"].get(provider, 0) + 1
                )
