...
class SystemValidationResult:
    ...
    provider_results: Dict[str, "ProviderValidationResult"] = field(
...
class ConfigurationValidator:
    ...
    def validate_all_configurations(self) -> SystemValidationResult:
            ...
            try:
                ...
                if provider_result.valid:
                    ...
                    result.recommendations.extend(
                        [
                            f"{provider_name}: {rec}"
                            for rec in provider_result.recommendations
                        ]
                    )
                ...
                result.critical_issues.append(
                    f"Validation error for {provider_name}: {str(e)}"
                )
        ...
        if healthy_count == 0:
            ...
            result.warnings.append(
                f"Only {healthy_count}/{len(providers)} providers are healthy"
            )
        else:
        ...
        logger.info(
            f"Configuration validation complete in {result.validation_time:.2f}s: {result.overall_status}"
        )
        return result
    ...
    def validate_provider_config(self, provider_name: str) -> ProviderValidationResult:
        ...
        if not env_result.all_present:
            ...
            recommendations.extend(
                [f"Set environment variable: {var}" for var in env_result.missing_vars]
            )
        ...
        if not endpoint_result.reachable and spec.requires_api_key:
            warnings.append(
                f"API endpoint not reachable: {endpoint_result.error_message}"
            )
    ...
    def verify_dependencies(self, provider_name: str) -> DependencyCheckResult:
        ...
        for dep in spec.required_dependencies:
            ...
            if is_available:
                ...
                install_commands[dep] = self._install_commands.get(
                    dep, f"pip install {dep}"
                )
    ...
    def test_api_endpoints(self, provider_name: str) -> EndpointTestResult:
        ...
        if endpoint_url in self._endpoint_cache:
            ...
            if (
                hasattr(cached_result, "timestamp")
                and cached_result.timestamp is not None
                and time.time() - cached_result.timestamp < 300
            ):
        ...
        try:
            ...
            if hostname:
                ...
                result.dns_resolved = False
                result.error_message = "Invalid URL: no hostname found"
                result.network_accessible = False
                return result
    ...
    def _generate_system_recommendations(
        ...
        if missing_deps:
            recommendations.append(
                f"Install missing dependencies: {', '.join(sorted(missing_deps))}"
            )

        if missing_env_vars:
            recommendations.append(
                f"Set missing environment variables: {', '.join(sorted(missing_env_vars))}"
            )

        # Provider-specific recommendations
        healthy_providers = [
            name for name, res in result.provider_results.items() if res.valid
        ]
        if not healthy_providers:
            recommendations.append(
                "Consider setting up at least one cloud provider (OpenAI, Gemini, or DeepSeek)"
            )
            recommendations.append(
                "Alternatively, set up local providers (HuggingFace, local models)"
            )
...
def validate_provider_configuration(
    provider_name: str, registry=None
