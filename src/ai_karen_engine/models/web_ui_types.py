...
class ChatProcessRequest(BaseModel):
    ...
    relevant_memories: List[Dict[str, Any]] = Field(default_factory=list, description="Relevant memories")
...
class ChatProcessResponse(BaseModel):
    ...
    finalResponse: str = Field(..., description="Final response to user (camelCase for JS compatibility)")
    acknowledgement: Optional[str] = Field(None, description="Initial acknowledgement message")
    ai_data_for_final_response: Optional[Dict[str, Any]] = Field(None, description="AI metadata")
    suggested_new_facts: Optional[List[str]] = Field(None, description="Suggested facts to remember")
...
class WebUIMemoryQuery(BaseModel):
    ...
    time_range: Optional[List[datetime]] = Field(None, description="Time range [start, end]")
    top_k: Optional[int] = Field(5, ge=1, le=100, description="Maximum number of results")
    ...
    def validate_text(cls, v: str) -> str:
        ...
        if len(cleaned_text) < 1:
            raise ValueError("Query text must contain at least 1 character after cleaning")
    ...
    @classmethod
    def validate_time_range(cls, v: Optional[List[datetime]]) -> Optional[List[datetime]]:
        """Validate time range."""
        if v is None:
            return v
        
        if not isinstance(v, list) or len(v) != 2:
            raise ValueError("Time range must be a list with exactly 2 datetime elements [start, end]")
    ...
    def validate_metadata_filter(cls, v: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        for key, value in v.items():
            ...
            if len(key) > 100:
                raise ValueError("Metadata filter key length cannot exceed 100 characters")
            ...
            if not isinstance(value, (str, int, float, bool, type(None))):
                if isinstance(value, (list, dict)):
                    ...
                    try:
                        ...
                        raise ValueError(f"Metadata filter value for key '{key}' must be JSON serializable")
                else:
                    raise ValueError(f"Metadata filter value for key '{key}' must be a basic type (str, int, float, bool, None, list, dict)")
...
class WebUIMemoryEntry(BaseModel):
    ...
    timestamp: int = Field(..., description="Unix timestamp in milliseconds for JS compatibility")
...
class WebUIMemoryQueryResponse(BaseModel):
    ...
    memories: List[WebUIMemoryEntry] = Field(default_factory=list, description="Memory entries")
    total_count: int = Field(0, ge=0, description="Total number of memories found")
    query_time_ms: float = Field(0.0, ge=0.0, description="Query execution time in milliseconds")
...
class WebUIPluginExecuteResponse(BaseModel):
    ...
    result: Optional[Dict[str, Any]] = Field(None, description="Plugin execution result")
...
class WebUIUsageAnalytics(BaseModel):
    ...
    most_used_features: List[Dict[str, Any]] = Field(default_factory=list, description="Most used features")
    user_activity: Dict[str, Any] = Field(default_factory=dict, description="User activity data")
