...
class DatabaseConsolidationMigrator:
    ...
    async def migrate_all_data(self) -> MigrationResult:
        ...
        """
        result = MigrationResult(success=False, started_at=datetime.now(timezone.utc))
    ...
    async def _initialize_connections(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Initialized {len(self.sqlite_connections)} SQLite connections and PostgreSQL connection"
            )
                            ...
                            if cursor.fetchone():
                                ...
                                for user_row in users:
                                    try:
                                        ...
                                        old_user_id = user_row.get(
                                            "user_id"
                                        ) or user_row.get("id")

                                        if not old_user_id:
                                            self.logger.warning(
                                                f"User without ID found, skipping: {dict(user_row)}"
                                            )
                                            continue
                                        ...
                                        if existing_result:
                                            ...
                                            self.logger.info(
                                                f"User {email} already exists, using existing UUID: {postgres_uuid}"
                                            )
                                        else:
                                            # Insert new user
                                            await self._insert_user_to_postgres(
                                                conn,
                                                user_row,
                                                postgres_uuid,
                                                table_name,
    ...
    async def _insert_user_to_postgres(
        self, conn, user_row: sqlite3.Row, postgres_uuid: str, table_name: str
    ) -> None:
        ...
        try:
            ...
            if not password_hash:
                ...
                self.logger.warning(
                    f"No password hash found for user {user_row['email']}, using placeholder"
                )
            ...
            """
                ),
                {
                    ...
                    "full_name": safe_get(user_row, "full_name")
                    or safe_get(user_row, "name"),
                    "tenant_id": safe_get(
                        user_row, "tenant_id", str(uuid.uuid4())
                    ),  # Generate tenant UUID if missing
                ...
                for sqlite_conn in self.sqlite_connections:
                    ...
                    for table_name in session_tables:
                        try:
                            ...
                            if cursor.fetchone():
                                cursor.execute(
                                    f"SELECT * FROM {table_name} WHERE is_active = 1"
                                )
                                ...
                                for session_row in sessions:
                                    try:
                                        ...
                                        if (
                                            not old_user_id
                                            or old_user_id not in user_mapping
                                        ):
                                            self.logger.warning(
                                                f"Session without valid user_id found, skipping: {dict(session_row)}"
                                            )
                                            continue
                                        ...
                                        if session_token:
                                            ...
                                            if existing_session.fetchone():
                                                self.logger.info(
                                                    f"Session {session_token} already exists, skipping"
                                                )
                                                continue

                                        await self._insert_session_to_postgres(
                                            conn, session_row, postgres_user_id
                                        )
                                        session_count += 1
    ...
    async def _insert_session_to_postgres(
        self, conn, session_row: sqlite3.Row, postgres_user_id: str
    ) -> None:
        ...
        try:
            ...
            if isinstance(security_flags, str):
                ...
                security_flags_list = (
                    security_flags if isinstance(security_flags, list) else []
                )
            ...
            if isinstance(geolocation, str) and geolocation:
                ...
                geolocation_dict = (
                    geolocation if isinstance(geolocation, dict) else None
                )
            ...
            """
                ),
                {
                    ...
                    "session_token": safe_get(
                        session_row, "session_token", str(uuid.uuid4())
                    ),
        ...
        try:
            async with self.async_postgres_engine.begin() as conn:
                for sqlite_conn in self.sqlite_connections:
                    ...
                    try:
                        cursor.execute(
                            "SELECT name FROM sqlite_master WHERE type='table' AND name='auth_password_reset_tokens'"
                        )
                        if cursor.fetchone():
                            cursor.execute(
                                "SELECT * FROM auth_password_reset_tokens WHERE used_at IS NULL"
                            )
                            ...
                            for token_row in reset_tokens:
                                try:
                                    ...
                                    postgres_user_id = user_mapping[old_user_id]
                                    await self._insert_reset_token_to_postgres(
                                        conn, token_row, postgres_user_id
                                    )
                                    token_count += 1

                                except Exception as e:
                                    self.logger.error(
                                        f"Failed to migrate reset token: {e}"
                                    )
                                    self.migration_stats["errors"] += 1
                    except sqlite3.OperationalError:
                        pass
                    ...
                    try:
                        cursor.execute(
                            "SELECT name FROM sqlite_master WHERE type='table' AND name='auth_email_verification_tokens'"
                        )
                        if cursor.fetchone():
                            cursor.execute(
                                "SELECT * FROM auth_email_verification_tokens WHERE used_at IS NULL"
                            )
                            ...
                            for token_row in verification_tokens:
                                try:
                                    ...
                                    postgres_user_id = user_mapping[old_user_id]
                                    await self._insert_verification_token_to_postgres(
                                        conn, token_row, postgres_user_id
                                    )
                                    token_count += 1
                                    ...
                                    self.logger.error(
                                        f"Failed to migrate verification token: {e}"
                                    )
                                    self.migration_stats["errors"] += 1
                    except sqlite3.OperationalError:
                        pass
    ...
    async def _insert_reset_token_to_postgres(
        self, conn, token_row: sqlite3.Row, postgres_user_id: str
    ) -> None:
        ...
        try:
            ...
            """
                ),
                {
                    "id": str(uuid.uuid4()),
                    "user_id": postgres_user_id,
                    "token": safe_get(token_row, "token_hash")
                    or safe_get(token_row, "token", str(uuid.uuid4())),
                    "expires_at": expires_at,
            ...
            raise DatabaseOperationError(
                f"Failed to insert reset token to PostgreSQL: {e}"
            )

    async def _insert_verification_token_to_postgres(
        self, conn, token_row: sqlite3.Row, postgres_user_id: str
    ) -> None:
        ...
        try:
            ...
            raise DatabaseOperationError(
                f"Failed to insert verification token to PostgreSQL: {e}"
            )
        ...
        try:
            ...
            for sqlite_conn in self.sqlite_connections:
                ...
                for table in [
                    ...
                    try:
                        cursor.execute(
                            f"SELECT COUNT(*) FROM {table} WHERE used_at IS NULL"
                        )
                        count = cursor.fetchone()[0]
            ...
            async with self.async_postgres_engine.begin() as conn:
                ...
                result = await conn.execute(
                    text(
                        "SELECT COUNT(*) FROM password_reset_tokens WHERE used = false"
                    )
                )
                ...
                result = await conn.execute(
                    text(
                        "SELECT COUNT(*) FROM email_verification_tokens WHERE used = false"
                    )
                )
                ...
                # Validate counts
                validation_results["user_count_match"] = (
                    sqlite_counts["users"] == postgres_user_count
                )
                validation_results["session_count_match"] = (
                    sqlite_counts["sessions"] == postgres_session_count
                )
                ...
                validation_results["foreign_key_integrity"] = (
                    orphaned_sessions == 0
                    and orphaned_reset_tokens == 0
                    and orphaned_verification_tokens == 0
                )
                ...
                validation_results[
                    "data_integrity_checks"
                ] = await self._perform_data_integrity_checks(conn)

            # Overall success
            validation_results["overall_success"] = (
                validation_results["user_count_match"]
                and validation_results["foreign_key_integrity"]
            ...
            checks.append(
                {
                    ...
                    "details": f"Found {null_tenant_count} users without tenant_id"
                    if null_tenant_count > 0
                    else "All users have tenant_id",
                }
            )
            ...
            checks.append(
                {
                    ...
                    "details": f"Found {expired_active_sessions} expired but active sessions"
                    if expired_active_sessions > 0
                    else "No expired active sessions",
                }
            )
...
class MigrationValidator:
            ...
            async with self.async_engine.begin() as conn:
                ...
                if not all_passed:
                    validation_report[
                        ...
                        validation_report["checks_performed"]
                    )
    ...
    async def _check_data_consistency(self, conn) -> Dict[str, Any]:
        ...
        if sessions_without_users > 0:
            issues.append(
                f"{sessions_without_users} sessions without corresponding users"
            )
    ...
    async def _check_foreign_key_constraints(self, conn) -> Dict[str, Any]:
        ...
        if session_violations > 0:
            constraint_violations.append(
                f"{session_violations} sessions with invalid user_id"
            )
        ...
        if reset_token_violations > 0:
            constraint_violations.append(
                f"{reset_token_violations} reset tokens with invalid user_id"
            )
        ...
        if verification_token_violations > 0:
            constraint_violations.append(
                f"{verification_token_violations} verification tokens with invalid user_id"
            )
    ...
    async def _check_data_anomalies(self, conn) -> Dict[str, Any]:
        ...
        result = await conn.execute(
            text(
                "SELECT COUNT(*) FROM users WHERE email IS NULL OR password_hash IS NULL"
            )
        )
        ...
        if users_with_null_required > 0:
            anomalies.append(
                f"{users_with_null_required} users with null required fields"
            )
        ...
        if invalid_expiration_sessions > 0:
            anomalies.append(
                f"{invalid_expiration_sessions} sessions with invalid expiration dates"
            )
        ...
        for check in checks:
            if not check.get("passed", False):
                ...
                if check_name == "table_existence":
                    ...
                    sessions_without_users = check.get("details", {}).get(
                        "sessions_without_users", 0
                    )
                    if sessions_without_users > 0:
                        recommendations.append(
                            "Remove orphaned sessions or fix user references"
                        )
                    ...
                    if anomalies:
                        recommendations.append(
                            "Address data anomalies: " + "; ".join(anomalies)
                        )
