...
class AuthDataMigrator:
        ...
        try:
            ...
            for db_path in source_databases:
                if not Path(db_path).exists():
                    migration_summary["warnings"].append(f"Source database not found: {db_path}")
                    continue
                
                try:
                    db_summary = self._migrate_single_database(db_path, dry_run)
                    migration_summary["migrated_users"] += db_summary.get("users", 0)
                    migration_summary["migrated_sessions"] += db_summary.get("sessions", 0)
    ...
    def _migrate_single_database(self, db_path: str, dry_run: bool) -> Dict[str, int]:
        ...
        try:
            ...
            if "auth_sessions" in tables:
                ...
                summary["sessions"] = self._migrate_legacy_sessions(source_conn, dry_run)
            ...
            if "auth_events" in tables:
                ...
                summary["events"] = self._migrate_legacy_audit_logs(source_conn, dry_run)
    ...
    def _migrate_users(self, source_conn: sqlite3.Connection, dry_run: bool) -> int:
        ...
        for user_row in users:
            try:
                ...
                logger.error(f"Failed to migrate user {safe_get(user_row, 'email', 'unknown')}: {e}")
        
        target_conn.commit()
        return migrated_count
    
    def _migrate_legacy_users(self, source_conn: sqlite3.Connection, dry_run: bool) -> int:
        ...
        for user_row in users:
            try:
                ...
                logger.error(f"Failed to migrate legacy user {safe_get(user_row, 'email', 'unknown')}: {e}")
        ...
        for session_row in sessions:
            try:
                ...
                if not target_cursor.fetchone():
                    logger.warning(f"User {session_row['user_id']} not found for session migration")
                    continue
                ...
                """, (
                    ...
                    safe_get(session_row, "created_at", datetime.utcnow().isoformat()),
                    safe_get(session_row, "last_accessed", datetime.utcnow().isoformat()),
                ...
                logger.error(f"Failed to migrate session {safe_get(session_row, 'session_token', 'unknown')}: {e}")
        
        target_conn.commit()
        return migrated_count
    
    def _migrate_legacy_sessions(self, source_conn: sqlite3.Connection, dry_run: bool) -> int:
        ...
        for session_row in sessions:
            try:
                ...
                session_token = safe_get(session_row, "session_token") or safe_get(session_row, "token") or str(uuid.uuid4())
    ...
    def _migrate_events(self, source_conn: sqlite3.Connection, dry_run: bool) -> int:
        ...
        cursor.execute("SELECT * FROM auth_events WHERE timestamp > ? ORDER BY timestamp DESC LIMIT 10000", (cutoff_date,))
        ...
        for event_row in events:
            try:
                ...
                logger.error(f"Failed to migrate event {safe_get(event_row, 'event_id', 'unknown')}: {e}")
        
        target_conn.commit()
        return migrated_count
    
    def _migrate_legacy_audit_logs(self, source_conn: sqlite3.Connection, dry_run: bool) -> int:
        """Migrate events from legacy audit_logs table format."""
        cursor = source_conn.cursor()
        # Only migrate recent events (last 30 days)
        cutoff_date = (datetime.utcnow() - timedelta(days=30)).isoformat()
        cursor.execute("SELECT * FROM audit_logs WHERE created_at > ? ORDER BY created_at DESC LIMIT 10000", (cutoff_date,))
        events = cursor.fetchall()
        ...
        for event_row in events:
            try:
                ...
                timestamp = safe_get(event_row, "created_at", datetime.utcnow().isoformat())
                ...
                """, (
                    ...
                    json.dumps(safe_get(event_row, "details", {})) if isinstance(safe_get(event_row, "details"), dict) else safe_get(event_row, "details", "{}"),
                    "migrated-legacy"
                ))
    ...
    def cleanup_old_data(self, older_than_days: int = 90) -> Dict[str, int]:
        ...
        try:
            ...
            cursor.execute("DELETE FROM auth_password_reset_tokens WHERE expires_at < ?", (datetime.utcnow().isoformat(),))
            cleanup_summary["expired_tokens"] += cursor.rowcount
            
            # Clean up expired email verification tokens
            cursor.execute("DELETE FROM auth_email_verification_tokens WHERE expires_at < ?", (datetime.utcnow().isoformat(),))
            cleanup_summary["expired_tokens"] += cursor.rowcount
            
            # Clean up old events (keep recent ones for audit)
            cursor.execute("DELETE FROM auth_events WHERE timestamp < ? AND event_type NOT IN ('login', 'logout', 'password_change')", (cutoff_date,))
            cleanup_summary["old_events"] = cursor.rowcount
            
            # Clean up inactive sessions
            cursor.execute("DELETE FROM auth_sessions WHERE is_active = 0 AND created_at < ?", (cutoff_date,))
            ...
            raise DatabaseOperationError(f"Failed to cleanup old data: {e}", operation="cleanup")
    ...
    def validate_migration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if users_without_passwords > 0:
                validation_report["warnings"].append(f"{users_without_passwords} users without password hashes")
            ...
            if orphaned_sessions > 0:
                validation_report["issues"].append(f"{orphaned_sessions} orphaned sessions found")
            ...
            if duplicate_emails:
                validation_report["issues"].append(f"{len(duplicate_emails)} duplicate email addresses found")
            
            # Get statistics
            cursor.execute("SELECT COUNT(*) as count FROM auth_users")
            validation_report["statistics"]["total_users"] = cursor.fetchone()["count"]
            
            cursor.execute("SELECT COUNT(*) as count FROM auth_sessions WHERE is_active = 1")
            validation_report["statistics"]["active_sessions"] = cursor.fetchone()["count"]
