...
class SQLiteFallbackAuth:
        ...
        **kwargs
        ...
        try:
            ...
            """, (
                ...
                json.dumps(roles or ["user"]), tenant_id,
    ...
    async def authenticate_user(
        ...
        **kwargs
        ...
        try:
            ...
            user_id, email, password_hash, full_name, roles_json, tenant_id, is_verified, is_active, failed_attempts, locked_until = row
            ...
            if not self._verify_password(password, password_hash):
                ...
                if failed_attempts >= 5:  # Lock after 5 failed attempts
                    locked_until_new = (datetime.utcnow() + timedelta(minutes=15)).isoformat()
        ...
        if session.is_expired():
            ...
            if session.user_data.user_id in self.user_sessions:
                self.user_sessions[session.user_data.user_id] = [
                    s for s in self.user_sessions[session.user_data.user_id] 
                    if s != session_token
                ]
            return None
    ...
    async def get_user_by_email(self, email: str) -> Optional[UserData]:
        ...
        try:
            ...
            user_id, email, full_name, roles_json, tenant_id, is_verified, is_active, created_at, updated_at, last_login_at = row
    ...
    async def _log_auth_event(
        ...
        try:
            ...
            """, (
                str(uuid4()), event_type.value, user_id, email, ip_address, user_agent,
                session_token, success, error_message, json.dumps(details or {}), risk_score
            ))
