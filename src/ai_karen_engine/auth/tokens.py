...
class TokenManager:
    ...
    def __init__(self, config: JWTConfig) -> None:
        ...
        if jwt is None:
            raise ImportError("PyJWT is required for token management. Install with: pip install PyJWT")
    ...
    async def create_access_token(
        ...
        if expires_delta:
            ...
            expire = datetime.utcnow() + self.config.access_token_expiry
    ...
    async def create_refresh_token(
        ...
        if expires_delta:
            ...
            expire = datetime.utcnow() + self.config.refresh_token_expiry
    ...
    async def create_password_reset_token(
        ...
        if expires_delta:
            ...
            expire = datetime.utcnow() + self.config.password_reset_token_expiry
    ...
    async def create_email_verification_token(
        ...
        if expires_delta:
            ...
            expire = datetime.utcnow() + self.config.email_verification_token_expiry
    ...
    async def validate_token(
        ...
        try:
            ...
            raise InvalidTokenError(f"Token validation failed: {e}", token_type=expected_type)
    ...
    async def refresh_access_token(
        ...
        if payload.get("sub") != user_data.user_id:
            raise InvalidTokenError("Refresh token does not belong to user", token_type="refresh")
        
        # Create new access token
        return await self.create_access_token(user_data)
    
    def get_token_payload_without_validation(self, token: str) -> Optional[Dict[str, Any]]:
        """
