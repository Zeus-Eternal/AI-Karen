...
class DatabaseSchemaManager:
    ...
    def create_unified_schema(self) -> None:
        ...
        try:
            ...
            raise DatabaseOperationError(f"Failed to create schema: {e}", operation="create_schema")
    ...
    def _create_indexes(self) -> None:
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_users_active ON auth_users (is_active)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_users_verified ON auth_users (is_verified)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_users_created ON auth_users (created_at)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_users_last_login ON auth_users (last_login_at)")
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_sessions_expires ON auth_sessions (expires_in)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_sessions_created ON auth_sessions (created_at)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_sessions_last_accessed ON auth_sessions (last_accessed)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_sessions_ip ON auth_sessions (ip_address)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_sessions_device ON auth_sessions (device_id)")
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_events_success ON auth_events (success)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_events_ip ON auth_events (ip_address)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_events_session ON auth_events (session_token)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_events_correlation ON auth_events (correlation_id)")
        
        # Rate limiting indexes
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_rate_limits_identifier ON auth_rate_limits (identifier)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_rate_limits_type ON auth_rate_limits (limit_type)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_rate_limits_window ON auth_rate_limits (window_start)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_rate_limits_blocked ON auth_rate_limits (blocked_until)")
        
        # RBAC indexes
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_user_roles_user ON auth_user_roles (user_id)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_user_roles_role ON auth_user_roles (role_name)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_user_roles_active ON auth_user_roles (is_active)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_role_permissions_role ON auth_role_permissions (role_name)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_role_permissions_permission ON auth_role_permissions (permission_id)")
        
        # Device tracking indexes
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_user_devices_user ON auth_user_devices (user_id)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_user_devices_fingerprint ON auth_user_devices (device_fingerprint)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_user_devices_trusted ON auth_user_devices (is_trusted)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_user_devices_last_seen ON auth_user_devices (last_seen_at)")
        
        # Security alerts indexes
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_security_alerts_user ON auth_security_alerts (user_id)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_security_alerts_type ON auth_security_alerts (alert_type)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_security_alerts_severity ON auth_security_alerts (severity)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_security_alerts_status ON auth_security_alerts (status)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_auth_security_alerts_created ON auth_security_alerts (created_at)")
    ...
    def _create_triggers(self) -> None:
        ...
        cursor.execute("""
            ...
            BEGIN
                UPDATE auth_sessions SET last_accessed = datetime('now') WHERE session_token = NEW.session_token;
            END
        """)
    ...
    def _insert_default_data(self) -> None:
        ...
        default_permissions = [
            ...
            ("auth:change_password", "authentication", "change_password", "Allow user to change password"),
            ("auth:reset_password", "authentication", "reset_password", "Allow user to reset password"),
            ("user:read_profile", "user", "read", "Allow user to read their profile"),
            ("user:update_profile", "user", "update", "Allow user to update their profile"),
            ("admin:manage_users", "user", "manage", "Allow admin to manage users"),
            ("admin:view_audit_logs", "audit", "read", "Allow admin to view audit logs"),
        ]
        ...
        for perm_name, resource_type, action, description in default_permissions:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """, (perm_id, perm_name, resource_type, action, description, datetime.utcnow().isoformat()))
    ...
    def get_schema_version(self) -> Optional[str]:
        ...
        try:
            cursor = self.connection.cursor()
            cursor.execute("SELECT version FROM auth_schema_version ORDER BY applied_at DESC LIMIT 1")
            row = cursor.fetchone()
    ...
    def optimize_database(self) -> None:
        ...
        try:
            ...
            raise DatabaseOperationError(f"Failed to optimize database: {e}", operation="optimize")
    ...
    def get_table_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            raise DatabaseOperationError(f"Failed to get table statistics: {e}", operation="get_statistics")
