...
class UserData:
    ...
    def validate(self) -> bool:
        ...
        return (
            ...
            self.failed_login_attempts >= 0 and
            isinstance(self.preferences, dict)
        )
...
class SessionData:
    ...
    def validate(self) -> bool:
        ...
        return (
            bool(self.session_token) and
            bool(self.access_token) and
            bool(self.refresh_token) and
            self.expires_in > 0 and
            self.user_data.validate() and
...
class AuthEvent:
    ...
    def validate(self) -> bool:
        ...
        return (
            isinstance(self.event_type, AuthEventType) and
            isinstance(self.timestamp, datetime) and
            0.0 <= self.risk_score <= 1.0 and
            self.processing_time_ms >= 0.0 and
            isinstance(self.details, dict) and
            isinstance(self.security_flags, list)
        )
