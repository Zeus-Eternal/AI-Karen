...
from .models import AuthEvent, UserData, SessionData
...
class OptimizedAuthDatabaseClient:
    ...
    async def upsert_user(self, user_data: UserData, password_hash: str) -> UserData:
        ...
        try:
            async with self.session_factory() as session:
                ...
                if not user_result:
                    raise DatabaseOperationError("Failed to upsert user", operation="upsert_user")
    ...
    async def get_user_with_roles(self, email: str, required_roles: List[str]) -> Optional[UserData]:
        """
    ...
    async def bulk_update_user_preferences(self, updates: List[Tuple[str, Dict[str, Any]]]) -> int:
        """
        ...
        try:
            ...
            raise DatabaseOperationError(
                f"Failed to bulk update preferences: {e}", operation="bulk_update_preferences"
            )
    ...
    async def create_session_optimized(self, session_data: SessionData) -> None:
        ...
        try:
            ...
            raise DatabaseOperationError(
                f"Failed to create optimized session: {e}", operation="create_session_optimized"
            )

    async def validate_session_with_user(self, session_token: str) -> Optional[Tuple[SessionData, UserData]]:
        """
        ...
        try:
            ...
            raise DatabaseOperationError(
                f"Failed to validate session with user: {e}", operation="validate_session_with_user"
            )
