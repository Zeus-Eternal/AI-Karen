        ...
        self._alert_rules = [
            {
                ...
                "condition": lambda: self.metrics_collector.get_rate("auth.login.failed", 5) > 10,
                "severity": "high",
            ...
            },
            {
                ...
                "condition": lambda: self.metrics_collector.get_rate("auth.errors", 5) > 5,
                "severity": "medium",
            ...
            },
            {
                ...
                "condition": lambda: self.metrics_collector.get_rate("auth.security.blocked", 5) > 2,
                "severity": "high",
            ...
            },
            {
                ...
                "condition": lambda: self.metrics_collector.get_histogram_stats("auth.timing.authenticate").get("p95", 0) > 5000,
                "severity": "medium",
            ...
            },
            {
                ...
                "condition": lambda: self.metrics_collector.get_rate("auth.anomaly.detected", 5) > 1,
                "severity": "high",
    ...
    async def _check_alert_rules(self) -> None:
        ...
        for rule in self._alert_rules:
            try:
                ...
                self.logger.error(f"Error checking alert rule {rule.get('name', 'unknown')}: {e}")
...
class AuthMonitor:
    ...
    async def _record_event_metrics(self, event: AuthEvent) -> None:
        ...
        if event.risk_score > 0:
            await self.metrics.record_histogram("auth.risk_score", event.risk_score, base_tags)
        ...
        if not event.success and event.error_message:
            error_tags = {**base_tags, "error_type": self._classify_error(event.error_message)}
    ...
    async def record_user_activity(
        ...
        await self.metrics.record_gauge(f"auth.user.last_activity.{user_id}", time.time())
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        critical_alerts = [
            alert for alert in self.alerts.get_active_alerts()
