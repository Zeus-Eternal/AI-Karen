...
from ai_karen_engine.extensions.platform.core.manager import get_plugin_manager
from ai_karen_engine.extensions.platform.core.manager import (
    PluginManager as ExtensionManager,
)
...
class HookEnabledFileService(FileAttachmentService):
    ...
    async def upload_file(
        self, request: FileUploadRequest, file_handle: BinaryIO
    ) -> FileUploadResponse:
        ...
        try:
            ...
            pre_upload_summary = await self.hook_manager.trigger_hooks(
                pre_upload_context
            )
            ...
            for result in pre_upload_summary.results:
                if not result.success:
                    logger.warning(
                        f"Pre-upload hook {result.hook_id} failed: {result.error}"
                    )
                    ...
                    return FileUploadResponse(
                        ...
                        message=result.data.get(
                            "block_reason", "Upload blocked by security hook"
                        ),
                    )
            ...
            if upload_response.success:
                ...
                post_upload_context = HookContext(
                    ...
                    data={
                        ...
                        "pre_upload_results": [
                            r.dict() for r in pre_upload_summary.results
                        ],
                    },
                    user_context={
                        "user_id": request.user_id,
    ...
    async def _handle_post_upload_hooks(self, context: HookContext):
        ...
        try:
            ...
            for result in post_upload_summary.results:
                if result.success:
                    logger.debug(
                        f"Post-upload hook {result.hook_id} completed successfully"
                    )
                else:
                    logger.warning(
                        f"Post-upload hook {result.hook_id} failed: {result.error}"
                    )
    ...
    async def _process_file(
        self, file_id: str, file_path: Path, metadata: FileMetadata
    ) -> None:
        ...
        try:
            ...
            if self.enable_security_scan:
                ...
                security_summary = await self.hook_manager.trigger_hooks(
                    security_context
                )
            ...
            if self.enable_content_extraction:
                ...
                analysis_summary = await self.hook_manager.trigger_hooks(
                    analysis_context
                )
            ...
            if metadata.file_type in [FileType.IMAGE, FileType.AUDIO, FileType.VIDEO]:
                ...
                multimedia_summary = await self.hook_manager.trigger_hooks(
                    multimedia_context
                )
            ...
            thumbnail_path = await self._generate_thumbnail_with_hooks(
                file_path, metadata.file_type
            )
            if thumbnail_path:
            ...
            self.event_bus.publish(
                ...
                "file_processing_complete",
                {
                    ...
                    "extracted_content_available": metadata.extracted_content
                    is not None,
                },
            )
            ...
            logger.error(
                f"Enhanced file processing failed for {file_id}: {e}", exc_info=True
            )
    ...
    async def _execute_pre_upload_hooks(self, context: HookContext) -> Dict[str, Any]:
        ...
        try:
            ...
            validation_plugins = await self.plugin_manager.get_plugins_by_category(
                "file_validation"
            )
            ...
            for plugin_name in validation_plugins:
                try:
                    result = await self.plugin_manager.run_plugin(
                        plugin_name, context.data, context.user_context
                    )
    ...
    async def _execute_post_upload_hooks(self, context: HookContext) -> Dict[str, Any]:
        ...
        try:
            ...
            processing_plugins = await self.plugin_manager.get_plugins_by_category(
                "file_post_upload"
            )
    ...
    async def _execute_security_hooks(self, context: HookContext) -> Dict[str, Any]:
        ...
        try:
            ...
            security_plugins = await self.plugin_manager.get_plugins_by_category(
                "file_security"
            )
    ...
    async def _execute_analysis_hooks(self, context: HookContext) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis_plugins = await self.plugin_manager.get_plugins_by_category(
                "file_analysis"
            )
    ...
    async def _execute_multimedia_hooks(self, context: HookContext) -> Dict[str, Any]:
        ...
        try:
            ...
            multimedia_plugins = await self.plugin_manager.get_plugins_by_category(
    ...
    async def _process_security_results(
        self, security_summary, file_id: str, file_path: Path, metadata: FileMetadata
    ) -> str:
        ...
        try:
            ...
            if base_result == "malicious" or is_malicious:
                await self._quarantine_file(
                    file_id, file_path, "Malicious content detected by security hooks"
                )
                return "quarantined"
    ...
    async def _process_analysis_results(
        self, analysis_summary, file_id: str, metadata: FileMetadata
    ) -> None:
            ...
            for result in analysis_summary.results:
                if result.success and result.data:
                    ...
                    for plugin_analysis in plugin_results:
                        if plugin_analysis.get("success") and plugin_analysis.get(
                            "result"
                        ):
                            ...
                            if analysis_result.get("analysis_data"):
                                ...
                                analysis_data[plugin_name] = analysis_result[
                                    "analysis_data"
                                ]
    ...
    async def _process_multimedia_results(
        self, multimedia_summary, file_id: str, file_path: Path, metadata: FileMetadata
    ) -> None:
        ...
        try:
            ...
            for result in multimedia_summary.results:
                if result.success and result.data:
                    ...
                    for plugin_multimedia in plugin_results:
                        if plugin_multimedia.get("success") and plugin_multimedia.get(
                            "result"
                        ):
            ...
            # Update metadata
            if multimedia_data:
                metadata.analysis_results.update(
                    {"multimedia_analysis": multimedia_data}
                )
    ...
    async def _execute_extension_processing(
        ...
        try:
            ...
            extensions = self.extension_manager.get_extensions_by_category(
                "file_processing"
            )
    ...
    async def get_file_analysis(self, file_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            analysis = {
                ...
                "processing_complete": metadata.processing_status
                == ProcessingStatus.COMPLETED,
                "security_status": metadata.security_scan_result.value
                ...
                "features": {
                    ...
                    "extracted_content_available": metadata.extracted_content
                    is not None,
                    "plugin_analysis_available": "plugin_analysis"
                    in metadata.analysis_results,
                    "multimedia_analysis_available": "multimedia_analysis"
                    in metadata.analysis_results,
                },
            }
