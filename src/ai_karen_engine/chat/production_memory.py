...
from ai_karen_engine.database.models.auth_models import ChatMemory
from ai_karen_engine.database.client import get_db_session
from ai_karen_engine.clients.database.milvus_client import store_vector, recall_vectors
...
class ProductionChatMemory:
    ...
    async def store_turn(
        ...
        try:
            ...
            self.redis_client.expire(turns_key, timedelta(days=chat_config.short_term_days))
    ...
    async def semantic_search(
        ...
        try:
            ...
            logger.info(f"Semantic search returned {len(filtered_results)} results for user {user_id}")
    ...
    async def cleanup_expired_memory(self, user_id: str) -> Dict[str, int]:
        ...
        try:
            with get_db_session() as db:
                ...
                chat_memories = db.query(ChatMemory).filter(
    ...
    async def update_user_memory_settings(
        ...
        try:
            with get_db_session() as db:
                chat_memory = db.query(ChatMemory).filter(
                    and_(
                        ChatMemory.user_id == user_id,
                ...
                if "summarize_threshold_tokens" in settings_update:
                    chat_memory.summarize_threshold_tokens = settings_update["summarize_threshold_tokens"]
                ...
                logger.info(f"Updated memory settings for user {user_id}, chat {chat_id}")
                return True
    ...
    async def _summarize_if_needed(
        ...
        try:
            ...
            self.redis_client.setex(
                summary_key,
                timedelta(days=chat_config.short_term_days),
                summary
            )
    ...
    async def _update_redis_ttls(self, user_id: str, chat_id: str, chat_config: ChatMemory):
        ...
        if self.redis_client.exists(summary_key):
            self.redis_client.expire(summary_key, timedelta(days=chat_config.short_term_days))
