...
class WebSocketMessage:
    ...
    def __init__(self, content: str, type_: 'MessageType', metadata: Optional[Dict[str, Any]] = None):
...
class PresenceManager:
    ...
    async def update_presence(self, user_id: str, status: str, conversation_id: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None) -> None:
    ...
    def get_online_users(self, conversation_id: Optional[str] = None) -> List[CollaborationUser]:
...
class TypingManager:
    ...
    def __init__(self):
        self.typing_data: Dict[str, Dict[str, Any]] = {}  # conversation_id -> {user_id: typing_info}
        self.event_bus = get_event_bus()
    
    async def set_typing(self, user_id: str, conversation_id: str, is_typing: bool, expires_in_seconds: int = 5) -> None:
        ...
        try:
            ...
            logger.debug(f"Set typing indicator for user {user_id} in conversation {conversation_id}: {is_typing}")
...
class CollaborationManager:
    ...
    async def create_session(
        ...
        try:
            ...
            initiator = CollaborationUser(
                ...
                username=metadata.get("initiator_username", initiator_user_id) if metadata else initiator_user_id,
                status="online"
            )
            ...
            logger.info(f"Created collaboration session {session_id} for conversation {conversation_id}")
            return session_id
    ...
    async def join_session(self, session_id: str, user_id: str, username: Optional[str] = None) -> bool:
    ...
    def get_conversation_sessions(self, conversation_id: str) -> List[CollaborationSession]:
...
class WebSocketGateway:
    ...
    def __init__(self, chat_orchestrator=None, cleanup_interval: float = 30.0, heartbeat_interval: float = 30.0) -> None:
        ...
        # Connection tracking
        self.active_connections: Dict[str, Dict[str, Any]] = {}  # connection_id -> connection_info
    ...
    async def start(self) -> None:
        ...
        self._collaboration_cleanup_task = asyncio.create_task(self._cleanup_collaboration_sessions())
        self._tasks = [
        ...
        try:
            while True:
                ...
                for session_id, session in self.collaboration_manager.active_sessions.items():
                    ...
                    if session.started_at and (current_time - session.started_at) > timedelta(minutes=30):
                        # Check if any participants are still online
                        has_online_participants = False
                        for participant in session.participants:
                            presence = self.presence_manager.get_presence(participant.user_id)
                ...
                for session_id in inactive_sessions:
                    ...
                    logger.info(f"Cleaned up inactive collaboration session: {session_id}")
        ...
        try:
            ...
            for user_id in list(self.message_queue.keys()):
                ...
                self.message_queue[user_id] = [
                    ...
                    if datetime.fromisoformat(msg.get("timestamp", current_time.isoformat())) > cutoff_time
                ]
        ...
        try:
            ...
            for connection_id, connection_info in self.active_connections.items():
                try:
                    ...
                    logger.debug(f"Failed to send heartbeat to connection {connection_id}: {e}")
    ...
    async def _process_websocket_message(self, connection_id: str, message_data: Dict[str, Any]) -> None:
    ...
    async def _handle_auth_message(self, connection_id: str, message_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if user_id:
                ...
                await self._send_message(connection_info["websocket"], {
                    "type": "auth_success",
                    "user_id": user_id,
                    "connection_id": connection_id
                })
    ...
    async def _handle_presence_update(self, connection_id: str, message_data: Dict[str, Any]) -> None:
    ...
    async def _handle_typing_indicator(self, connection_id: str, message_data: Dict[str, Any]) -> None:
    ...
    async def _handle_collaboration_invite(self, connection_id: str, message_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if conversation_id:
                ...
                for invited_user_id in invited_users:
                    await self._send_to_user(invited_user_id, {
    ...
    async def _handle_collaboration_join(self, connection_id: str, message_data: Dict[str, Any]) -> None:
            ...
            if session_id:
                ...
                if success:
                    ...
                    if session:
                        ...
                        for participant in session.participants:
                            await self._send_to_user(participant.user_id, {
    ...
    async def _handle_collaboration_leave(self, connection_id: str, message_data: Dict[str, Any]) -> None:
    ...
    async def _handle_collaborative_edit(self, connection_id: str, message_data: Dict[str, Any]) -> None:
    ...
    async def _handle_chat_message(self, connection_id: str, message_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if self.chat_orchestrator and message_content:
                ...
                logger.info(f"Processing chat message from user {user_id}: {message_content[:50]}...")
    ...
    async def _broadcast_to_conversation(self, conversation_id: str, message_data: Dict[str, Any], exclude_user: Optional[str] = None) -> None:
    ...
    def get_conversation_connections(self, conversation_id: str) -> List[Dict[str, Any]]:
