...
from .response_formatting_models import (
    ...
    SyntaxHighlightConfig, FormattingConfig, ScreenSize, DeviceType
)

logger = logging.getLogger(__name__)

# --- Internal Responsive Engine ---

class ResponsiveEngine:
    """
    Internal engine for device-specific response adaptation.
    Handles layout changes based on display context and screen constraints.
    """
    ...
    async def adapt(self, content: str, context: ResponseContext, layout_type: LayoutType) -> Dict[str, Any]:
        """Adapt content based on display context."""
        device_type = self._detect_device_type(context)
        
        # Apply specific adaptations if available
        ...
        return {
            ...
            'accessibility_adaptations': [f'optimized-for-{device_type.value}'] if device_type != DeviceType.DESKTOP else []
        }

    def _detect_device_type(self, context: ResponseContext) -> DeviceType:
        if context.display_context == DisplayContext.MOBILE: return DeviceType.PHONE
        if context.display_context == DisplayContext.TABLET: return DeviceType.TABLET
        if context.display_context == DisplayContext.TERMINAL: return DeviceType.DESKTOP
        return DeviceType.DESKTOP

    def _adapt_table(self, content: str, device: DeviceType) -> str:
        if device == DeviceType.PHONE:
            # Simple transformation of MD tables to bullet points for small screens
            lines = content.split('\n')
            new_lines = []
            for line in lines:
    ...
    def _adapt_list(self, content: str, device: DeviceType) -> str: return content
    def _adapt_code(self, content: str, device: DeviceType) -> str: return content
...
class PrettyOutputLayer:
    ...
    Consolidates legacy logic into a modular, high-performance transformation engine.
    """
    ...
    def __init__(self, config: Optional[FormattingConfig] = None):
        ...
        logger.info(f"PrettyOutputLayer initialized with profile: {self.config.output_profile.value}")

    def _init_registries(self):
        """Initialize layout and profile registries."""
        ...
        self._profile_formatters = {
            ...
            OutputProfile.VERBOSE: lambda c, ctx: f"Detailed Response:\n{c}\n[Words: {len(c.split())}]",
            OutputProfile.ACCESSIBLE: lambda c, ctx: f'<section aria-label="AI response">{c}</section>',
            OutputProfile.TECHNICAL: lambda c, ctx: f"```technical\n{c}\n```",
            OutputProfile.CONVERSATIONAL: lambda c, ctx: f"Here's what I found: {c}",
        }

    async def _ensure_external_subsystems(self):
        """Lazy load and initialize expensive external subsystems."""
    ...
    async def format_response(
        ...
        try:
            ...
            # 3. Detection Phase (Content Type & Layout)
            detected_type = ContentType.TEXT
            confidence = 0.5
            layout_hint = LayoutHint(layout_type=self.config.default_layout)
            
            if self.content_detector:
                detection = await self.content_detector.detect_content_type(response_content, user_query=context.user_query)
            ...
            formatted_content = self._layout_formatters.get(layout_hint.layout_type, self._layout_formatters[LayoutType.DEFAULT])(
                response_content, layout_hint.parameters, context
            )

            # 5. Enhancement Phase (Syntax & Responsive)
            if self.config.enable_syntax_highlighting and self.syntax_highlighter:
                if detected_type in [ContentType.CODE, ContentType.PYTHON, ContentType.JAVASCRIPT, ContentType.JSON]:
                    try:
                        res = await self.syntax_highlighter.highlight_code(formatted_content, SyntaxHighlightConfig(language=detected_type.value))
                        formatted_content = getattr(res, 'highlighted_content', formatted_content)
                    except Exception: pass

            css_classes = []
            accessibility_features = []
            if self.config.enable_responsive_formatting:
                resp = await self.responsive_engine.adapt(formatted_content, context, layout_hint.layout_type)
            ...
            return {"content": response_content, "metadata": {"error": str(e), "processing_time": time.time() - start_time}}
