...
class ToolExecutionContext(BaseModel):
    ...
    execution_id: str = Field(
        default_factory=lambda: str(uuid.uuid4()), description="Execution ID"
    )
...
class BaseTool(ABC):
    ...
    def _apply_validation_rules(
        self, param_name: str, value: Any, rules: Dict[str, Any]
    ) -> List[str]:
...
class BuiltinTool(BaseTool):
    ...
    async def execute(
        ...
        try:
            ...
            return ToolExecutionResult(
                success=True, result=result, execution_time=time.time() - start_time
            )
...
class ToolIntegrationService:
    ...
    def __init__(self):
        ...
        try:
            ...
            initialize_weather_tools(self)
            logger.info("Weather tools registered successfully")
        except Exception as e:
            logger.error(f"Failed to initialize weather tools: {e}")
    ...
    def _register_builtin_tools(self):
        ...
        calculator_tool = BuiltinTool(
            calculator_metadata, calculator_params, self._calculator_function
        )
        ...
        text_analyzer_params = [
            ...
            ToolParameter(
                ...
                default_value="basic",
                validation_rules={
                    "allowed_values": ["basic", "sentiment", "keywords", "readability"]
                },
            ),
        ]

        text_analyzer_tool = BuiltinTool(
            text_analyzer_metadata, text_analyzer_params, self._text_analyzer_function
        )
        ...
        url_shortener_tool = ExternalAPITool(
            url_shortener_metadata, url_shortener_params, url_shortener_config
        )
