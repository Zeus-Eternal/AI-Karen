...
from ai_karen_engine.database.conversation_manager import normalize_user_id
...
class ConversationManager:
    ...
    def __init__(
        ...
        # Services
        self.search_service = ConversationSearchService(distilbert_service)
        ...
        try:
            ...
            if not title:
                title = await self._generate_conversation_title(
                    normalize_user_id(user_id), initial_message
                )
    ...
    async def move_to_folder(
        ...
        try:
            async with self.db_client.get_async_session() as session:
                ...
                # Update metadata
                metadata = cast(Dict[str, Any], db_conversation.conversation_metadata or {})
