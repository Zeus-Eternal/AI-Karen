"""
Security utilities for the AI-Karen chat system.
    ...
    def _check_anomalies(self, current_event: SecurityEvent) -> None:
        ...
        if len(failed_auth_events) >= self.anomaly_thresholds["failed_auth_per_hour"]:
            self._create_anomaly_alert(
                ...
                ThreatLevel.HIGH,
                {"count": len(failed_auth_events), "timeframe": "1 hour"},
            )
        ...
        if (
            ...
            >= self.anomaly_thresholds["rate_limit_exceeded_per_hour"]
        ):
            self._create_anomaly_alert(
                "high_rate_limit_violations",
                ThreatLevel.HIGH,
                {"count": len(rate_limit_events), "timeframe": "1 hour"},
            )

    def _create_anomaly_alert(
        self, anomaly_type: str, threat_level: ThreatLevel, details: Dict[str, Any]
    ) -> None:
    ...
    def get_threat_summary(self, hours: int = 24) -> Dict[str, Any]:
        ...
        for threat_level in ThreatLevel:
            threat_counts[threat_level.value] = len(
                [e for e in recent_events if e.threat_level == threat_level]
            )
