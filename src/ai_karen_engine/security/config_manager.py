...
class ConfigurationManager:
    ...
    def __init__(
        ...
        # File watching
        self._file_observer: Optional[Observer] = (
            None if not WATCHDOG_AVAILABLE else None
        )
    ...
    async def initialize(
        self, initial_config: Optional[IntelligentAuthConfig] = None
    ) -> bool:
        """
        ...
        if watch and self.enable_hot_reload:
            ...
            if self._file_observer:
                self._file_observer.schedule(
                    self._file_watcher, str(file_path.parent), recursive=False
                )
    ...
    async def rollback_to_version(self, version_id: str) -> bool:
        ...
        with self._config_lock:
            ...
            success = await self.update_config(
                config, target_version.source, f"Rollback to version {version_id}"
            )
    ...
    def get_change_history(self, limit: int = 50) -> List[ConfigChangeEvent]:
        ...
        with self._config_lock:
            return (
                self._change_history[-limit:]
                if limit > 0
                else self._change_history.copy()
            )
    ...
    async def validate_config(
        self, config: IntelligentAuthConfig
    ) -> tuple[bool, List[str]]:
        """
        ...
        try:
            ...
            if not (
                0
                <= thresholds.low_risk_threshold
                <= thresholds.medium_risk_threshold
                <= thresholds.high_risk_threshold
                <= thresholds.critical_risk_threshold
                <= 1.0
            ):
                errors.append(
                    "Risk thresholds must be in ascending order between 0 and 1"
                )
    ...
    def get_configuration_summary(self) -> Dict[str, Any]:
        ...
        with self._config_lock:
            ...
            return {
                ...
                "current_config_valid": current_config.validate()
                if current_config
                else False,
                ...
                "file_watching_active": self._file_observer is not None
    ...
    async def _reload_from_file(self, file_path: Path) -> None:
        ...
        try:
            ...
            await self.update_config(
                config, ConfigSource.FILE, f"Hot-reload from {file_path}"
            )
        except Exception as e:
    ...
    async def _notify_configurable_services(
        self, new_config: IntelligentAuthConfig
    ) -> None:
        """Notify all registered configurable services of configuration change."""
        for service in self._configurable_services:
            try:
                success = await service.update_config(new_config)
                if not success:
                    self.logger.warning(
                        "Config update rejected by %s", service.__class__.__name__
                    )
            except Exception as e:
...
async def save_config_to_file(
    config: IntelligentAuthConfig, file_path: Union[str, Path]
) -> None:
