...
from ai_karen_engine.security.models import AuthContext, ThreatAnalysis
...
class AttackSignature:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "AttackSignature":
        ...
        return cls(
            ...
            threat_actor=ThreatActor(data["threat_actor"])
...
class AttackCampaign:
    ...
    def add_event(self, event: CampaignEvent) -> None:
        ...
        if event.auth_context.geolocation:
            ...
            self.geographic_distribution[country] = (
                self.geographic_distribution.get(country, 0) + 1
            )
...
class CampaignAnalysisResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            "detected_campaigns": [
                campaign.to_dict() for campaign in self.detected_campaigns
            ],
            "new_campaigns": [campaign.to_dict() for campaign in self.new_campaigns],
            "updated_campaigns": [
                campaign.to_dict() for campaign in self.updated_campaigns
            ],
            "campaign_correlations": self.campaign_correlations,
            "threat_intelligence_updates": [
                indicator.to_dict() for indicator in self.threat_intelligence_updates
            ],
...
class CampaignDatabase:
    ...
    def __init__(self, persistence_file: Optional[str] = None):
        ...
        self.campaign_index: Dict[str, Set[str]] = defaultdict(
            set
    ...
    def _update_index(self, campaign: AttackCampaign) -> None:
        ...
        # Index by type
        self.campaign_index[f"type:{campaign.campaign_type.value}"].add(
            campaign.campaign_id
        )

        # Index by threat actor
        if campaign.threat_actor:
            self.campaign_index[f"actor:{campaign.threat_actor.value}"].add(
                campaign.campaign_id
            )
...
class CampaignAnalyzer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.campaign_db = CampaignDatabase(
            config.get(
                "persistence_file", "data/intelligent_auth/attack_campaigns.json"
            )
        )
    ...
    async def analyze_attack_campaigns(
        self,
        ...
        # Correlate campaigns
        campaign_correlations = await self._correlate_campaigns(
            new_campaigns + updated_campaigns
        )

        # Generate threat intelligence updates
        threat_intel_updates = await self._generate_threat_intelligence(
            new_campaigns + updated_campaigns
        )
    ...
    async def _detect_new_campaigns(
        ...
        for group_id, group_events in campaign_groups.items():
            ...
            campaign_type, threat_actor, confidence = await self._classify_campaign(
                group_events
            )
            ...
            if confidence >= self.correlation_threshold:
                ...
                logger.info(
                    f"Detected new campaign: {campaign_id} ({campaign_type.value})"
                )

        return new_campaigns

    async def _update_existing_campaigns(
        self, events: List[CampaignEvent]
    ) -> List[AttackCampaign]:
        ...
        for event in events:
            ...
            # Check by IP
            ip_campaigns = self.campaign_db.find_campaigns_by_ip(
                event.auth_context.client_ip
            )
            matching_campaigns.extend(ip_campaigns)

            # Check by user
            user_campaigns = self.campaign_db.find_campaigns_by_user(
                event.auth_context.email
            )
    ...
    def _group_events_by_similarity(
        self, events: List[CampaignEvent]
    ...
    def _group_events_by_heuristics(
        self, events: List[CampaignEvent]
    ) -> Dict[str, List[CampaignEvent]]:
    ...
    async def _classify_campaign(
        self, events: List[CampaignEvent]
    ) -> Tuple[CampaignType, Optional[ThreatActor], float]:
    ...
    def _calculate_signature_match(
        self, events: List[CampaignEvent], signature: AttackSignature
    ) -> float:
    ...
    def _check_indicator_match(
        self, events: List[CampaignEvent], indicator: str
    ) -> bool:
        ...
        if indicator == "rapid_attempts":
            ...
            for i in range(1, len(timestamps)):
                ...
                ).total_seconds() < 60:  # Within 1 minute
                    return True
            ...
            return any(
                "password" in event.auth_context.password_hash.lower()
                for event in events
            )
            ...
            return any(
                event.auth_context.geolocation
                and not event.auth_context.geolocation.is_usual_location
                for event in events
            )
            ...
            return any(
                event.auth_context.is_tor_exit_node or event.auth_context.is_vpn
                for event in events
            )
            ...
            return (
                len(set(user_agents)) / len(user_agents) < 0.5
            )  # More than 50% similarity
    ...
    def _calculate_event_campaign_similarity(
        self, event: CampaignEvent, campaign: AttackCampaign
    ) -> float:
    ...
    def _calculate_campaign_similarity(
        self, campaign1: AttackCampaign, campaign2: AttackCampaign
    ) -> float:
        ...
        if (
            campaign1.threat_actor
            and campaign2.threat_actor
            and campaign1.threat_actor == campaign2.threat_actor
        ):
            score += 0.1
    ...
    async def _generate_threat_intelligence(
        ...
        try:
            from ai_karen_engine.security.threat_intelligence import (
                ReputationLevel,
                ThreatIndicator,
                ThreatIndicatorType,
                ThreatSource,
            ...
            logger.warning(
                "Could not import threat intelligence classes, returning empty list"
            )
            return []
        ...
        for campaign in campaigns:
            ...
            for ip in campaign.source_ips:
                if len(campaign.events) >= 5:  # Only for significant campaigns
                    ...
                    if campaign.campaign_type in [
                        CampaignType.APT_CAMPAIGN,
                        CampaignType.BOTNET_ACTIVITY,
                    ]:
            ...
            for ua in campaign.user_agents:
                if any(
                    pattern in ua.lower()
                    for pattern in ["bot", "crawler", "scanner", "tool"]
                ):
                    indicator = ThreatIndicator(
                        value=ua,
    ...
    def get_campaign_statistics(self) -> Dict[str, Any]:
        ...
        active_campaigns = sum(
            1 for campaign in campaigns if campaign.last_seen >= cutoff
        )

        # Calculate average duration
        durations = [
            (campaign.last_seen - campaign.first_seen).total_seconds()
            for campaign in campaigns
        ]
