...
try:
    ...
    from ai_karen_engine.services.distilbert_service import (  # type: ignore[import-not-found]
        DistilBertService,
    )
except ImportError:
    # Fallback imports for testing
    from distilbert_service import DistilBertService  # type: ignore[import-not-found]

    from models import AuthContext, EmbeddingAnalysis  # type: ignore[attr-defined]
...
class BehavioralEmbeddingConfig:
    ...
    cluster_count: int = 5
    profile_storage_path: Optional[str] = "data/user_profiles.json"
...
class BehavioralEmbeddingService:
    ...
    def __init__(
        ...
        try:
            if self.attack_db_client is not None:
                self.attack_pattern_embeddings = (
                    self.attack_db_client.get_attack_embeddings()
                )
        ...
        # User behavioral profiles storage (in-memory with optional persistence)
        self.profile_storage_path = self.config.profile_storage_path
        ...
        self.embedding_cache: TTLCache[str, BehavioralEmbeddingResult] = TTLCache(
            maxsize=self.config.cache_size, ttl=self.config.cache_ttl
        )
        self.similarity_cache: TTLCache[str, float] = TTLCache(
            maxsize=self.config.cache_size // 2, ttl=self.config.cache_ttl // 2
        )
    ...
    def _load_user_profiles(self) -> None:
        """Load user behavioral profiles from persistent storage if available."""
            ...
            if isinstance(data, dict):
                with self.profiles_lock:
                    for user_id, profile_data in data.items():
                        self.user_profiles[user_id] = UserBehavioralProfile.from_dict(
                            profile_data
                        )
            self.logger.info(
                "Loaded %d user behavioral profiles", len(self.user_profiles)
            )
        except FileNotFoundError:
            self.logger.info(
                "Profile storage file not found at %s; starting fresh",
                self.profile_storage_path,
            )
        except Exception as e:
            self.logger.error(f"Failed to load user profiles: {e}")
            self.profile_storage_path = None

    def _save_user_profiles(self) -> None:
        """Persist user behavioral profiles to storage if enabled."""
        if not self.profile_storage_path:
            return
        try:
            with self.profiles_lock:
            ...
            with open(self.profile_storage_path, "w", encoding="utf-8") as f:
                json.dump(data, f, indent=2)
        except Exception as e:
            self.logger.error(f"Failed to persist user profiles: {e}")
            self.profile_storage_path = None

    def _load_attack_pattern_embeddings(
        self, path: str
        ...
        try:
            ...
            self.logger.error(
                f"Failed to load attack pattern embeddings from {path}: {e}"
            )
            return []
            ...
            features.update(
                {
                    "time_since_last_login_hours": (
                        ...
                        if auth_context.time_since_last_login
                        else 0
                    ),
                    "is_business_hours": 9 <= auth_context.timestamp.hour <= 17,
                    "is_night_time": auth_context.timestamp.hour < 6
                    or auth_context.timestamp.hour > 22,
                }
            )
            ...
            features.update(
                {
                    ...
                    "latitude_zone": int(auth_context.geolocation.latitude / 10)
                    * 10,  # Rough zone
    ...
    def _create_context_text(
        self, auth_context: AuthContext, context_features: Dict[str, Any]
    ) -> str:
        ...
        email_domain = (
            auth_context.email.split("@")[-1]
            if "@" in auth_context.email
            else "unknown"
        )
        ...
        if self.config.enable_device_features:
            text_parts.append(
                f"ua_hash:{context_features.get('user_agent_hash', 'unknown')}"
            )
        ...
        threat_level = (
            "low"
            if auth_context.threat_intel_score < 0.3
            else "medium"
        ...
        if auth_context.previous_failed_attempts > 0:
            text_parts.append(
                f"failed_attempts:{min(auth_context.previous_failed_attempts, 5)}"
            )
    ...
    async def _calculate_similarity_scores(
        self, user_email: str, embedding_vector: List[float]
    ) -> Dict[str, float]:
        ...
        try:
            ...
            for typical_embedding in user_profile.typical_embeddings:
                similarity = self._cosine_similarity(
                    embedding_vector, typical_embedding
                )
                ...
                similarity_scores.update(
                    {
                        ...
                        "avg_similarity_to_profile": sum(similarities)
                        / len(similarities),
    ...
    async def update_user_behavioral_profile(
        ...
        with self.profiles_lock:
            ...
            if user_email not in self.user_profiles:
                self.user_profiles[user_email] = UserBehavioralProfile(
                    user_id=user_email
                )
            ...
            if len(profile.typical_embeddings) > self.config.max_profile_embeddings:
                ...
                profile.typical_embeddings = profile.typical_embeddings[
                    -self.config.max_profile_embeddings :
                ]
    ...
    async def analyze_embedding_for_anomalies(
        self, auth_context: AuthContext, embedding_result: BehavioralEmbeddingResult
    ) -> EmbeddingAnalysis:
        ...
        try:
            ...
            similarity_to_user_profile = similarity_scores.get(
                "max_similarity_to_profile", 0.0
            )
            ...
            if self.attack_pattern_embeddings:
                attack_similarities = [
                    self._cosine_similarity(embedding_result.embedding_vector, emb)
            ...
            if (
                user_profile
                and len(user_profile.typical_embeddings) >= self.config.cluster_count
            ):
                try:
    ...
    async def _generate_fallback_embedding(
        self, auth_context: AuthContext, start_time: float
    ) -> BehavioralEmbeddingResult:
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "distilbert_model": distilbert_health.model_name
            if hasattr(distilbert_health, "model_name")
            else "unknown",
        }
    ...
    async def batch_generate_embeddings(
        ...
        for auth_context in auth_contexts:
            try:
                ...
                self.logger.error(
                    f"Failed to generate embedding for context {auth_context.request_id}: {e}"
                )
                # Add fallback result
                fallback_result = await self._generate_fallback_embedding(
                    auth_context, time.time()
                )
