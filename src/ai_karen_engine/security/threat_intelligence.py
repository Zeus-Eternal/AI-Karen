...
from ai_karen_engine.security.campaign_analyzer import (
...
class ThreatFeedManager:
    ...
    def _check_rate_limit(self, service: str, limit: int, window: int) -> bool:
        ...
        service_requests[:] = [
            req_time for req_time in service_requests if now - req_time < window
        ]
...
class ThreatIndicatorDatabase:
    ...
    def get_indicator(
        self, value: str, indicator_type: ThreatIndicatorType
...
class ThreatIntelligenceEngine:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.indicator_db = ThreatIndicatorDatabase(
            config.get(
                "persistence_file", "data/intelligent_auth/threat_indicators.json"
            )
        )
    ...
    async def analyze_ip_reputation(self, ip: str) -> IPReputationResult:
        ...
        result = IPReputationResult(
            ...
            first_seen=min(match.first_seen for match in local_matches)
            if local_matches
            else None,
            last_seen=max(match.last_seen for match in local_matches)
            if local_matches
            else None,
    ...
    async def get_threat_context(self, context: AuthContext) -> ThreatContext:
        ...
        if context.user_agent:
            ...
            ua_indicator = self.indicator_db.get_indicator(
                context.user_agent.lower(), ThreatIndicatorType.USER_AGENT
            )
            if ua_indicator:
        ...
        email_indicator = self.indicator_db.get_indicator(
            context.email, ThreatIndicatorType.EMAIL
        )
        if email_indicator:
        ...
        threat_categories = self._determine_threat_categories(
            ip_reputation, threat_indicators
        )
        ...
        campaign_id = await self.detect_attack_campaign_correlation(
            context,
            ThreatAnalysis(
    ...
    def _calculate_threat_risk_score(
        self, ip_reputation: IPReputationResult, indicators: List[ThreatIndicator]
    ) -> float:
    ...
    def _determine_threat_categories(
        self, ip_reputation: IPReputationResult, indicators: List[ThreatIndicator]
    ) -> List[str]:
    ...
    async def analyze_attack_campaigns(
        ...
        return await self.campaign_analyzer.analyze_attack_campaigns(
            auth_attempts, time_window_hours
        )

    async def detect_attack_campaign_correlation(
        self, context: AuthContext, threat_analysis: ThreatAnalysis
    ) -> Optional[str]:
        """Detect if authentication attempt is part of an existing campaign."""
        # Check if this attempt matches any existing campaigns
        recent_campaigns = self.campaign_analyzer.campaign_db.find_recent_campaigns(
            hours=72
        )

        # Check by IP
        ip_campaigns = self.campaign_analyzer.campaign_db.find_campaigns_by_ip(
            context.client_ip
        )
        matching_campaigns = [c for c in ip_campaigns if c in recent_campaigns]

        # Check by user
        user_campaigns = self.campaign_analyzer.campaign_db.find_campaigns_by_user(
            context.email
        )
        matching_campaigns.extend(
            [
                c
                for c in user_campaigns
                if c in recent_campaigns and c not in matching_campaigns
            ]
        )
    ...
    def update_threat_intelligence(self, new_indicators: List[ThreatIndicator]) -> None:
        ...
        logger.info(
            f"Updated threat intelligence with {len(new_indicators)} new indicators"
        )
