...
import { SearchModeId, WebSearchOptions, WebSearchState, WebSearchResponse } from '../types';
...
export function useWebSearch() {
  ...
  const setMode = useCallback((mode: SearchModeId) => {
    // Reset options when mode changes to prevent stale config bleeding
    setState(prev => ({ ...prev, mode, options: {}, error: null }));
  }, []);

  const updateOptions = useCallback((newOptions: Partial<WebSearchOptions>) => {
  ...
  const executeSearch = useCallback(async () => {
    ...
    try {
      ...
      const response = await webSearchApi.executeSearch(payload);
