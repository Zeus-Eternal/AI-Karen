...
export default function MemoryObservabilityDashboard({ events, summary }: { events: MemoryObservabilityEvent[]; summary: MemoryObservabilitySummary }) {
  return <div className='space-y-4'><MemoryCircuitBreakerPanel state={summary.circuit_breaker_state} /><MemoryLatencyChart latencies={summary.store_latencies_ms} /><MemoryEventTable events={events} /></div>;
}
