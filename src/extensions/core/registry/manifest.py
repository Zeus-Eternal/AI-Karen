...
class ExtensionManifest(BaseModel):
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ExtensionManifest":
        ...
        if "plugin_api_version" in data or "entry_point" in data or "module" in data:
            normalized = {
                "name": data["name"],
                "version": data["version"],
                "display_name": data.get("display_name", data["name"].replace("-", " ").title()),
                "description": data.get("description", ""),
                "author": data.get("author", "Unknown"),
                "license": data.get("license", "unknown"),
                "category": data.get("category", data.get("plugin_type", "general")),
                "tags": data.get("tags", []),
                "api_version": data.get("api_version", data.get("plugin_api_version", "1.0")),
                "capabilities": {
                ...
                "trusted_ui": data.get("trusted_ui", False),
                "enable_external_workflow": data.get("enable_external_workflow", False),
            }
