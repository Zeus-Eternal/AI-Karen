...
class TestCompletePipeline(unittest.TestCase):
    ...
    def test_complete_integration_pipeline(self):
        ...
        try:
            ...
            print(
                f"   Response formatting available: {health.get('response_formatting', {}).get('available', False)}"
            )
            ...
            for formatter in formatters:
                print(
                    f"   - {formatter.get('name', 'unknown')}: {formatter.get('version', 'unknown')}"
                )
...
Difficulty: Easy""",
            ...
            for i, test_case in enumerate(test_cases, 1):
                ...
                result = orchestrator._apply_response_formatting(
                    test_case["prompt"], test_case["response"], {}
                )
                end_time = time.time()
            ...
            print(
                f"   - Total attempts: {orchestrator_metrics.get('total_attempts', 0)}"
            )
            print(
                f"   - Successful formats: {orchestrator_metrics.get('successful_formats', 0)}"
            )
            print(
                f"   - Success rate: {orchestrator_metrics.get('success_rate', 0):.2%}"
            )

            print(f"   Integration metrics:")
            print(
                f"   - Total requests: {integration_metrics.get('total_requests', 0)}"
            )
            print(
                f"   - Successful formats: {integration_metrics.get('successful_formats', 0)}"
            )
            ...
            print(
                f"   Available formatters: {len(detailed_stats.get('available_formatters', []))}"
            )
            print(
                f"   Supported content types: {len(detailed_stats.get('supported_content_types', []))}"
            )

            if detailed_stats.get("integration_level", {}).get(
                "content_type_detections"
            ):
                print("   Content type distribution:")
                for content_type, count in detailed_stats["integration_level"][
                    "content_type_detections"
                ].items():
    ...
    def test_prometheus_metrics_if_available(self):
        ...
        try:
            ...
            orchestrator._apply_response_formatting(
                "test prometheus", "test response for prometheus metrics", {}
            )
