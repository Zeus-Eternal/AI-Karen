...
class TestContentTypeDetector:
    ...
    async def test_detect_with_nlp_analysis(self):
        ...
        mock_spacy_service.get_linguistic_features = AsyncMock(
            return_value={"keywords": ["movie", "film", "director"]}
        )
    ...
    async def test_detect_with_nlp_unavailable(self):
        ...
        with patch(
            "content_detector.nlp_service_manager",
            side_effect=ImportError("NLP not available"),
        ):
    ...
    async def test_detect_error_handling(self):
        ...
        with patch.object(
            self.detector, "_get_nlp_analysis", side_effect=Exception("NLP error")
        ):
            query = "test query"
    ...
    def test_calculate_content_score(self):
        ...
        score = self.detector._calculate_content_score(
            text, patterns, entities, keywords
        )
    ...
    def test_generate_reasoning(self):
        ...
        reasoning = self.detector._generate_reasoning(
            content_type, text, entities, keywords
        )
