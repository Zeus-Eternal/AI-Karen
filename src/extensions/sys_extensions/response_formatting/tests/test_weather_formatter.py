...
from ..formatters.weather_formatter import (
...
class TestWeatherResponseFormatter(unittest.TestCase):
    ...
    def test_confidence_score_calculation(self):
        ...
        high_confidence_content = (
            "Weather in Boston: 72°F, sunny, humidity 45%, wind 10 mph"
        )
        score = self.formatter.get_confidence_score(
            high_confidence_content, self.mock_context
        )
        self.assertGreater(
            score, 0.7, "Should have high confidence for clear weather content"
        )

        # Medium confidence weather content
        medium_confidence_content = "It's sunny and warm today"
        score = self.formatter.get_confidence_score(
            medium_confidence_content, self.mock_context
        )
        ...
        score = self.formatter.get_confidence_score(
            low_confidence_content, self.mock_context
        )
        self.assertEqual(
            score, 0.0, "Should have zero confidence for non-weather content"
        )
    ...
    def test_extract_weather_info_alerts(self):
        ...
        self.assertTrue(
            any("warning" in alert_type.lower() for alert_type in alert_types)
        )
    ...
    def test_format_response_with_forecast(self):
        ...
        self.assertTrue(
            result.has_interactive_elements
        )  # Forecast has interactive elements
    ...
    def test_uv_level_calculation(self):
        ...
        for uv_index, expected_level in test_cases:
            ...
            self.assertEqual(
                level, expected_level, f"UV index {uv_index} should be {expected_level}"
            )
    ...
    def test_performance_with_long_content(self):
        ...
        long_content = (
            "Weather forecast: " + "Today is sunny. " * 100 + "Temperature is 75°F."
        )
