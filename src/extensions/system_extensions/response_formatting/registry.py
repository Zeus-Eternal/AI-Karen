...
class ResponseFormatterRegistry:
    ...
    def register_formatter(self, formatter: ResponseFormatter) -> None:
        ...
        if not isinstance(formatter, ResponseFormatter):
            raise ValueError(
                f"Formatter must be an instance of ResponseFormatter, got {type(formatter)}"
            )

        with self._lock:
            if formatter.name in self._formatters:
                logger.warning(
                    f"Formatter '{formatter.name}' is already registered, replacing"
                )
            ...
            for content_type in formatter.get_supported_content_types():
                ...
                self._formatters_by_type[content_type] = [
                    f
                    for f in self._formatters_by_type[content_type]
    ...
    def get_formatters_for_content_type(
    ...
    def find_best_formatter(
        ...
        with self._lock:
            ...
            if context.detected_content_type:
                candidates = self.get_formatters_for_content_type(
                    context.detected_content_type
                )
            else:
            ...
            logger.debug(
                f"Selected formatter: {best_formatter.name} (score: {best_score})"
            )
    ...
    def load_formatter_from_module(self, module_path: str, class_name: str) -> bool:
        ...
        try:
            ...
            if not inspect.isclass(formatter_class) or not issubclass(
                formatter_class, ResponseFormatter
            ):
            ...
            logger.error(
                f"Failed to load formatter {class_name} from {module_path}: {e}"
            )
            return False
    ...
    def validate_formatter(self, formatter: ResponseFormatter) -> List[str]:
        ...
        try:
            ...
            if not isinstance(content_types, list):
                ...
                errors.append(
                    "get_supported_content_types() must return ContentType enums"
                )
        except Exception as e:
        ...
        with self._lock:
            return {
                ...
                "supported_content_types": [
                    ct.value for ct in self.get_supported_content_types()
                ],
            }
