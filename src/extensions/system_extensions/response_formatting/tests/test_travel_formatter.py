...
from ..formatters.travel_formatter import (
...
class TestTravelResponseFormatter(unittest.TestCase):
    ...
    def test_confidence_score_calculation(self):
        ...
        high_confidence_content = (
            "Travel to Paris, France. Day 1: Visit Eiffel Tower. Hotel recommendations."
        )
        self.context.response_content = high_confidence_content
        score = self.formatter.get_confidence_score(
            high_confidence_content, self.context
        )
        self.assertGreater(
            score, 0.7, "Should have high confidence for explicit travel content"
        )

        # Medium confidence - some travel keywords
        medium_confidence_content = (
            "Planning a vacation to see some tourist attractions"
        )
        self.context.response_content = medium_confidence_content
        score = self.formatter.get_confidence_score(
            medium_confidence_content, self.context
        )
        self.assertGreater(
            score, 0.3, "Should have medium confidence for travel keywords"
        )
        ...
        score = self.formatter.get_confidence_score(
            low_confidence_content, self.context
        )
        self.assertEqual(
            score, 0.0, "Should have zero confidence for non-travel content"
        )
    ...
    def test_extract_itinerary_information(self):
        ...
        self.assertTrue(
            any("Eiffel Tower" in activity.name for activity in day1_activities)
        )
