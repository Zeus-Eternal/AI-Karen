...
class TestResponseFormattingIntegration(unittest.TestCase):
    ...
    def test_content_type_detection(self):
        ...
        async def run_test():
            ...
            self.assertIn(
                movie_result.content_type, [ContentType.MOVIE, ContentType.DEFAULT]
            )
            ...
            self.assertIn(
                code_result.content_type, [ContentType.CODE, ContentType.DEFAULT]
            )
...
class TestLLMOrchestratorIntegration(unittest.TestCase):
    ...
    def test_formatting_metrics_tracking(self):
        ...
        try:
            ...
            orchestrator._apply_response_formatting("test prompt", "test response", {})

            # Check metrics were updated
            updated_metrics = orchestrator.get_formatting_metrics()
            self.assertGreaterEqual(
                updated_metrics["total_attempts"], initial_metrics["total_attempts"]
            )
        ...
        try:
            ...
            for test_case in test_cases:
                with self.subTest(expected_type=test_case["expected_type"]):
                    ...
                    self.assertTrue(
                        result == test_case["response"]  # No formatting applied
                        or result != test_case["response"]  # Formatting applied
                    )
