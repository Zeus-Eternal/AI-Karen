...
class CodeResponseFormatter(ResponseFormatter):
    ...
    def __init__(self):
        ...
        self._language_patterns = {
            ...
            "json": [r"^\s*{", r"^\s*\[", r'"\w+"\s*:', r"\.json\b"],
            "yaml": [r"^\w+\s*:", r"^\s*-\s+\w+", r"\.yaml\b", r"\.yml\b"],
            "xml": [r"<\?xml", r"<\w+>.*</\w+>", r"\.xml\b"],
        }
    ...
    def get_confidence_score(self, content: str, context: ResponseContext) -> float:
        ...
        keyword_matches = sum(
            1 for keyword in code_keywords if keyword in content_lower
        )
        ...
        if not code_blocks:
            ...
            for code in inline_matches:
                if len(code.strip()) > 15:  # Only longer inline code
                    ...
                    code_blocks.append(
                        CodeBlock(
                            language=language, code=code.strip(), line_numbers=False
                        )
                    )
    ...
    def _extract_plain_text_code(self, content: str) -> List[CodeBlock]:
        ...
        if current_block and len(current_block) >= 2:
            ...
            code_blocks.append(
                CodeBlock(
                    language=language, code=code, line_numbers=len(current_block) > 3
                )
            )
    ...
    def _detect_language_from_code(self, code: str) -> str:
        ...
        for lang, patterns in self._language_patterns.items():
            score = sum(
                1 for pattern in patterns if re.search(pattern, code, re.IGNORECASE)
            )
            if score > 0:
    ...
    def _extract_steps(self, content: str) -> List[str]:
        ...
        for pattern in self._step_patterns:
            ...
            for match in matches:
                step = match.strip()
                if (
                    len(step) > 10 and step not in steps
                ):  # Avoid duplicates and short steps
                    steps.append(step)
    ...
    def _generate_code_html(self, code_info: CodeInfo, context: ResponseContext) -> str:
        ...
        html_parts.append(
            f'<h2 class="code-title">{self._escape_html(code_info.title)}</h2>'
        )
        ...
        if code_info.tags:
            ...
            for tag in code_info.tags:
                html_parts.append(
                    f'<span class="code-tag">{self._escape_html(tag)}</span>'
                )
        ...
        if code_info.steps:
            ...
            for step in code_info.steps:
                html_parts.append(
                    f'<li class="step-item">{self._escape_html(step)}</li>'
                )
        ...
        if code_info.language:
            html_parts.append(
                f'<span class="metadata-item">Language: <strong>{code_info.language}</strong></span>'
            )
        if code_info.complexity:
            html_parts.append(
                f'<span class="metadata-item">Complexity: <strong>{code_info.complexity}</strong></span>'
            )
        html_parts.append("</div>")
    ...
    def _generate_code_block_html(
        self, block: CodeBlock, index: int, theme_name: str
    ) -> str:
        """
        ...
        if block.filename:
            html_parts.append(
                f'<span class="code-filename">{self._escape_html(block.filename)}</span>'
            )
        html_parts.append(
            f'<Button class="copy-button" onclick="copyCodeToClipboard(this, {index})">Copy</Button>'
        )
        html_parts.append("</div>")
        ...
        if block.line_numbers:
            ...
            html_parts.append(
                f'<pre class="code-pre"><code class="language-{block.language}">{self._escape_html(block.code)}</code></pre>'
            )
        html_parts.append("</div>")
    ...
    def _get_css_classes(self, context: ResponseContext) -> List[str]:
        ...
        Returns:
            List of CSS classes
        """
        base_classes = ["response-formatted", "code-response", "themed-content"]
