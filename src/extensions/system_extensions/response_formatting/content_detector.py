...
class ContentTypeDetector:
    ...
    async def detect_content_type(
        ...
        try:
            ...
            if best_score < 0.1:  # Lowered threshold for better detection
    ...
    async def _get_nlp_analysis(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            entities = (
                [ent.label_ for ent in parsed_message.entities]
    ...
    def _calculate_content_score(
        ...
        if entities and patterns.get("entities"):
            entity_matches = sum(
                1 for entity in entities if entity in patterns["entities"]
            )
        ...
        if keywords:
            nlp_keyword_matches = sum(
                1 for kw in keywords if kw.lower() in patterns["keywords"]
            )
    ...
    def _generate_reasoning(
        ...
        if content_type in self._content_patterns:
            matched_keywords = [
                kw
                for kw in self._content_patterns[content_type]["keywords"]
                if kw in text.lower()
            ]
            if matched_keywords:
