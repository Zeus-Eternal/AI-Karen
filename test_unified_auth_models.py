...
def test_session_data_model():
    ...
    assert "suspicious_location" in session.security_flags, "Security flag should be added"
    
    session.remove_security_flag("suspicious_location")
    assert "suspicious_location" not in session.security_flags, "Security flag should be removed"
    ...
    assert session.invalidation_reason == "manual_logout", "Invalidation reason should be set"
    
    # Test serialization
    session_dict = session.to_dict()
    assert isinstance(session_dict, dict), "to_dict should return dictionary"
    assert session_dict["session_token"] == "session123", "Serialized session_token should match"
    
    # Test deserialization
    session2 = SessionData.from_dict(session_dict)
    assert session2.session_token == session.session_token, "Deserialized session should match original"
    assert session2.user_data.user_id == session.user_data.user_id, "Deserialized user data should match"
...
def test_auth_event_model():
    ...
    assert event_dict["event_type"] == "login_success", "Serialized event_type should match"
    
    # Test deserialization
    event2 = AuthEvent.from_dict(event_dict)
    assert event2.event_type == event.event_type, "Deserialized event should match original"
...
def test_auth_config():
    ...
    config = AuthConfig()
    assert isinstance(config.database, DatabaseConfig), "Database config should be DatabaseConfig instance"
    assert isinstance(config.jwt, JWTConfig), "JWT config should be JWTConfig instance"
    assert isinstance(config.session, SessionConfig), "Session config should be SessionConfig instance"
    assert isinstance(config.security, SecurityConfig), "Security config should be SecurityConfig instance"
    assert isinstance(config.intelligence, IntelligenceConfig), "Intelligence config should be IntelligenceConfig instance"
    assert isinstance(config.features, FeatureToggles), "Features config should be FeatureToggles instance"
    ...
    assert config_from_env.jwt.secret_key == "test-secret-key", "Secret key should be loaded from env"
    assert config_from_env.jwt.access_token_expire_minutes == 30, "Access token expiry should be loaded from env"
    assert config_from_env.features.use_database, "Database feature should be enabled from env"
    assert config_from_env.features.enable_rate_limiting, "Rate limiting should be enabled from env"
    ...
    assert config_from_dict.jwt.secret_key == "dict-secret", "Secret key should be loaded from dict"
    assert config_from_dict.jwt.access_token_expire_minutes == 45, "Access token expiry should be loaded from dict"
    assert config_from_dict.security.max_failed_attempts == 3, "Max failed attempts should be loaded from dict"
    assert config_from_dict.features.enable_intelligent_auth, "Intelligent auth should be enabled from dict"
    ...
    try:
        ...
        assert config_from_json.jwt.secret_key == "dict-secret", "Config should be loaded from JSON file"
    finally:
        os.unlink(json_file)
    ...
    assert "Consolidated auth service" in mode_desc, "Mode description should mention consolidated service"
    
    # Clean up environment variables
    for key in ["AUTH_SECRET_KEY", "AUTH_ACCESS_TOKEN_EXPIRE_MINUTES", "AUTH_USE_DATABASE", "AUTH_ENABLE_RATE_LIMITING"]:
...
def test_unified_exceptions():
    ...
    assert str(error) == "InvalidCredentialsError: Invalid email or password", "Exception string representation should be correct"
    assert error.details["email"] == "test@example.com", "Exception details should be set"
    assert error.user_message == "Invalid email or password", "User message should be set"
    
    # Test exception serialization
    error_dict = error.to_dict()
    assert isinstance(error_dict, dict), "Exception to_dict should return dictionary"
    assert error_dict["error_type"] == "InvalidCredentialsError", "Error type should be correct"
    assert error_dict["message"] == "Invalid email or password", "Message should be correct"
    ...
    for exception, expected_status in exceptions_to_test:
        assert is_user_error(exception), f"{exception.__class__.__name__} should be a user error"
        status_code = get_http_status_code(exception)
        assert status_code == expected_status, f"{exception.__class__.__name__} should return status {expected_status}, got {status_code}"
...
def test_integration():
    ...
    assert session2.session_token == session.session_token, "Session round-trip should preserve data"
    assert event2.event_type == event.event_type, "Event round-trip should preserve data"
    assert config2.jwt.secret_key == config.jwt.secret_key, "Config round-trip should preserve data"
