...
def test_auth_middleware_error_handling_consistency():
    ...
    try:
        ...
        assert 'JSONResponse({"detail": "Rate limit exceeded"}, status_code=429)' in content
        assert 'JSONResponse({"detail": "Authentication failed"}, status_code=401)' in content
...
def test_auth_models_consistency():
    ...
    try:
        from ai_karen_engine.auth.models import UserData, SessionData, AuthEvent
