...
class DatabaseSessionBackend:
    ...
    def _row_to_session_data(self, row) -> SessionData:
        ...
        user_data = UserData(
            ...
            else (json.loads(row[15]) if row[15] else []),  # u.roles
            tenant_id=str(row[16]),  # u.tenant_id
            preferences=row[17]
            if isinstance(row[17], dict)
            else (json.loads(row[17]) if row[17] else {}),  # u.preferences
            is_verified=bool(row[18]),  # u.is_verified
            is_active=bool(row[19]),  # u.is_active
            created_at=row[20]
            if isinstance(row[20], datetime)
            else datetime.fromisoformat(str(row[20])),  # u.created_at
            updated_at=row[21]
            if isinstance(row[21], datetime)
            else datetime.fromisoformat(str(row[21])),  # u.updated_at
            last_login_at=row[22]
            if isinstance(row[22], datetime)
            else (
                datetime.fromisoformat(str(row[22])) if row[22] else None
            ),  # u.last_login_at
            failed_login_attempts=row[23] or 0,  # u.failed_login_attempts
            locked_until=row[24]
            if isinstance(row[24], datetime)
            else (
        ...
        return SessionData(
            ...
            else datetime.fromisoformat(str(row[4])),  # s.created_at
            last_accessed=row[5]
            if isinstance(row[5], datetime)
            else datetime.fromisoformat(str(row[5])),  # s.last_accessed
            ...
            else (json.loads(row[9]) if row[9] else None),  # s.geolocation
            risk_score=row[10] or 0.0,  # s.risk_score
            security_flags=row[11]
            if isinstance(row[11], list)
            else (json.loads(row[11]) if row[11] else []),  # s.security_flags
        )
