...
class SecurityEventCorrelator:
    ...
    def __init__(self, config: AuthConfig):
        ...
        self._failed_attempts_by_ip: Dict[str, deque] = defaultdict(
            lambda: deque(maxlen=100)
        )
        self._failed_attempts_by_user: Dict[str, deque] = defaultdict(
            lambda: deque(maxlen=100)
        )
        ...
        self.credential_stuffing_threshold = (
            5  # Failed attempts across multiple users from same IP
        )
    ...
    async def _detect_brute_force(self, event: AuthEvent) -> Optional[SecurityPattern]:
        ...
        recent_count = sum(
            1 for attempt in recent_failures if attempt.timestamp > cutoff_time
        )
        ...
        if recent_count >= self.brute_force_threshold:
            ...
            for pattern in self._active_patterns.values():
                if (
                    pattern.pattern_type == "brute_force"
                    and event.ip_address in pattern.source_ips
                ):
    ...
    async def _detect_credential_stuffing(
        self, event: AuthEvent
    ) -> Optional[SecurityPattern]:
        ...
        if len(targeted_users) >= self.credential_stuffing_threshold:
            ...
            for pattern in self._active_patterns.values():
                if (
                    pattern.pattern_type == "credential_stuffing"
                    and event.ip_address in pattern.source_ips
                ):
                    existing_pattern = pattern
                    break
            ...
            if existing_pattern:
                ...
                return SecurityPattern(
                    ...
                    confidence=min(
                        1.0,
                        len(targeted_users) / (self.credential_stuffing_threshold * 2),
                    ),
    ...
    async def _detect_anomalous_behavior(
        ...
        for pattern in self._active_patterns.values():
            if (
                pattern.pattern_type == "anomalous_behavior"
                and pattern_key in pattern.details.get("pattern_keys", [])
            ):
                existing_pattern = pattern
                break
        ...
        if existing_pattern:
            ...
            existing_pattern.confidence = max(
                existing_pattern.confidence, event.risk_score
            )
            if event.user_id:
...
class PerformanceTrendAnalyzer:
    ...
    async def _analyze_metric_trend(
        self, metric_name: str, period_minutes: int
        ...
        comparison_cutoff = datetime.now(timezone.utc) - timedelta(
            minutes=period_minutes // 2
        )
        comparison_points = [
            (ts, val) for ts, val in recent_points if ts <= comparison_cutoff
        ]
        ...
        if abs_change < self.significant_change_threshold * 100:
            ...
            trend_direction = (
                "improving" if "success" in metric_name.lower() else "degrading"
            )
            trend_strength = min(1.0, abs_change / 50.0)  # Normalize to 0-1
        else:
            trend_direction = (
                "degrading" if "success" in metric_name.lower() else "improving"
            )
    ...
    def get_current_trends(
        ...
        if metric_name:
            return [
                trend for key, trend in self._trend_cache.items() if metric_name in key
            ]
...
class EnhancedAuthMonitor:
    ...
    async def analyze_auth_event(self, event: AuthEvent) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis_results["security_patterns"] = [
                p.to_dict() for p in security_patterns
            ]
            ...
            await self.performance_analyzer.record_metric_point(
                success_metric, 1.0 if event.success else 0.0, event.timestamp
            )
    ...
    def _generate_recommendations(
        self, event: AuthEvent, security_patterns: List[SecurityPattern]
    ) -> List[str]:
    ...
    def get_comprehensive_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_security_patterns": len(
                self.security_correlator.get_active_patterns()
            ),
