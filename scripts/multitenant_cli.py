...
def delete_tenant(args):
    ...
    if not args.force:
        confirm = input("This will permanently delete all tenant data. Continue? (y/N): ")
        if confirm.lower() != 'y':
...
Examples:
  ...
  %(prog)s delete-tenant --tenant-id 12345678-1234-1234-1234-123456789abc --force
    ...
    parser.add_argument(
        '--database-url',
        default=os.getenv('DATABASE_URL', 'postgresql://postgres:postgres@localhost/ai_karen'),
    ...
    create_parser.add_argument('--tenant-id', help='Tenant ID (auto-generated if not provided)')
    ...
    validate_parser = subparsers.add_parser('validate-tenant', help='Validate tenant schema')
    validate_parser.add_argument('--tenant-id', required=True, help='Tenant ID to validate')
