...
class NeuroVaultMemoryService:
    ...
    def __init__(self):
        ...
        self.signing_key = os.getenv(
            "KARI_MEMORY_SIGNING_KEY", "mirrorsnap-sovereign-key"
        )
    ...
    def _calculate_relevance(
        self, query: str, event: MemoryEvent, context: Dict[str, Any]
    ) -> float:
    ...
    def _context_similarity(
        self, event_context: Dict[str, Any], query_context: Dict[str, Any]
    ) -> float:
    ...
    def get_memory_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_recall_latency": (
                np.mean(self.metrics["recall_latency"])
                if self.metrics["recall_latency"]
                else 0
            ),
            "avg_rerank_latency": (
                np.mean(self.metrics["rerank_latency"])
                if self.metrics["rerank_latency"]
                else 0
            ),
            "memory_hit_rate": self.metrics["memory_hits"]
            / max(self.metrics["memory_hits"] + self.metrics["memory_misses"], 1),
            "plugin_success_rate": (
                self.metrics["plugin_invocations"] - self.metrics["plugin_failures"]
            )
            / max(self.metrics["plugin_invocations"], 1),
            "context_alignment_score": self.metrics["context_alignments"]
            / max(len(self.memory_events), 1),
        }

    def get_recent_plugin_activity(self, limit: int = 10) -> List[Dict[str, Any]]:
        """Get recent plugin activity for monitoring"""
        recent_events = sorted(
            self.plugin_events, key=lambda x: x.timestamp, reverse=True
        )[:limit]
        return [asdict(event) for event in recent_events]

    def get_user_memory_stats(self, user_id: str) -> Dict[str, Any]:
        """Return memory and plugin usage statistics for a specific user."""
        memories = [e for e in self.memory_events if e.user_id == user_id]
        plugins = [
            p for p in self.plugin_events if p.user_context.get("user_id") == user_id
        ]
        last_memory = max((e.timestamp for e in memories), default=None)
        last_plugin = max((p.timestamp for p in plugins), default=None)
        return {
            "memory_events": len(memories),
            "plugin_invocations": len(plugins),
            "last_memory": last_memory,
            "last_plugin": last_plugin,
        }

    def search_memories_by_context(
        self, context_filter: Dict[str, Any], limit: int = 20
    ) -> List[MemoryEvent]:
