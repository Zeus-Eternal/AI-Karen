...
class RBACService:
    ...
    def create_session(
        self, user: User, ip_address: str = "127.0.0.1", user_agent: str = "Unknown"
    ) -> UserSession:
        """Create user session"""
        session = UserSession(
    ...
    def update_user_role(
        self, user_id: str, new_role: UserRole, updated_by: User
    ) -> bool:
    ...
    def update_user_credentials(
        ...
        *,
        ...
        """Update user's login credentials and profile data."""
    ...
    def get_user_activity(self, user: User) -> Dict[str, Any]:
        """Return session and memory usage statistics for ``user``."""
        sessions = [
            s for s in self.sessions.values() if s.user_id == user.id and s.is_active
        ]
        mem_stats = memory_service.get_user_memory_stats(user.id)
        return {
        ...
        return {
            ...
            "recent_logins": len(
                [
                    ...
                    if u.last_login
                    and u.last_login > datetime.now() - timedelta(days=7)
                ]
            ),
        }
