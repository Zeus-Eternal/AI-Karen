...
def create_enhanced_pages():
    ...
    pages = {
        ...
        "Chat": {"func": render_integrated_chat, "icon": "💬", "desc": "AI conversation with memory"},
        "Memory": {"func": render_memory_explorer, "icon": "🧠", "desc": "Explore and search memories"},
        "Analytics": {"func": render_analytics_dashboard, "icon": "📊", "desc": "Usage insights and metrics"},
        "Plugins": {"func": render_plugin_manager, "icon": "🧩", "desc": "Manage and execute plugins"},
        "System Health": {"func": render_system_health, "icon": "🏥", "desc": "Monitor system status"},
        "Page Navigator": {"func": render_page_navigator, "icon": "🧭", "desc": "Navigate available pages"},
    ...
    if BACKEND_AVAILABLE:
        pages.update({
            ...
            "Presence": {"func": presence_page, "icon": "👥", "desc": "User presence and activity"}
        })
...
def render_enhanced_sidebar(current_page: str, user_ctx: Dict[str, Any]):
    ...
    from components.navigation import render_favorites_menu, render_recent_pages, render_page_search
    ...
    with st.sidebar:
        ...
        backend_status = "✅ Connected" if st.session_state.get('backend_integrated') else "⚠️ Limited"
        ...
        # Debug mode toggle
        debug_mode = st.checkbox("Debug Mode", value=st.session_state.get('show_debug_info', False))
        ...
        # Compact mode toggle
        compact_mode = st.checkbox("Compact Mode", value=st.session_state.get('compact_mode', False))
