...
export class ServerHelper {
   ...
   * Throws error if server is not available instead of falling back to mock mode
   */
  async navigateWithFallback(url: string, fallbackContent?: string): Promise<boolean> {
    ...
    if (!serverAvailable) {
      throw new Error(`Server is not available at ${this.config.baseURL}. Please ensure the production server is running before executing API validation tests.`);
    }
    ...
    try {
      ...
      throw new Error(`Failed to navigate to ${url}: ${error instanceof Error ? error.message : error}. Please ensure the production server is accessible.`);
    }
  }
  ...
  /**
   * Mock API response for testing - only used when explicitly called
   * @deprecated This method should only be used for development testing, not for production API validation
   */
  async setupMockApiResponses(): Promise<void> {
    console.warn('WARNING: setupMockApiResponses() called. This will intercept real API calls and use mock data instead.');
  ...
  /**
   * Setup for real API testing - ensures no API routes are mocked
   */
  async setupRealApiTesting(): Promise<void> {
    console.log('Setting up real API testing - no API routes will be mocked');
    
    // Ensure no API routes are intercepted by unrouting any existing API routes
    // This allows real API calls to go through to the production server
    await this.page.unroute('**/api/**');
    
    // Add error message simulation script for error handling tests
    await this.page.addInitScript(() => {
      // Store error messages globally for test access
      (window as any).__testErrors = [];
