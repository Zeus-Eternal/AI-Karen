...
// Define types locally since monitoring/types doesn't exist
...
// Create connectivity logger
const connectivityLogger = {
  logError: (message: string, error: Error, context?: string) => {
    logger.error(message, { component: 'useSystemHealth', metadata: { error: error.message, context } });
  }
};
  ...
  const checkSystemHealth = useCallback(async (): Promise<SystemHealth> => {
    try {
      ...
      const health: SystemHealth = {
        overall:
          avgResponseTime < 2000 && errorRate < 2
            ? 'healthy'
            : avgResponseTime < 5000 && errorRate < 5
            ? 'degraded'
            : 'critical',
        components: {
          backend: {
