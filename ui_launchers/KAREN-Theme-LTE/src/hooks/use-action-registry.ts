...
export function useActionRegistry(options: UseActionRegistryOptions = {}): UseActionRegistryReturn {
      ...
      if (autoToast) {
        if (failureCount === 0) {
          addToast({
            ...
            message: `Successfully completed ${successCount} actions`,
            type: 'success',
            duration: 3000
          });
          ...
          addToast({
            ...
            message: `${successCount} actions completed, ${failureCount} failed`,
            type: 'warning',
            duration: 4000
          });
        } else {
          addToast({
            ...
            message: `${failureCount} actions failed`,
            duration: 5000
          });
        }
      }
  ...
  const addEventListener = useCallback((
    ...
    registry.addEventListener(eventName, listener as (event: Event) => void);
  }, [registry]);
  ...
  const removeEventListener = useCallback((
    ...
    registry.removeEventListener(eventName, listener as (event: Event) => void);
  }, [registry]);
