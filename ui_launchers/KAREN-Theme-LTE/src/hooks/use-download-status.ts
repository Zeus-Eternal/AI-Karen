...
const handleErrorWithContext = (message: string, context: Record<string, unknown>) => {
  safeError(message, context.error || new Error(message));
};
...
export function useDownloadStatus(): DownloadStatusHookReturn {
  ...
  const getDownloadStatus = useCallback(async (taskId: string): Promise<DownloadTask | null> => {
    try {
      ...
      handleErrorWithContext(`Failed to get download status for task ${taskId}:`, {
        component: 'useDownloadStatus',
  ...
  const refreshDownloads = useCallback(async () => {
    try {
      ...
      handleErrorWithContext('Failed to refresh downloads:', {
        component: 'useDownloadStatus',
        operation: 'refreshDownloads',
        metadata: { error }
      });
    }
  ...
  const cancelDownload = useCallback(async (taskId: string) => {
    try {
      ...
      handleErrorWithContext(`Failed to cancel download ${taskId}:`, {
        component: 'useDownloadStatus',
  ...
  const pauseDownload = useCallback(async (taskId: string) => {
    try {
      await backend.makeRequestPublic(`/api/downloads/${taskId}/pause`, {
        method: 'POST'
      });
      ...
      safeError(`Failed to pause download ${taskId}:`, {
        component: 'useDownloadStatus',
  ...
  const resumeDownload = useCallback(async (taskId: string) => {
    try {
      await backend.makeRequestPublic(`/api/downloads/${taskId}/resume`, {
        method: 'POST'
      });
      ...
      safeError(`Failed to resume download ${taskId}:`, {
        component: 'useDownloadStatus',
  ...
  const retryDownload = useCallback(async (taskId: string) => {
    ...
    try {
      ...
      safeError(`Failed to retry download ${taskId}:`, {
        component: 'useDownloadStatus',
        operation: 'retryDownload',
        metadata: { error, taskId }
      });
      throw error;
    }
