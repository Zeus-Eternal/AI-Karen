...
import { AccessibilityTestSuiteImpl, type AccessibilityTestSuite, type AccessibilityReport, type AccessibilityIssue } from '../lib/accessibility/accessibility-testing';
...
export function useAccessibilityMonitor(
  ...
  const { report, runTest } = useAccessibilityTesting(elementRef, {
    ...
    onTestComplete: (report) => {
      const newViolations = report.violations.map((v: AccessibilityIssue) => v.description);
      const newWarnings = report.warnings.map((w: AccessibilityIssue) => w.description);
