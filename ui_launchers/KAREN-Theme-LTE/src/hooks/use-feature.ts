...
import { safeError, safeWarn } from '@/lib/errorHandler';
...
const resolveFeatureFlags = (): Record<FeatureFlag, boolean> => {
  ...
  try {
    ...
    if (!validation.isValid) {
      safeError('Invalid feature flag configuration:', { metadata: { errors: validation.errors } });
      // Fall back to safe defaults
      resolvedFlags = mergeWithEnvironmentDefaults({});
    } else {
      if (validation.warnings.length > 0) {
        safeWarn('Feature flag configuration warnings:', { metadata: { warnings: validation.warnings } });
      }
    ...
    return resolvedFlags || mergeWithEnvironmentDefaults({});
    
  } catch (error) {
    safeError('Failed to resolve feature flags:', {
      metadata: {
