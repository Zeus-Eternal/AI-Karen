...
import { getPerformanceConfig } from '@/lib/config';
...
export function useMonitoring() {
  ...
  const [state, setState] = useState<MonitoringState>(() => ({
    health: {
      metrics: getPerformanceConfig().enablePerformanceMonitoring ? healthMonitor.getMetrics() : null,
      alerts: getPerformanceConfig().enablePerformanceMonitoring ? healthMonitor.getAlerts(50) : [],
  ...
  useEffect(() => {
    if (!getPerformanceConfig().enablePerformanceMonitoring) {
      return;
    }
