...
const createInitialValidationState = (): FormValidationState => ({
  fields: {
    ...
    username: createInitialFieldState(),
    remember: createInitialFieldState(),
    consent: createInitialFieldState(),
  },
  isValid: false,
  hasErrors: false,
  isValidating: false,
...
export function useFormValidation(config: UseFormValidationConfig = {}): UseFormValidationReturn {
  ...
  const updateFieldState = useCallback((field: FormFieldType, updates: Partial<FieldValidationState>) => {
  ...
  const validateForm = useCallback((credentials: LoginCredentials, requireTwoFactor: boolean = false): FormValidationResult => {
