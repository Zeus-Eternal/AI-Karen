...
import type { KarenSettings as CopilotKarenSettings } from '@/services/copilotService';
...
const normalizePluginParameters = (config?: PluginManifest['config']): PluginParameterConfig[] => {
  ...
  if ('parameters' in config && Array.isArray(config.parameters)) {
    return config.parameters.map((param): PluginParameterConfig => {
      ...
      const parameter = param as PluginParameter;
...
export function useCopilot(options: UseCopilotOptions = {}): UseCopilotReturn {
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        setState((prev: CopilotState) => ({
          ...prev,
          securityContext,
          // Convert LNMInfo to ensure proper structure
          availableLNMs: availableLNMs.map((lnm: unknown) => ({
            id: (lnm as LNMInfo).id,
            name: (lnm as LNMInfo).name,
            description: (lnm as LNMInfo).description,
            capabilities: (lnm as LNMInfo).capabilities,
          ...
          availablePlugins: availablePlugins.map((plugin) => {
            return {
              ...
              riskLevel: plugin.riskLevel || 'safe',
              // Ensure config has the correct structure
              config: (plugin.config && typeof plugin.config === 'object') ? {
                parameters: normalizePluginParameters(plugin.config)
              } : undefined
            };
          })
        }));
    ...
    try {
      ...
      if (success) {
        ...
        const assistantMessage: CopilotMessage = {
          ...
          timestamp: new Date(),
          metadata: {
            workflows: [{ ...workflow, title: workflow.name }]
          }
        };
    ...
    try {
      ...
      if (artifactContent) {
        ...
        const assistantMessage: CopilotMessage = {
          ...
          timestamp: new Date(),
          metadata: {
            artifacts: [{ ...artifact, type: 'code' as const }]
          }
        };
  ...
  const getSecurityContext = useCallback(async () => {
    try {
      const context = await copilotService.getSecurityContext();
      // DEBUG: Log security mode to validate type mismatch
      console.log('DEBUG: SecurityContext from service:', context);
      console.log('DEBUG: securityMode value:', context.securityMode);
      console.log('DEBUG: securityMode type:', typeof context.securityMode);
      return context;
    } catch (err) {
  ...
  const clearErrorWithId = useCallback(() => {
    setError(null);
  }, []);
