...
export const validateFileUpload = (
  ...
  const sanitizedName = file.name
    ...
    .substring(0, 255); // Limit length to avoid long filenames
...
// Caching for frequently sanitized inputs (for performance optimization)
const sanitizedCache = new Map<string, string>();

/**
 * Cache and return sanitized input
 * @param input - The raw input string to sanitize
 * @param allowHtml - Whether to allow HTML tags
 * @returns Sanitized string from cache
 */
export const cachedSanitizeInput = (input: string, allowHtml: boolean = false): string => {
  const cacheKey = `${input}_${allowHtml}`;
  if (sanitizedCache.has(cacheKey)) {
    return sanitizedCache.get(cacheKey)!;
  }

  const sanitized = sanitizeInput(input, allowHtml);
