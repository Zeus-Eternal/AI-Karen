...
export class HotReloadAuthManager {
  ...
  private getOrCreateSessionId(): string {
    ...
    try {
      let sessionId = sessionStorage.getItem(HotReloadAuthManager.SESSION_ID_KEY);
      if (!sessionId) {
        sessionId = `hr_${Date.now()}_${Math.random().toString(36).substring(2, 15)}`;
  ...
  private isHotReloadScenario(): boolean {
    ...
    const indicators = [
      ...
      ['3000', '3001', '8000', '8001', '8010', '8020', '5173', '5174'].includes(window.location.port),
  ...
  private collectCurrentTokens(): Record<string, string> {
    ...
    try {
      ...
      const extensionToken = localStorage.getItem('karen_extension_access_token');
      if (extensionToken) {
        tokens.extension_access = extensionToken;
      }

      const extensionRefreshToken = localStorage.getItem('karen_extension_refresh_token');
  ...
  private restoreStateIfNeeded(): void {
    ...
    try {
      ...
      const isPending = sessionStorage.getItem('kari_hot_reload_pending') === 'true';
      if (!isPending) return;

      // Get preserved state
      const stateJson = sessionStorage.getItem(HotReloadAuthManager.STORAGE_KEY);
      if (!stateJson) {
      ...
      if (stateAge > HotReloadAuthManager.MAX_STATE_AGE_MS) {
        logger.debug('Preserved auth state too old, discarding', { ageMs: stateAge });
  ...
  private restoreTokens(tokens: Record<string, string>): void {
    try {
      ...
      if (tokens.extension_access) {
        localStorage.setItem('karen_extension_access_token', tokens.extension_access);
      }

      if (tokens.extension_refresh) {
        localStorage.setItem('karen_extension_refresh_token', tokens.extension_refresh);
      }
      ...
      if (tokens.development) {
        ...
        if (devAuthManager.isEnabled()) {
          ...
          logger.debug('Development tokens restoration handled by dev auth manager');
        }
      }
