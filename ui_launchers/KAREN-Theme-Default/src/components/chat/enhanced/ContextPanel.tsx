...
/**
 * ContextPanel
 * Sidebar intelligence: related threads, memory context, smart suggestions, and user patterns.
 * - Search + filter (status/complexity/type)
 * - Fast, memoized derivations
 * - Shadcn + lucide clean UI
 */
export const ContextPanel: React.FC<ContextPanelProps> = ({
  ...
  const topSuggestions = useMemo<ContextSuggestion[]>(() => {
    const msgs = conversation?.currentThread?.messages ?? [];
    return msgs
      .flatMap((m) => m.metadata?.suggestions ?? [])
          ...
          <Badge
            variant={
              thread.metadata?.complexity === "complex"
                ? "destructive"
                : thread.metadata?.complexity === "medium"
                ? "default"
                : "secondary"
            }
            className="text-xs sm:text-sm md:text-base"
          >
            {thread.metadata?.complexity ?? "simple"}
          </Badge>
        </div>
      ...
      >
        <div className="flex items-start justify-between mb-2">
          ...
          <span className="text-xs text-muted-foreground sm:text-sm md:text-base">
            {Math.round((memory.relevance ?? 0) * 100)}% relevant
          </span>
        </div>
        ...
        <div className="flex items-center justify-between text-xs text-muted-foreground sm:text-sm md:text-base">
          <span>{format(safeDate(memory.timestamp), "MMM dd, yyyy")}</span>
          {memory.source && (
            <span className="flex items-center gap-1">
  ...
  const renderSuggestionItem = useCallback(
    (suggestion: ContextSuggestion) => (
      <button
        key={suggestion.id}
        className="w-full text-left rounded-md hover:bg-muted/60 transition-colors"
        ...
        <div className="space-y-2 mt-2">
          <div className="relative">
            ...
            <Input
              ...
              className="pl-8 h-9"
              placeholder="Search threads, topics, memories…"
