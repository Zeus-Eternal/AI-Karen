// ui_launchers/KAREN-Theme-Default/src/components/chat/enhanced/ReasoningVisualization.tsx
"use client";
...
/* ---------- Types ---------- */
import type { ReasoningChain, ReasoningStep } from "@/types/enhanced-chat";
...
export const ReasoningVisualization: React.FC<ReasoningVisualizationProps> = ({
  ...
  /* ---------- Progress (simple: 100% when steps are present) ---------- */
  const overallProgress = useMemo(() => {
    const total = reasoning.steps?.length ?? 0;
    if (total === 0) return 0;
    return 100; // all steps computed & shown
  }, [reasoning.steps]);
  ...
  const handleExport = () => {
    try {
      ...
      toast({
        variant: "destructive",
        title: "Export Failed",
        description: "Unable to export reasoning chain.",
      });
    }
  };
  ...
  const copyReasoningSummary = async () => {
    const summary = reasoning.steps
      .map(
        (step, i) =>
          `${i + 1}. ${step.description} (${Math.round(step.confidence * 100)}% confidence)`
      )
      .join("\n");
        ...
        <Collapsible open={isExpanded} onOpenChange={() => toggleStep(step.id)}>
            ...
            <CardHeader className="pb-3 cursor-pointer hover:bg-muted/50 transition-colors">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-shrink-0 w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center">
                  <span className="text-sm font-medium text-primary md:text-base lg:text-lg">
                    {index + 1}
                  </span>
                </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-muted-foreground sm:text-sm md:text-base">
                      {format(toDate(step.timestamp), "HH:mm:ss")}
                    </span>
                  </div>
                </div>
          ...
          <CollapsibleContent>
            <CardContent className="pt-0">
                ...
                <div className="mb-4">
                  ...
                  <ul className="space-y-1">
                    {step.evidence.map((evidence, idx) => (
                      <li
                        key={`${step.id}-ev-${idx}`}
                        className="text-sm text-muted-foreground flex items-start gap-2 md:text-base lg:text-lg"
                      >
              ...
              <div className="mb-4">
                <h4 className="text-sm font-medium mb-2 md:text-base lg:text-lg">
                  Confidence Analysis
                </h4>
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground sm:text-sm md:text-base">
                    Confidence reflects retrieval quality, consistency, and agreement with
                    sources (if provided).
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 text-xs text-muted-foreground sm:text-sm md:text-base">
                ...
                <span>
                  Processed at {format(toDate(step.timestamp), "MMM dd, yyyy HH:mm:ss")}
                </span>
              </div>
            </CardContent>
          </CollapsibleContent>
      ...
      <TooltipProvider>
        <Tooltip>
          ...
          <TooltipContent>
            ...
            <p>
              {reasoning.steps.length} steps with {Math.round(reasoning.confidence * 100)}%
              confidence
            </p>
          </TooltipContent>
        </Tooltip>
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {getConfidenceLabel(reasoning.confidence)} (
              {Math.round(reasoning.confidence * 100)}%)
            </div>

            <Button variant="outline" size="sm" onClick={copyReasoningSummary} aria-label="Copy summary">
              <Copy className="h-4 w-4" />
            </Button>

            <Button variant="outline" size="sm" onClick={handleExport} aria-label="Export reasoning">
          ...
          <div className="space-y-4 pb-4">
            ...
            {reasoning.sources.length > 0 && (
              <div className="pt-4">
                ...
                {showSources && <div className="mt-4">{renderSources()}</div>}
              </div>
            )}
          </div>
        </ScrollArea>
