// ui_launchers/KAREN-Theme-Default/src/components/error/ChatErrorBoundary.tsx
"use client";
...
export class ChatErrorBoundary extends Component<Props, State> {
  ...
  private reportToExternalService = async (
    ...
    try {
      ...
      // Example send (disabled by default):
      ...
      // Silently ignore reporting errors to avoid loops
    }
  };
  ...
  handleRetry = () => {
    ...
    const since = Date.now() - lastErrorTime;
    if (since < minRetryDelay) {
      const t = setTimeout(() => this.handleRetry(), minRetryDelay - since);
  ...
  handleReload = () => {
    if (this.errorReportingEnabled && this.telemetryService?.track) {
      ...
      // Flush telemetry best-effort before reload
    ...
    >
      <div className="text-center space-y-4 max-w-2xl">
        ...
        {process.env.NODE_ENV === "development" && error && (
          <details className="text-left bg-red-100 dark:bg-red-900/40 p-4 rounded border">
            ...
            <div className="space-y-2">
              <div>
                ...
                <pre className="mt-1 text-sm text-red-700 dark:text-red-300 overflow-auto bg-red-50 dark:bg-red-900/20 p-2 rounded">
                  <code>
              ...
              {error.stack && (
                <div>
                  ...
                  <pre className="mt-1 text-xs text-red-700 dark:text-red-300 overflow-auto bg-red-50 dark:bg-red-900/20 p-2 rounded max-h-40">
        ...
        {errorId && (
          <div className="text-sm text-red-600 dark:text-red-400 space-y-1">
            <p>
              Error ID:{" "}
              <code className="bg-red-100 dark:bg-red-900/40 px-2 py-1 rounded text-xs font-mono">
                {errorId}
              </code>
            </p>
            <p className="text-xs opacity-75">
              Share this ID if you contact support so we can trace the incident.
            </p>
          </div>
        )}
        ...
        {retryCount >= maxRetries && (
          <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
            <p className="text-yellow-800 dark:text-yellow-200 text-sm">
              <strong>Maximum retry attempts reached.</strong> Please reload the
              page or report this issue if it persists.
            </p>
          </div>
        )}
      </div>
