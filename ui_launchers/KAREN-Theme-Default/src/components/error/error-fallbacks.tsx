// ui_launchers/KAREN-Theme-Default/src/components/error/error-fallbacks.tsx
"use client";
      ...
      <Card className="w-full max-w-md border-destructive/50">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex gap-2">
            {canRetry && onRetry && (
              <Button onClick={onRetry} className="flex-1" aria-label="Retry action">
    ...
    <div className={`flex items-center justify-center p-4 ${className || ""}`}>
      <Card className="w-full max-w-md border-orange-500/50">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-xl font-semibold">Network disconnected</CardTitle>
          <CardDescription>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex gap-2">
            {canRetry && onRetry && (
              <Button onClick={onRetry} className="flex-1" aria-label="Retry request">
                <RefreshCw className="h-4 w-4 mr-2" />
                Retry
              </Button>
            )}
    ...
    <div className={`flex items-center justify-center p-4 ${className || ""}`}>
      <Card className="w-full max-w-md border-red-500/50">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-xl font-semibold">Server error</CardTitle>
          <CardDescription>The server encountered an error. Our team has been notified.</CardDescription>
          {errorId && (
            <Badge variant="outline" className="w-fit mx-auto mt-2">
              ID: {errorId.slice(-8)}
            </Badge>
  ...
  return (
    <div className={`flex items-center justify-center p-4 ${className || ""}`}>
      <Card className="w-full max-w-md border-yellow-500/50">
        <CardHeader className="text-center">
          <div className="flex justify-center mb-4">
            <DatabaseIcon className="h-12 w-12 text-yellow-500" />
          </div>
          <CardTitle className="text-xl font-semibold">Data access issue</CardTitle>
          <CardDescription>Unable to load data. This may be a temporary issue.</CardDescription>
          {errorId && (
            <Badge variant="outline" className="w-fit mx-auto mt-2">
              ID: {errorId.slice(-8)}
            </Badge>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex gap-2">
            {canRetry && onRetry && (
              <Button onClick={onRetry} className="flex-1" aria-label="Retry data fetch">
                <RefreshCw className="h-4 w-4 mr-2" />
                Retry
              </Button>
            )}
...
export function CompactErrorFallback({
  ...
  return (
    <div
      className={`p-4 border border-destructive/50 rounded-lg bg-destructive/5 ${className || ""}`}
    >
      <div className="flex items-center space-x-3">
        ...
        {canRetry && onRetry && (
          <Button size="sm" variant="outline" onClick={onRetry} aria-label="Retry action">
    ...
    <div className={`flex items-center space-x-2 text-sm ${className || ""}`}>
      ...
      {canRetry && onRetry && (
        <Button
          variant="link"
          onClick={onRetry}
          className="text-xs sm:text-sm md:text-base p-0 h-auto"
