...
export const ErrorToast: React.FC<ErrorToastProps> = ({
  ...
  const handleAutoClose = useCallback(() => {
    ...
    handleClose();
  }, [id, message, type, duration, correlationId, priority, track, handleClose]);
  ...
  const startTimer = useCallback(() => {
    ...
    if (showProgress) {
      ...
      progressTimerRef.current = setInterval(() => {
        ...
        if (newProgress <= 0 && progressTimerRef.current) {
          clearInterval(progressTimerRef.current);
          progressTimerRef.current = null;
        }
      }, progressInterval);
    }
  }, [duration, persistent, showProgress, clearTimers, handleAutoClose]);
  ...
  }, [id, message, title, type, duration, persistent, correlationId, priority, position, track, startTimer, clearTimers]);
  ...
  return (
    <div
      className={`fixed z-50 ${maxWidth} w-full transform transition-all duration-300 ease-in-out ${getPositionClasses()} ${
      ...
      onBlur={resumeTimer}
    >
      <div className={`relative flex items-start p-4 rounded-lg ${getTypeStyles()} ${getPriorityStyles()}`}>
        {/* Progress bar */}
        ...
        <div className="flex items-start space-x-3 w-full">
          ...
          <div className="flex-1 min-w-0">
            {title && (
              <h4 className="text-sm font-semibold mb-1 md:text-base lg:text-lg">{title}</h4>
            )}
            <p className="text-sm leading-5 md:text-base lg:text-lg">{message}</p>
...
export const ErrorToastContainer: React.FC<ToastContainerProps> = ({
  ...
  const visibleToasts = [...toasts]
    .sort((a, b) => {
      const p = { critical: 4, high: 3, medium: 2, low: 1 } as const;
      const ap = p[a.priority || "medium"] || 2;
      const bp = p[b.priority || "medium"] || 2;
      return bp - ap;
    })
    .slice(0, maxToasts);
    ...
    >
      {visibleToasts.map((toast, index) => (
        <div key={toast.id} className="pointer-events-auto" style={{ zIndex: 1000 - index }}>
          <ErrorToast {...toast} position={position} onClose={() => onRemove(toast.id!)} />
        </div>
      ))}
    </div>
  );
