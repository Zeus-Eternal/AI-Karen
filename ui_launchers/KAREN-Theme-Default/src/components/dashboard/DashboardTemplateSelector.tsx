// ui_launchers/KAREN-Theme-Default/src/components/dashboard/DashboardTemplateSelector.tsx
"use client";
...
> = ({
  ...
  // Filtered templates (role + category + search)
  const filteredTemplates = useMemo(() => {
    const q = searchQuery.trim().toLowerCase();

    return templates.filter((template) => {
      // Role-based access
      if (template.category === "role-based" && template.roles?.length) {
        const hasRequiredRole = template.roles.some((r) => userRoles.includes(r));
  ...
  }, [templates, userRoles, selectedCategory, searchQuery]);
  ...
  return (
    <ErrorBoundary fallback={({ error, resetError }) => (
      <div className="p-4 text-center">
        <p className="text-destructive">Something went wrong in DashboardTemplateSelector</p>
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogTrigger asChild>
          <Button variant="outline" className={className} aria-label="Open Dashboard Templates">
                      ...
                      <div key={category}>
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <h3 className="text-lg font-semibold">
                            {getCategoryLabel(
                              category as DashboardTemplate["category"]
                            )}
                          </h3>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm md:text-base lg:text-lg">
                  ...
                  <div>
                    ...
                    {previewTemplate.config.layout}
                  </div>
                  <div>
                    <span className="font-medium">Widgets:</span>{" "}
                    {previewTemplate.config.widgets.length}
                  </div>
                  <div>
                    <span className="font-medium">Refresh:</span>{" "}
                    {Math.round(previewTemplate.config.refreshInterval / 1000)}s
                  </div>
                </div>
                ...
                {previewTemplate.config.widgets.length > 0 && (
                  <div>
                    ...
                    <div className="mt-2 space-y-2">
                      {previewTemplate.config.widgets.map((w) => (
                        <div
                          key={w.id}
                          className="flex items-center justify-between p-2 bg-muted rounded sm:p-4 md:p-6"
                        >
                          <div>
