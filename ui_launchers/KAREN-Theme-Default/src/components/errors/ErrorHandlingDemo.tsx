// ui_launchers/KAREN-Theme-Default/src/components/errors/ErrorHandlingDemo.tsx
/**
...
export function ErrorHandlingDemo() {
  ...
  /**
   * Wrapped demos (withErrorHandling)
   * These functions intentionally throw; the wrapper should capture and return a structured result.
   * If an implementation returns instead of throwing, we still handle that in the invoker.
   */
  const simulateNetworkError = withErrorHandling(
    async () => {
  ...
  const simulateAuthError = withErrorHandling(
    async () => {
  ...
  const simulateDatabaseError = withErrorHandling(
    async () => {
  ...
  const simulateTimeoutError = withErrorHandling(
    async () => {
  ...
  /**
   * Direct demos (handleError)
   * These return the ErrorHandlingResult directly via handleError.
   */
  const simulateValidationError = async (): Promise<ErrorHandlingResult> => {
  ...
  const simulateConfigError = async (): Promise<ErrorHandlingResult> => {
    const result = await handleError(
      new Error("Invalid backend URL configuration"),
      {
        enableRecovery: false,
  ...
  /**
   * Invokers
   * handleWrappedDemo: preferred path for withErrorHandling-wrapped demos.
   * If the wrapper throws, we catch and process via handleError as a fallback.
   */
  const handleWrappedDemo = async (
    ...
    try {
      ...
      // Some implementations of withErrorHandling return an ErrorHandlingResult with ok=false.
      const maybeResult = result as Partial<ErrorHandlingResult> | undefined;
      if (maybeResult && typeof maybeResult === "object" && "ok" in maybeResult) {
        if ((maybeResult as ErrorHandlingResult).ok === false) {
          const errResult = maybeResult as ErrorHandlingResult;
          setState((prev) => ({
            ...prev,
            isLoading: false,
            lastError: errResult,
            errorHistory: [
              ...prev.errorHistory.slice(-9),
              errResult.categorizedError,
            ],
          }));
          return;
        }
      ...
      // If no structured error returned, treat as success
      updateState({
        isLoading: false,
        successMessage: `${label} demo completed successfully!`,
        lastError: null,
      });
    } catch (error) {
      // Fallback: use handleError
      ...
      setState((prev) => ({
        ...
        errorHistory: [
          ...prev.errorHistory.slice(-9),
          errorResult.categorizedError,
        ],
      }));
    }
  };

  const handleDirectErrorDemo = async (
    demoFunction: () => Promise<ErrorHandlingResult>,
    label: string
  ) => {
    updateState({ isLoading: true, lastError: null, successMessage: "" });
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      <div className="bg-white rounded-lg shadow-lg p-6">
        ...
        <p className="text-gray-600 mb-6">
          Explore how Kari&rsquo;s error layer categorizes failures, chooses recovery
          strategies, and crafts user-friendly messages with retry/backoff logic.
        </p>

        {/* Demo Controls */}
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
          <Button
            onClick={() => handleWrappedDemo(simulateNetworkError, "Network Error")}
          ...
          <Button
            onClick={() => handleWrappedDemo(simulateAuthError, "Auth Error")}
            disabled={state.isLoading}
          ...
          <Button
            onClick={() => handleWrappedDemo(simulateDatabaseError, "Database Error")}
            disabled={state.isLoading}
          ...
          <Button
            onClick={() => handleWrappedDemo(simulateTimeoutError, "Timeout Error")}
            disabled={state.isLoading}
              ...
              <div>
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div>
                    ...
                    <span
                      className={`ml-2 ${
                        state.lastError.categorizedError.retryable
                          ? "text-green-600"
                          : "text-red-600"
                      }`}
                    >
              ...
              <div>
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div>
                    ...
                    <span
                      className={`ml-2 ${
                        state.lastError.requiresUserAction
                          ? "text-orange-600"
                          : "text-green-600"
                      }`}
                    >
        ...
        {state.errorHistory.length > 0 && (
          <div className="bg-white border border-gray-200 rounded-md p-6">
            ...
            <div className="space-y-2">
              {state.errorHistory.map((err, index) => {
                const t = new Date(err.timestamp as any);
                return (
                  <div
                    key={`${err.code}-${index}`}
                  ...
                  >
                    <div className="flex items-center space-x-3">
                      <span className="text-sm text-gray-500">
                        #{state.errorHistory.length - index}
                      </span>
                      <span
                        className={`px-2 py-1 rounded-full text-xs ${getCategoryColor(
                          err.category
