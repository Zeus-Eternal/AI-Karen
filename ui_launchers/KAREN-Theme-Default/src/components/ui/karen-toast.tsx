  ...
  ({ className, duration = 5000, variant = "default", enableAnimations = true, style, ...props }, ref) => {
    ...
    React.useEffect(() => {
      ...
      const interval = setInterval(() => {
        setProgress((previous) => Math.max(previous - 100 / Math.max(duration / 100, 1), 0));
      }, 100);

      return () => clearInterval(interval);
    }, [duration, enableAnimations]);

    const progressClassName = progressVariantStyles[variant ?? "default"] ?? progressVariantStyles.default;

    return (
      <div
        ref={ref}
        className={cn(
          "pointer-events-none absolute inset-x-0 bottom-0 h-1 w-full overflow-hidden rounded-b-lg bg-current/10",
          className
        )}
        style={style}
        {...props}
...
const KarenToast = React.forwardRef<React.ElementRef<typeof ToastPrimitives.Root>, KarenToastRootProps>(
  (
    ...
    const expandableContentId = React.useId();
                ...
                <div className="mt-1">
                  ...
                  {isExpanded ? (
                    <div
                      ...
                      className="mt-2 rounded-md bg-black/5 p-2 text-xs dark:bg-white/5 sm:text-sm"
                    >
                      {alert.expandableContent}
                    </div>
                  ) : null}
