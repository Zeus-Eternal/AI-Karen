...
>(({ className, duration = 5000, variant = "default", enableAnimations = true, style, ...props }, ref) => {
  ...
  React.useEffect(() => {
    ...
    const tick = 100;
    const decrement = 100 / (duration / tick);
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "h-1 w-full overflow-hidden rounded-full transition-all duration-100 ease-linear motion-reduce:transition-none",
        getProgressColor(),
        className
      )}
      style={{
...
const KarenToast = React.forwardRef<React.ElementRef<typeof ToastPrimitives.Root>, KarenToastRootProps>(
  (
    ...
    const resolvedVariant: KarenToastVariant = alert?.variant ?? variant ?? "default";
