  ...
  (
    ...
    React.useEffect(() => {
      ...
      const intervalId: ReturnType<typeof setInterval> = setInterval(() => {
        setProgress((previous) => {
          const next = previous - 100 / (duration / 100);
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "relative mt-4 h-1.5 w-full overflow-hidden rounded-full bg-current/10",
          className
        )}
        aria-hidden="true"
        {...props}
...
const KarenToast = React.forwardRef<React.ElementRef<typeof ToastPrimitives.Root>, KarenToastRootProps>(
  (
    ...
    const resolvedVariant = (alert?.variant ?? variant ?? "default") as VariantProps<
      typeof karenToastVariants
    >["variant"];
          ...
          {alert?.expandableContent ? (
            <div>
              <button
                ...
                className="flex items-center gap-1 text-xs font-medium opacity-75 transition-opacity hover:opacity-100 sm:text-sm md:text-base"
                aria-expanded={isExpanded}
                aria-controls={`karen-toast-${alert.id}-details`}
              >
              ...
              {isExpanded ? (
                <div
                  id={`karen-toast-${alert.id}-details`}
