...
type KarenToastVariant = NonNullable<VariantProps<typeof karenToastVariants>["variant"]>;

const progressColors: Record<KarenToastVariant, string> = {
  default: "bg-gradient-to-r from-primary/80 to-primary shadow-sm",
  destructive: "bg-gradient-to-r from-red-500 to-red-700 shadow-sm shadow-red-200/50",
...
>(({ className, duration = 5000, variant = "default", enableAnimations = true, style, ...props }, ref) => {
  ...
  const resolvedVariant = (variant ?? "default") as KarenToastVariant;
  const progressClassName = progressColors[resolvedVariant] ?? progressColors.default;
        ...
        <div className="flex w-full flex-col space-y-3">
          <div className="flex items-start space-x-3">
            ...
            <div className="flex-1 space-y-1">
              ...
              {alert?.message ? (
                <p className="text-sm leading-relaxed text-current/80">{alert.message}</p>
              ) : null}
              {children}
            </div>
          </div>
            ...
            <div className="space-y-2">
              ...
              {isExpanded ? (
                <div
                  ...
                  className="rounded-md bg-black/5 p-2 text-xs sm:text-sm dark:bg-white/5"
                >
                  {alert.expandableContent}
                </div>
              ) : null}
...
type KarenToastViewportPosition = "top-right" | "top-left" | "bottom-right" | "bottom-left";

const KarenToastViewport = React.forwardRef<
  React.ElementRef<typeof ToastPrimitives.Viewport>,
  React.ComponentPropsWithoutRef<typeof ToastPrimitives.Viewport> & { position?: KarenToastViewportPosition }
