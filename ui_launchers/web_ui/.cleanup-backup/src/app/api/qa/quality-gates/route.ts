...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('Failed to generate quality gates:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'override' && gateId) {
      ...
      // Log the override
      console.log(`Quality gate ${gateId} overridden by user`);
    ...
    console.error('Failed to process quality gate action:', error);
    return NextResponse.json(
