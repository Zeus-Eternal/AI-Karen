...
function parseUrlEnv(value: string | undefined, defaultValue: string): string {
  ...
  try {
    ...
    console.warn(`Invalid URL in environment variable: ${value}, using default: ${defaultValue}`);
    return defaultValue;
  }
}
...
export function logConfigInfo(config: WebUIConfig): void {
  ...
  console.log('Fallback URLs:', config.fallbackBackendUrls);
  console.log('Container Config:', {
    host: config.containerBackendHost,
    port: config.containerBackendPort,
  });
  console.log('External Config:', {
    host: config.externalHost || 'not configured',
    port: config.externalBackendPort,
  });
