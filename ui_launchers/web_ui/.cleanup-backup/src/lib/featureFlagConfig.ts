...
export const loadConfigFromStorage = (
  ...
  try {
    ...
    if (!validation.isValid) {
      console.error('Invalid stored feature flag configuration:', validation.errors);
      return null;
    }

    if (validation.warnings.length > 0) {
      console.warn('Stored feature flag configuration warnings:', validation.warnings);
    }
...
export const createConfigUpdater = (
  ...
  return {
    updateFlag: (flag: FeatureFlag, value: boolean) => {
      ...
      if (validation.warnings.length > 0) {
        console.warn(`Updating flag '${flag}':`, validation.warnings);
      }
    ...
    updateFlags: (flags: Partial<Record<FeatureFlag, boolean>>) => {
      ...
      if (validation.warnings.length > 0) {
        console.warn('Updating flags:', validation.warnings);
      }
