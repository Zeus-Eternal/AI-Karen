...
export default function KariDevStudio() {
  ...
  const [selectedComponent, setSelectedComponent] =
    useState<KariComponent | null>(null);
  ...
  useCopilotAction({
    ...
    description:
      "Analyze the health of Kari components and suggest improvements",
    parameters: [
      {
        name: "component_type",
        type: "string",
        description:
          "Type of component to analyze (plugin, extension, hook, llm_provider)",
        required: false,
      },
    ],
    handler: async ({ component_type }) => {
      ...
      const unhealthyComponents = componentsToAnalyze.filter(
        (c) => c.health !== "healthy"
      );
      const lowPerformance = componentsToAnalyze.filter(
        (c) => c.metrics.success_rate < 0.9
      );
  ...
  useCopilotAction({
    ...
    handler: async ({ component_type, component_name, features }) => {
      return generateComponentBoilerplate(
        component_type,
        component_name,
        features?.split(",") || []
      );
    ...
    },
    {
      ...
      cellRenderer: (params: any) => (
        <div className="flex items-center gap-2">
          <MessageSquare
            className={`h-4 w-4 ${
              params.value ? "text-blue-500" : "text-gray-400"
            }`}
          />
          <Badge
    ...
    },
    {
      ...
      cellRenderer: (params: any) => (
        <div className="flex items-center gap-2">
          <Brain
            className={`h-4 w-4 ${
              params.value ? "text-purple-500" : "text-gray-400"
            }`}
          />
          <Badge
  ...
  const chatMetricsChart: AgCartesianChartOptions = {
    data: chatMetrics,
    series: [
      {
        type: "line",
    ...
    theme: {
      palette: {
        fills: ["#3b82f6", "#8b5cf6", "#10b981"],
        strokes: ["#3b82f6", "#8b5cf6", "#10b981"],
      },
    },
  };
  ...
  const handleComponentAction = async (componentId: string, action: string) => {
    try {
      const response = await fetch(
        `/api/developer/components/${componentId}/${action}`,
        {
          method: "POST",
        }
      );
  ...
  const generateHealthRecommendations = (
    unhealthy: KariComponent[],
    lowPerf: KariComponent[]
  ) => {
    ...
    if (lowPerf.length > 0) {
      recommendations.push(
        `${lowPerf.length} components have low success rates`
      );
                ...
                <CardContent className="p-4 sm:p-4 md:p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold">
                        {
                          components.filter((c) => c.health !== "healthy")
                            .length
                        }
                      </p>
                    </div>
