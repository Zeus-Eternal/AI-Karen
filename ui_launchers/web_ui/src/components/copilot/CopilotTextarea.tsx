...
export const CopilotTextarea: React.FC<CopilotTextareaProps> = ({
  ...
  const fetchSuggestions = useCallback(async (text: string, position: number) => {
    if (!enableSuggestions || !config.features.contextualSuggestions || text.length < 10) {
      setSuggestions([]);
      return;
    }
    ...
    try {
      ...
      const formattedSuggestions: AISuggestion[] = rawSuggestions.map((suggestion, index) => ({
      ...
      await triggerHooks('copilot_suggestions_displayed', {
  ...
  }, [enableSuggestions, config.features.contextualSuggestions, getSuggestions, language, triggerHooks, user?.user_id]);
  ...
  const applySuggestion = useCallback(async (suggestion: AISuggestion) => {
    ...
    const newValue = value.substring(0, start) + suggestion.content + value.substring(end);
    ...
    await triggerHooks('copilot_suggestion_applied', {
  ...
  }, [enableDocGeneration, value, language, generateDocumentation, onChange, toast]);
  ...
  const SuggestionCard = ({ suggestion }: { suggestion: AISuggestion }) => {
    const getIcon = () => {
      switch (suggestion.type) {
        ...
        case 'refactor': return <Zap className="h-4 w-4" />;
        case 'debug': return <AlertCircle className="h-4 w-4" />;
        case 'optimize': return <Sparkles className="h-4 w-4" />;
        case 'explain': return <FileText className="h-4 w-4" />;
    ...
    const getVariant = () => {
      switch (suggestion.type) {
        case 'debug': return 'destructive';
        case 'optimize': return 'default';
        default: return 'secondary';
      }
    };

    return (
      <Card className="mb-2 cursor-pointer hover:bg-muted/50 transition-colors" onClick={() => applySuggestion(suggestion)}>
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <div className="flex items-center gap-2 mt-2">
        ...
        {analysisResults && (
          <Badge variant={analysisResults.issues?.length > 0 ? 'destructive' : 'default'}>
          ...
          <CardContent className="p-3">
            ...
            {analysisResults.issues && analysisResults.issues.length > 0 ? (
              <div className="space-y-2">
                {analysisResults.issues.slice(0, 3).map((issue: any, index: number) => (
                  <div key={index} className="p-2 bg-destructive/10 rounded border-l-2 border-destructive">
                    <p className="text-sm font-medium text-destructive">{issue.type || 'Issue'}</p>
              ...
              <p className="text-sm text-green-600">No issues found! Your code looks good.</p>
            )}
          </CardContent>
        </Card>
      )}
