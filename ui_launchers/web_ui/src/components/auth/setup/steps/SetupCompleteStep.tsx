    ...
    <div className="space-y-6">
      ...
      <div className="text-center space-y-4">
        <div className="flex justify-center">
          <div className="relative">
            <CheckCircle className="h-20 w-20 text-green-600 drop-shadow-lg sm:w-auto md:w-full" />
            <div className="absolute inset-0 h-20 w-20 bg-green-600/20 rounded-full blur-xl animate-pulse sm:w-auto md:w-full" />
          </div>
        </div>
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-semibold text-center mb-4 flex items-center justify-center">
          <Sparkles className="h-5 w-5 mr-2 text-primary sm:w-auto md:w-full" />
          What's Been Set Up
        </h3>
              ...
              <CardContent className="p-4 sm:p-4 md:p-6">
                <div className="flex items-start space-x-3">
                  <div className="flex-shrink-0">
                    <feature.icon className="h-6 w-6 text-green-600 sm:w-auto md:w-full" />
                  </div>
                  <div className="flex-1">
                    <h4 className="font-semibold text-green-900 dark:text-green-100">
                      {feature.title}
                    </h4>
                    <p className="text-sm text-green-800 dark:text-green-200 mt-1 md:text-base lg:text-lg">
                      {feature.description}
                    </p>
                  </div>
                  <CheckCircle className="h-5 w-5 text-green-600 flex-shrink-0 sm:w-auto md:w-full" />
                </div>
              </CardContent>
            </Card>
          ))}
      ...
      <Card className="bg-purple-50 dark:bg-purple-950/20 border-purple-200 dark:border-purple-800">
        <CardContent className="p-4 sm:p-4 md:p-6">
          ...
          <ul className="space-y-2 text-sm text-purple-800 dark:text-purple-200 md:text-base lg:text-lg">
            {nextSteps.map((step, index) => (
              <li key={index} className="flex items-start space-x-2">
                <span className="flex-shrink-0 w-5 h-5 bg-purple-600 text-white rounded-full flex items-center justify-center text-xs font-bold mt-0.5 sm:w-auto md:w-full">
                  {index + 1}
                </span>
                <span>{step}</span>
              </li>
      ...
      <div className="text-center pt-6">
        <button
         ...
         aria-label="Button">
          {isLoading ? (
            <>
              <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-white mr-3 sm:w-auto md:w-full" />
              Finalizing Setup...
            </>
          ) : (
            <>
              <span>Enter AI Karen Dashboard</span>
              <ArrowRight className="ml-3 h-6 w-6 sm:w-auto md:w-full" />
            </>
          )}
        </Button>
        <p className="text-sm text-muted-foreground mt-3 md:text-base lg:text-lg">
