            ...
            <div className="space-y-4">
                  ...
                  {uploadFiles.map((uploadFile) => (
                    <div key={uploadFile.id} className="flex items-center gap-3 p-3 border rounded-lg sm:p-4 md:p-6">
                      ...
                      <div className="flex-1 min-w-0 sm:w-auto md:w-full">
                        ...
                        {uploadFile.error && (
                          <p className="text-xs text-red-600 mt-1 sm:text-sm md:text-base">{uploadFile.error}</p>
                        )}
                      </div>
                    </div>
                  ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {SUPPORTED_FORMATS.map((format) => (
                  <div key={format.value} className="p-3 border rounded-lg sm:p-4 md:p-6">
                    <div className="flex items-center gap-2 mb-1">
                      <Badge variant="outline">{format.label}</Badge>
                    </div>
                    <p className="text-xs text-muted-foreground sm:text-sm md:text-base">{format.description}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <select value={selectedArchitecture} onValueChange={setSelectedArchitecture} aria-label="Select option">
                    <selectTrigger aria-label="Select option">
                      <selectValue />
                    </SelectTrigger>
                    <selectContent aria-label="Select option">
                      {MODEL_ARCHITECTURES.map((arch) => (
                        <selectItem key={arch} value={arch} aria-label="Select option">
