        ...
        <CardContent className="space-y-4">
                  ...
                  <Tooltip key={mode}>
                    <TooltipTrigger asChild>
                      <button
                        ...
                        onClick={() = aria-label="Button"> handleModeChange(mode)}
                        disabled={isDisabled}
                        className="flex flex-col items-center gap-2 h-auto p-4 sm:p-4 md:p-6"
                      >
                        <div className="flex items-center gap-2">
                          {modeInfo.icon}
                          <span className="font-medium">{modeInfo.label}</span>
                        </div>
                        <div className="text-xs text-center opacity-75 sm:text-sm md:text-base">
          ...
          <div className="space-y-3">
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                     ...
                     aria-label="Button">
                      <Shuffle className="w-4 h-4 mr-1 sm:w-auto md:w-full" />
                      Quick Switch
                    </Button>
                  </TooltipTrigger>
                  <TooltipContent>
            ...
            {modelsForCurrentMode.length > 0 ? (
                      ...
                      <div className="flex items-center justify-between w-full">
                        ...
                        <div className="flex items-center gap-1">
                          {model.capabilities?.slice(0, 2).map((cap) => (
                            <Badge key={cap} variant="secondary" className="text-xs sm:text-sm md:text-base">
                              {cap}
                            </Badge>
                          ))}
                        </div>
              ...
              <div className="text-center p-4 border border-dashed rounded-lg sm:p-4 md:p-6">
                <AlertTriangle className="w-6 h-6 text-yellow-500 mx-auto mb-2 sm:w-auto md:w-full" />
          ...
          {chatContext && chatContext.conversationLength > 0 && (
            <div className="bg-blue-50 p-3 rounded-lg sm:p-4 md:p-6">
              <div className="flex items-start gap-2">
                <Clock className="w-4 h-4 text-blue-600 mt-0.5 flex-shrink-0 sm:w-auto md:w-full" />
          ...
          {modelsLoading && (
            <div className="text-center text-gray-500 py-2">
              <div className="inline-flex items-center gap-2">
                <div className="w-4 h-4 border-2 border-gray-300 border-t-gray-600 rounded-full animate-spin sm:w-auto md:w-full" />
                Loading models...
              </div>
            </div>
          )}
          {modelsError && (
            <div className="text-center text-red-600 py-2">
              <AlertTriangle className="w-4 h-4 mx-auto mb-1 sm:w-auto md:w-full" />
              <p className="text-sm md:text-base lg:text-lg">Error loading models: {modelsError}</p>
            </div>
          )}
        </CardContent>
      </Card>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          {pendingTransition && (
            <div className="space-y-4">
              {/* Transition Summary */}
              <div className="flex items-center justify-center gap-2 p-3 bg-gray-50 rounded-lg sm:p-4 md:p-6">
                ...
                <ArrowRight className="w-4 h-4 text-gray-400 sm:w-auto md:w-full" />
                <div className="text-center">
              ...
              {pendingTransition.warnings.length > 0 && (
                <div className="space-y-2">
                  <p className="text-sm font-medium md:text-base lg:text-lg">Changes:</p>
                  <ul className="space-y-1">
                    {pendingTransition.warnings.map((warning, index) => (
                      <li key={index} className="flex items-start gap-2 text-sm text-gray-600 md:text-base lg:text-lg">
                        <Info className="w-3 h-3 mt-0.5 flex-shrink-0 sm:w-auto md:w-full" />
                        {warning}
                      </li>
                    ))}
                  </ul>
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) = aria-label="Input"> setPreserveContext(e.target.checked)}
                      className="rounded"
                    />
                    <label htmlFor="preserveContext" className="text-sm font-medium md:text-base lg:text-lg">
                      Preserve conversation context
                    </label>
                  </div>
                  <p className="text-xs text-gray-600 ml-6 sm:text-sm md:text-base">
          ...
          <DialogFooter>
            <button variant="outline" onClick={handleCancelTransition} aria-label="Button">
              Cancel
            </Button>
            <button onClick={handleConfirmTransition} disabled={isTransitioning} aria-label="Button">
              {isTransitioning ? (
                <>
