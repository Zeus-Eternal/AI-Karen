...
export function ExtensionNavigation({ className, compact = false }: ExtensionNavigationProps) {
...
function ExtensionNavGroup({ extensionId, items, currentPath, compact }: ExtensionNavGroupProps) {
...
function ExtensionNavItem({ item, currentPath, compact, children = [] }: ExtensionNavItemProps) {
  const isActive = currentPath === item.path || currentPath.startsWith(item.path + '/');
  const hasChildren = children.length > 0;
  
  const IconComponent = item.icon && iconMap[item.icon] ? iconMap[item.icon] : Puzzle;
      ...
      >
        ...
        {!compact && (
          <>
            ...
            {/* Extension indicator */}
            <div className="ml-2 w-2 h-2 bg-blue-400 rounded-full opacity-60 sm:w-auto md:w-full" />
          </>
        )}
      </Link>
              ...
              {child.icon && iconMap[child.icon] ? (
                React.createElement(iconMap[child.icon], {
                  className: `flex-shrink-0 h-3 w-3 mr-2 ${
                    currentPath === child.path ? 'text-blue-500' : 'text-gray-400'
                  }`
                })
              ) : (
                <div className="w-3 h-3 mr-2 sm:w-auto md:w-full" />
              )}
 ...
 */
export function ExtensionNavigationBreadcrumbs({ className }: { className?: string }) {
  const pathname = usePathname();
  const navItems = useExtensionNavigation();
  ...
  const breadcrumbs = pathSegments.map((segment, index) => {
    ...
    return {
      label: item?.label || segment.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
      path,
