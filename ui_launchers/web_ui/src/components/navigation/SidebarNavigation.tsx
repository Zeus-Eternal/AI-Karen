  ...
  ({ 
    ...
    useEffect(() => {
      ...
      const handleKeyDown = (event: KeyboardEvent) => {
        ...
        const currentItemId = Array.from(itemRefs.current.entries()).find(([_, button]) => button === currentButton)?.[0];
        ...
        switch (event.key) {
          ...
          case 'ArrowRight':
            ...
            if (currentItem?.item.children && !expandedItems.has(currentItem.item.id)) {
          ...
          case 'ArrowLeft':
            ...
            if (currentItemForLeft?.item.children && expandedItems.has(currentItemForLeft.item.id)) {
    ...
    }, [enableKeyboardNavigation, flattenedItems, expandedItems, handleItemClick, toggleExpanded]);
    ...
    <li>
      <button
        ...
        className={cn(
          ...
          {
            // Active state
            'bg-[var(--color-primary-100)] text-[var(--color-primary-900)] dark:bg-[var(--color-primary-900)] dark:text-[var(--color-primary-100)]': isActive,
            // Hover state
            'hover:bg-[var(--color-neutral-200)] hover:text-[var(--color-neutral-900)] dark:hover:bg-[var(--color-neutral-800)] dark:hover:text-[var(--color-neutral-100)]': !isActive,
            // Default state
            'text-[var(--color-neutral-700)] dark:text-[var(--color-neutral-300)]': !isActive,
            // Disabled state
        ...
        <ul className="mt-[var(--space-xs)] space-y-[var(--space-xs)]" role="list">
          {item.children!.map((child) => (
            <NavigationItemComponent
              ...
              itemRef={(el) => {
                if (el) {
                  itemRefs.current.set(child.id, el);
                } else {
                  itemRefs.current.delete(child.id);
                }
              }}
            />
          ))}
...
// Utility functions
function findActiveItem(items: NavigationItem[], pathname: string): { item: NavigationItem; parent?: string } | null {
