            ...
            <div className="flex flex-wrap gap-2">
              {Object.entries(AUDIT_FILTER_PRESETS).map(([key, preset]) => (
                <button
                  ...
                  className="px-3 py-1 text-xs font-medium text-gray-700 bg-gray-100 rounded-full hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
                  {preset.name}
                </button>
              ))}
              ...
              <button
                ...
                className="px-3 py-1 text-xs font-medium text-red-700 bg-red-100 rounded-full hover:bg-red-200 focus:outline-none focus:ring-2 focus:ring-red-500"
              >
                Clear All
              </button>
            </div>
            ...
            <thead className="bg-gray-50 sticky top-0">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                  <input
                    type="checkbox"
                    checked={selectedLogs.size === logs.data.length && logs.data.length > 0}
                ...
                logs.data.map((log) => (
                  <tr key={log.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      {log.user?.full_name && (
                        <div className="text-sm text-gray-500">{log.user.full_name}</div>
                      )}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
