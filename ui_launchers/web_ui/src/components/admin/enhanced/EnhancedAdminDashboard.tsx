...
export function EnhancedAdminDashboard({ className = '' }: EnhancedAdminDashboardProps) {
  ...
  const loadDashboardData = async () => {
    const result = await handleAsyncOperation(async () => {
      ...
      if (!statsResponse.ok) {
        throw new Error(`Failed to load user statistics: ${statsResponse.statusText}`);
      }
      ...
      if (!activityResponse.ok) {
        throw new Error(`Failed to load activity summary: ${activityResponse.statusText}`);
      }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              },
              {
                ...
                subtitle: `${Math.round((dashboardData.userStats.verified_users / dashboardData.userStats.total_users) * 100)}% verified`,
                color: 'text-blue-600'
              },
              {
                title: '2FA Enabled',
                value: dashboardData.userStats.two_factor_enabled,
                subtitle: `${Math.round((dashboardData.userStats.two_factor_enabled / dashboardData.userStats.total_users) * 100)}% adoption`,
              ...
              <div>
                ...
                <dl className="space-y-2 text-sm md:text-base lg:text-lg">
                  {[
                    ...
                    { term: 'Successful Logins', value: dashboardData.activitySummary.successful_logins, className: 'text-green-600' },
                    { term: 'Failed Logins', value: dashboardData.activitySummary.failed_logins, className: 'text-red-600' }
                  ].map((metric, index) => (
                    <div key={index} className="flex justify-between">
                      <dt>{metric.term}:</dt>
                      <dd className={`font-medium ${metric.className || ''}`}>{metric.value}</dd>
                    </div>
                  ))}
                </dl>
              </div>
  ...
  return (
    <div className={`min-h-screen bg-gray-50 ${className}`} ref={dashboardRef}>
