    ...
    return (
      <Card>
        <CardContent className="py-12 text-center">
          <Package className="h-12 w-12 mx-auto text-muted-foreground mb-4 sm:w-auto md:w-full" />
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="flex-1 relative">
          ...
          <input
            ...
            onChange={(e) = aria-label="Input"> setLocalSearchQuery(e.target.value)}
            className="pl-10"
          />
        </div>
        <div className="flex items-center gap-2">
          <select
            value={sortBy}
            onChange={(e) = aria-label="Select option"> setSortBy(e.target.value as SortOption)}
            className="px-3 py-2 text-sm border rounded-md bg-background md:text-base lg:text-lg"
          >
          ...
          <button
            variant="outline"
            size="sm"
            onClick={() = aria-label="Button"> setSortDirection(sortDirection === 'asc' ? 'desc' : 'asc')}
          >
            {sortDirection === 'asc' ? '↑' : '↓'}
          </Button>
          ...
          >
            {viewMode === 'grid' ? <List className="h-4 w-4 sm:w-auto md:w-full" /> : <Grid className="h-4 w-4 sm:w-auto md:w-full" />}
          </Button>
        </div>
      </div>
      {/* Bulk Actions Bar */}
          ...
          <CardContent className="py-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-2">
                  <button size="sm" onClick={selectAllVisible} aria-label="Button">
                    Select All Visible ({filteredAndSortedModels.length})
                  </Button>
                  <button variant="outline" size="sm" onClick={clearSelection} aria-label="Button">
                    Clear Selection
                  </Button>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <button 
                  size="sm" 
                  variant="outline"
                  onClick={() = aria-label="Button"> handleBulkAction('pin')}
                ...
                <button 
                  size="sm" 
                  variant="outline"
                  onClick={() = aria-label="Button"> handleBulkAction('unpin')}
                  disabled={bulkActionLoading === 'unpin'}
                  className="gap-1"
                >
                  <PinOff className="h-3 w-3 sm:w-auto md:w-full" />
                  Unpin Selected
                </Button>
                <button 
                  size="sm" 
      ...
      {filteredAndSortedModels.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center">
            <Search className="h-12 w-12 mx-auto text-muted-foreground mb-4 sm:w-auto md:w-full" />
            ...
            <button 
              variant="outline" 
              onClick={() = aria-label="Button"> setLocalSearchQuery('')}
            >
              Clear Search
            </Button>
          </CardContent>
            ...
            <div key={model.id} className="relative">
              ...
              <div className="absolute top-2 right-2 z-10">
                <button
                  variant="ghost"
                  size="sm"
                  onClick={() = aria-label="Button"> handleTogglePin(model.id, model.pinned || false)}
                  className="h-8 w-8 p-0 bg-background/80 hover:bg-background sm:w-auto md:w-full"
                >
                  {model.pinned ? (
                    <Pin className="h-3 w-3 text-orange-500 sm:w-auto md:w-full" />
                  ) : (
                    <PinOff className="h-3 w-3 text-muted-foreground sm:w-auto md:w-full" />
                  )}
                </Button>
              </div>
              <ModelCard
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {model.pinned && (
                        <Badge variant="secondary" className="text-xs sm:text-sm md:text-base">
                          <Pin className="h-2 w-2 mr-1 sm:w-auto md:w-full" />
                          Pinned
                        </Badge>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button
                      variant="destructive"
                      size="sm"
                      onClick={() = aria-label="Button"> onAction(model.id, 'delete')}
                      className="gap-1"
                    >
                      <Trash2 className="h-3 w-3 sm:w-auto md:w-full" />
                      Delete
