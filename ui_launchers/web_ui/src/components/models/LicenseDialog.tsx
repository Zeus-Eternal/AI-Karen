...
export function LicenseDialog({
  ...
  const getRestrictionIcon = (restriction: string) => {
    ...
    if (restriction.toLowerCase().includes('attribution')) {
      return <FileText className="h-4 w-4 text-blue-500 sm:w-auto md:w-full" />;
    }
    return <AlertTriangle className="h-4 w-4 text-orange-500 sm:w-auto md:w-full" />;
  };
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[80vh] flex flex-col sm:w-auto md:w-full">
        <DialogHeader>
          ...
          <DialogDescription>
            Model <code className="bg-muted px-1 py-0.5 rounded text-sm md:text-base lg:text-lg">{modelId}</code> requires license acceptance before download.
          </DialogDescription>
        </DialogHeader>
        <div className="flex-1 space-y-4 overflow-hidden">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium md:text-base lg:text-lg">License Type:</span>
            ...
            {licenseInfo.url && (
              <button
                variant="outline"
                size="sm"
                onClick={() = aria-label="Button"> window.open(licenseInfo.url, '_blank')}
              >
                View Original
              </Button>
            )}
                ...
                <div className="space-y-2">
                  ...
                  <ul className="space-y-1">
                    {licenseInfo.restrictions.map((restriction, index) => (
                      <li key={index} className="flex items-center gap-2 text-sm md:text-base lg:text-lg">
          ...
          {/* License Usage Info */}
          <div className="grid grid-cols-2 gap-4 p-3 bg-muted rounded-lg sm:p-4 md:p-6">
            <div className="flex items-center gap-2">
          ...
          <div className="flex-1 min-h-0">
            <label className="text-sm font-medium mb-2 block md:text-base lg:text-lg">License Text:</label>
            <ScrollArea className="h-48 w-full border rounded-md p-3 sm:p-4 md:p-6">
              <pre className="text-xs whitespace-pre-wrap font-mono sm:text-sm md:text-base">
                {licenseInfo.text}
              </pre>
            </ScrollArea>
          </div>
          ...
          <div className="space-y-3 pt-2 border-t">
            <div className="flex items-center space-x-2">
              ...
              <label
                htmlFor="hasRead"
                className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 md:text-base lg:text-lg"
              >
        ...
        <DialogFooter className="gap-2">
          ...
          <button
           ...
           aria-label="Button">
            {isAccepting && <Loader2 className="mr-2 h-4 w-4 animate-spin sm:w-auto md:w-full" />}
            Accept License
          </Button>
        </DialogFooter>
      </DialogContent>
