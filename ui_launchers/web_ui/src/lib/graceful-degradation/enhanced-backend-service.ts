...
export class EnhancedBackendService {
  ...
  async makeEnhancedRequest<T>(options: EnhancedRequestOptions): Promise<T> {
    ...
    if (!healthStatus.isHealthy && healthStatus.consecutiveFailures >= 3) {
      console.warn(`Service ${serviceName} is unhealthy, attempting graceful degradation`);
  ...
  private async handleDisabledService<T>(
    ...
    console.warn(`Service ${serviceName} is disabled, attempting graceful degradation`);

    // Try to get cached data
    if (cacheKey && useStaleOnError) {
      const cachedData = extensionCache.getStale<T>(cacheKey, maxStaleAge);
      if (cachedData) {
        console.info(`Using stale cached data for disabled service ${serviceName}`);
        return cachedData;
      }
    }
    ...
    if (fallbackData !== undefined) {
      console.info(`Using fallback data for disabled service ${serviceName}`);
      return fallbackData;
    }
  ...
  private async handleUnhealthyService<T>(
    ...
    if (cacheKey && useStaleOnError) {
      const cachedData = extensionCache.getStale<T>(cacheKey, maxStaleAge);
      if (cachedData) {
        console.info(`Using stale cached data for unhealthy service ${serviceName}`);
        return cachedData;
      }
    }
    ...
    // Use fallback data if available
    if (fallbackData !== undefined) {
      console.info(`Using fallback data for unhealthy service ${serviceName}`);
      return fallbackData;
    }
  ...
  private async handleAuthenticationError<T>(
    ...
    console.warn(`Authentication error for service ${serviceName}:`, error.message);
    ...
    if (cacheKey && useStaleOnError) {
      const cachedData = extensionCache.getStale<T>(cacheKey, maxStaleAge);
      if (cachedData) {
        console.info(`Using stale cached data due to authentication error for ${serviceName}`);
        return cachedData;
      }
    }
    ...
    // Use fallback data if available
    if (fallbackData !== undefined) {
      console.info(`Using fallback data due to authentication error for ${serviceName}`);
      return fallbackData;
    }
  ...
  private async handleServiceUnavailableError<T>(
    ...
    if (cacheKey && useStaleOnError) {
      const cachedData = extensionCache.getStale<T>(cacheKey, maxStaleAge);
      if (cachedData) {
        console.info(`Using stale cached data due to service unavailable for ${serviceName}`);
        return cachedData;
      }
    }
    ...
    // Use fallback data if available
    if (fallbackData !== undefined) {
      console.info(`Using fallback data due to service unavailable for ${serviceName}`);
      return fallbackData;
    }
  ...
  private async handleFinalFailure<T>(
    ...
    console.error(`Final failure for service ${serviceName} after ${this.maxRetries} attempts:`, error.message);
    ...
    if (cacheKey && useStaleOnError) {
      const cachedData = extensionCache.getStale<T>(cacheKey, maxStaleAge);
      if (cachedData) {
        console.info(`Using stale cached data as last resort for ${serviceName}`);
        return cachedData;
      }
    }
    ...
    // Use fallback data if available
    if (fallbackData !== undefined) {
      console.info(`Using fallback data as last resort for ${serviceName}`);
      return fallbackData;
    }
