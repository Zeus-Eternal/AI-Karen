...
/**
 * Modern responsive utilities using CSS Grid and Flexbox
 * Replaces legacy responsive hooks with native CSS solutions
 */
...
/**
 * Generate responsive CSS classes using modern CSS Grid
...
export function generateResponsiveGrid(columns: {
  ...
  // Default columns
  if (columns.default) {
    classes.push(`karen-grid-cols-${columns.default}`);
  }

  // Responsive columns
  Object.entries(columns).forEach(([breakpoint, cols]) => {
    if (breakpoint !== 'default' && cols) {
      classes.push(`karen-grid-cols-${breakpoint}-${cols}`);
    }
  });
 ...
 * @param config - Flexbox configuration
 * @returns CSS class string
 */
export function generateResponsiveFlex(config: {
  direction?: 'row' | 'col';
  wrap?: boolean;
  ...
  const classes = ['karen-flex'];

  // Direction
  if (config.direction) {
    classes.push(config.direction === 'row' ? 'karen-flex-row' : 'karen-flex-col');
  }

  // Wrap
  if (config.wrap) {
    classes.push('karen-flex-wrap');
  }

  // Justify content
  if (config.justify) {
    classes.push(`karen-justify-${config.justify}`);
  }

  // Align items
  if (config.align) {
    classes.push(`karen-items-${config.align}`);
  }

  // Gap
  if (config.gap) {
    classes.push(`karen-gap-${config.gap}`);
  }
 ...
 * @returns CSS class string
 */
export function generateResponsiveContainer(size?: 'sm' | 'md' | 'lg' | 'xl' | '2xl'): string {
  const baseClass = 'karen-container';
  return size ? `${baseClass} karen-container-${size}` : baseClass;
}
 ...
 * @returns boolean
 */
export function isBreakpointActive(breakpoint: keyof ResponsiveConfig['breakpoints']): boolean {
  if (typeof window === 'undefined') return false;
  
  const width = window.innerWidth;
  return width >= responsiveConfig.breakpoints[breakpoint];
}

/**
 * Get current breakpoint
 * @returns current breakpoint or null
 */
export function getCurrentBreakpoint(): keyof ResponsiveConfig['breakpoints'] | null {
  ...
  const activeBreakpoints = Object.entries(breakpoints)
    ...
    .sort((a, b) => b[1] - a[1]); // Sort by width descending
  
  return activeBreakpoints.length > 0 ? (activeBreakpoints[0][0] as keyof ResponsiveConfig['breakpoints']) : null;
}
...
export function generateResponsiveSpacing(spacing: {
  ...
  // Padding
  if (spacing.padding) {
    if (spacing.padding.x) {
      classes.push(`px-${spacing.padding.x}`);
    }
    if (spacing.padding.y) {
      classes.push(`py-${spacing.padding.y}`);
    }
    if (spacing.padding.top) {
      classes.push(`pt-${spacing.padding.top}`);
    }
    if (spacing.padding.right) {
      classes.push(`pr-${spacing.padding.right}`);
    }
    if (spacing.padding.bottom) {
      classes.push(`pb-${spacing.padding.bottom}`);
    }
    if (spacing.padding.left) {
      classes.push(`pl-${spacing.padding.left}`);
    }
  }
  ...
  if (spacing.margin) {
    if (spacing.margin.x) {
      classes.push(`mx-${spacing.margin.x}`);
    }
    if (spacing.margin.y) {
      classes.push(`my-${spacing.margin.y}`);
    }
    if (spacing.margin.top) {
      classes.push(`mt-${spacing.margin.top}`);
    }
    if (spacing.margin.right) {
      classes.push(`mr-${spacing.margin.right}`);
    }
    if (spacing.margin.bottom) {
      classes.push(`mb-${spacing.margin.bottom}`);
    }
    if (spacing.margin.left) {
      classes.push(`ml-${spacing.margin.left}`);
    }
  }
...
/**
 * Modern responsive hook using ResizeObserver
 * Replaces legacy window resize listeners
 */
export function useResponsive() {
  const [currentBreakpoint, setCurrentBreakpoint] = useState<keyof ResponsiveConfig['breakpoints'] | null>(null);
  ...
  return {
    ...
    isSmallScreen: viewportSize.width < responsiveConfig.breakpoints.md,
    isMediumScreen: viewportSize.width >= responsiveConfig.breakpoints.md && viewportSize.width < responsiveConfig.breakpoints.lg,
    isLargeScreen: viewportSize.width >= responsiveConfig.breakpoints.lg,
  };
}
 ...
 * @returns CSS class string
 */
export function generateResponsiveImage(config: {
  ...
  // Aspect ratio
  if (config.aspect) {
    classes.push(`aspect-${config.aspect}`);
  }

  // Object fit
  if (config.objectFit) {
    classes.push(`object-${config.objectFit}`);
  }
 ...
 * @returns CSS class string
 */
export function generateResponsiveText(config: {
  ...
  // Text alignment
  if (config.align) {
    classes.push(`text-${config.align}`);
  }
...
/**
 * Legacy responsive utilities (deprecated)
 * @deprecated Use modern CSS Grid and Flexbox utilities instead
 */
export const legacyResponsiveUtils = {
  /**
   * @deprecated Use generateResponsiveGrid instead
   */
  useResponsiveColumns: () => {
    console.warn('useResponsiveColumns is deprecated. Use generateResponsiveGrid instead.');
    return { cols: 1, isMobile: false };
  },

  /**
   * @deprecated Use generateResponsiveFlex instead
   */
  useResponsiveFlex: () => {
    console.warn('useResponsiveFlex is deprecated. Use generateResponsiveFlex instead.');
    return { direction: 'row', wrap: false };
  },
   ...
   */
  useWindowSize: () => {
    console.warn('useWindowSize is deprecated. Use useResponsive hook instead.');
