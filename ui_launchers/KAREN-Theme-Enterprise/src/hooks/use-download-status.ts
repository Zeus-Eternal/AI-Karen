...
import { errorHandler, showSuccess, showInfo, showWarning } from '@/lib/errorHandler';
...
export function useDownloadStatus(): DownloadStatusHookReturn {
  ...
  const cancelDownload = useCallback(async (taskId: string) => {
    try {
      ...
      errorHandler.handleError(error, { operation: "download" });
      throw error;
    }
  }, [backend]);
  ...
  const pauseDownload = useCallback(async (taskId: string) => {
    try {
      ...
      errorHandler.handleError(error, { operation: "download", additionalData: { taskId } });
      throw error;
    }
  }, [backend]);
  ...
  const retryDownload = useCallback(async (taskId: string) => {
    ...
    try {
      ...
      errorHandler.handleError(error, { operation: "download", additionalData: { modelId: task?.modelId || "unknown model" } });
      throw error;
    }
