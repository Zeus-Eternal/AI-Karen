...
export const HookProvider: FC<HookProviderProps> = ({ children }) => {
      ...
      try {
        ...
        if (hook.conditions && Object.keys(hook.conditions).length > 0) {
          const conditionsMet = Object.entries(hook.conditions).every(([key, value]) => {
            return (context as Record<string, unknown>)[key] === value ||
                   (userContext as Record<string, unknown> | undefined)?.[key] === value;
          });
  ...
  const registerGridHook = useCallback((
    ...
    return registerHook(`grid_${gridId}_${event}`, (context: unknown, userContext?: unknown) => {
      return handler(context as Record<string, unknown>);
    }, {
      sourceType: 'ui',
      conditions: { gridId }
    });
  ...
  const registerChartHook = useCallback((
    ...
    return registerHook(`chart_${chartId}_${event}`, (context: unknown, userContext?: unknown) => {
      return handler(context as Record<string, unknown>);
    }, {
      sourceType: 'ui',
      conditions: { chartId }
  ...
  const registerChatHook = useCallback((
    ...
    return registerHook(`chat_${event}`, (context: unknown, userContext?: unknown) => {
      return handler(context as Record<string, unknown>);
    }, {
      sourceType: 'ui',
