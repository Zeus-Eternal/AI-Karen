  ...
  try {
    ...
    return {
      ...
      user: {
        ...
        roles: Array.isArray(userData.roles) ? userData.roles as RoleName[] : [],
        tenant_id: String(userData.tenant_id ?? ''),
        role: String(userData.role ?? RBACService.getInstance().getHighestRole(Array.isArray(userData.roles) ? userData.roles as RoleName[] : [])),
      },
      responseTime,
      retryCount: result.retryCount ?? 0,
      databaseConnectivity,
