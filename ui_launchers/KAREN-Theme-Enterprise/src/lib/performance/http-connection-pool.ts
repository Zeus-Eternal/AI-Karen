...
export class HttpConnectionPool {
  ...
  async request(
    ...
    try {
      ...
      if (connection) {
        ...
        this.metrics.activeConnections = Math.max(this.metrics.activeConnections - 1, 0);
  ...
  private processQueuedRequests(): void {
    if (this.isShuttingDown || this.requestQueue.length === 0 || this.queueProcessingScheduled) {
      return;
    }
    ...
    setTimeout(() => {
      ...
      void this.dispatchQueuedRequest();
    }, 0);
  }

  private async dispatchQueuedRequest(): Promise<void> {
    ...
    try {
      const response = await this.request(queuedRequest.url, queuedRequest.options, true);
