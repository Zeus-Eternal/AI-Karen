...
export class DatabaseQueryOptimizer {
  ...
  /**
   * High level helper for authentication operations that uses query optimization.
   */
  async authenticateUser(email: string, passwordHash: string): Promise<unknown> {
    const query = 'SELECT * FROM users WHERE email = $1 AND password_hash = $2';
    return this.executeAuthQuery(query, [email, passwordHash]);
  }

  /**
   * Helper to lookup users by id while benefiting from caching.
   */
