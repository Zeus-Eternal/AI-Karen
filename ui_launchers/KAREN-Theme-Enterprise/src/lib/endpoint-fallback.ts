...
export class EndpointFallbackService {
  ...
  // Removed endpoint tester dependency for production cleanup
  ...
  private async performHealthChecks(): Promise<void> {
    ...
    const healthCheckPromises = allEndpoints.map(async (endpoint) => {
      try {
        // Simplified health check without external dependency
        ...
        const result: EndpointTestResult = {
          ...
          responseTime: 0, // Could calculate with performance.now() if needed
