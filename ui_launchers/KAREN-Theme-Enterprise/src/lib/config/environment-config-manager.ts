...
export class EnvironmentConfigManager {
  ...
  private determinePrimaryUrl(): string {
    ...
    // Use standardized variables first, then fall back to environment-based generation
    if (standardizedUrls.length > 0) {
      return this.normalizeUrl(standardizedUrls[0]);
    }
    ...
    switch (this.environment.networkMode) {
      case 'container': {
        ...
        return `${protocol}://${containerHost}:${containerPort}`;
      }
      case 'external': {
        const externalHost = this.getEnvVar('KAREN_EXTERNAL_HOST', this.environment.detectedHostname || 'localhost');
        const externalPort = this.getEnvVar('KAREN_EXTERNAL_BACKEND_PORT', isProduction ? '443' : '8000');
        const protocol = isProduction ? 'https' : 'http';
        return `${protocol}://${externalHost}:${externalPort}`;
      }
      case 'localhost':
      default:
        return isProduction ? 'https://api.yourdomain.com' : 'http://localhost:8000';
    }
  }
  /**
