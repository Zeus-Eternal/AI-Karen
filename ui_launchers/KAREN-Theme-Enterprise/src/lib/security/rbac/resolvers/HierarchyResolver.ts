...
import { RoleName, RoleHierarchy, RoleHierarchyItem } from '../types';
import {
  RoleNotFoundError,
...
export class HierarchyResolver {
  ...
  public getHierarchy(): RoleHierarchy {
    ...
    for (const roleName of allRoles) {
      hierarchy[roleName] = ROLE_HIERARCHY_LEVELS[roleName] || 0;
    }
  ...
  public getParentRole(roleName: RoleName): RoleName | null {
    ...
    // Find parent by checking which role has this role as a child
    for (const [potentialParent, level] of Object.entries(hierarchy)) {
      const childRoles = this.getChildRoles(potentialParent as RoleName);
      if (childRoles.includes(roleName)) {
        return potentialParent as RoleName;
      }
    }
    return null;
  }
  ...
  public getChildRoles(roleName: RoleName): RoleName[] {
    ...
    return this.getChildRoles(roleName);
  }
  ...
  public getHighestRole(): RoleName | null {
    ...
    for (const [roleName, roleInfo] of Object.entries(hierarchy)) {
      if (roleInfo > highestLevel) {
        highestLevel = roleInfo;
  ...
  public getLowestRole(): RoleName | null {
    ...
    for (const [roleName, roleInfo] of Object.entries(hierarchy)) {
      if (roleInfo < lowestLevel) {
        lowestLevel = roleInfo;
