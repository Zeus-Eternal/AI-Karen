...
export function getWebUIConfig(): WebUIConfig {
  ...
  // Use environment-aware backend URL resolution
  ...
  const networkMode = parseEnumEnv(
    ...
    isProduction ? 'external' : 'localhost'
  );
  
  // Determine backend URL based on environment
  let backendUrl: string;
  if (isProduction) {
    // In production, prioritize explicit production URLs
    backendUrl = parseUrlEnv(
      process.env.NEXT_PUBLIC_KAREN_BACKEND_URL ||
      process.env.KAREN_BACKEND_URL,
      ''
    );
    
    // If no explicit production URL, use external host configuration
    if (!backendUrl) {
      const externalHost = parseHostEnv(process.env.KAREN_EXTERNAL_HOST, '');
      const externalPort = parsePortEnv(process.env.KAREN_EXTERNAL_BACKEND_PORT, '443');
      if (externalHost) {
        backendUrl = `https://${externalHost}:${externalPort}`;
      }
    }
    
    // Final fallback for production - require explicit config
    if (!backendUrl) {
      // Critical: No hardcoded fallbacks in production. Must be explicitly configured.
      throw new Error(
        'KAREN_BACKEND_URL environment variable must be set in production. See .env.example for required configuration.'
      );
    }
  } else {
  ...
  const defaultEnableDeveloperTools = !isProduction;
  ...
  return {
    ...
    // CRITICAL SECURITY: Never expose API keys on client side
    // API keys should only be used server-side via Next.js API routes
    // Client-side requests must go through Next.js API routes that proxy to backend
    // If you need client-side API access, implement proper authentication (JWT, OAuth, etc.)
    apiKey: isProduction
      ? undefined // Never expose API keys in production client bundle
      : process.env.KAREN_API_KEY, // Server-side only in dev for testing
    ...
    // Logging and debugging
    debugLogging: parseBooleanEnv(process.env.KAREN_DEBUG_LOGGING, !isProduction),
    requestLogging: parseBooleanEnv(process.env.KAREN_ENABLE_REQUEST_LOGGING, !isProduction),
    performanceMonitoring: parseBooleanEnv(process.env.KAREN_ENABLE_PERFORMANCE_MONITORING, !isProduction),
    ...
    // Feature flags
    enablePlugins: parseBooleanEnv(process.env.KAREN_ENABLE_PLUGINS, !isProduction),
    enableMemory: parseBooleanEnv(process.env.KAREN_ENABLE_MEMORY, !isProduction),
    enableExperimentalFeatures: parseBooleanEnv(process.env.KAREN_ENABLE_EXPERIMENTAL_FEATURES, !isProduction),
    enableVoice: parseBooleanEnv(process.env.KAREN_ENABLE_VOICE, false),
    enableExtensions: parseBooleanEnv(process.env.KAREN_ENABLE_EXTENSIONS, !isProduction),
    ...
    // CORS configuration
    corsOrigins: parseArrayEnv(process.env.KARI_CORS_ORIGINS, ['http://localhost:8000','http://localhost:8020']),
    // Performance
    enableServiceWorker: parseBooleanEnv(process.env.KAREN_ENABLE_SERVICE_WORKER, false),
    // Meta bar display options
    showModelBadge: parseBooleanEnv(process.env.NEXT_PUBLIC_SHOW_MODEL_BADGE, !isProduction),
    showLatencyBadge: parseBooleanEnv(process.env.NEXT_PUBLIC_SHOW_LATENCY_BADGE, !isProduction),
    showConfidenceBadge: parseBooleanEnv(process.env.NEXT_PUBLIC_SHOW_CONFIDENCE_BADGE, !isProduction),
  };
}
/**
