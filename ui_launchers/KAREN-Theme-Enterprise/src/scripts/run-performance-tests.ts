...
class PerformanceTestRunner {
  ...
  private safeParseVitestJson(output: string): VitestJsonReport | null {
    ...
    try {
      ...
      const payload = firstBrace >= 0 ? output.slice(firstBrace) : output;
      const parsed = JSON.parse(payload);
      if (parsed && typeof parsed === "object") {
        const candidate = parsed as Record<string, unknown>;
        const numFailedTests = candidate.numFailedTests;
        const numFailedTestSuites = candidate.numFailedTestSuites;
        if (
          typeof numFailedTests === "number" &&
          typeof numFailedTestSuites === "number"
        ) {
          return {
            numFailedTests,
            numFailedTestSuites,
  ...
  private formatError(error: unknown): string {
    ...
    if (error && typeof error === "object") {
      ...
      return (
        errLike.message ??
        errLike.stderr?.toString?.() ??
        errLike.stdout?.toString?.() ??
        "Unknown error"
      );
    }
  ...
  private extractMetricsFromOutput(output: string): Record<string, unknown> {
