...
interface AccessibilityRunResult {
  score: number;
  violations: number;
  passes: number;
  incomplete: number;
...
class AccessibilityCIRunner {
  ...
  private async runTests(): Promise<AccessibilityResult[]> {
  ...
  private async runPlaywrightTest(_url: string): Promise<AccessibilityRunResult> {
  ...
  private async generateReports(results: AccessibilityResult[]): Promise<void> {
    const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
    // Generate JSON report
    const jsonReport: AccessibilityReport = {
      timestamp: new Date().toISOString(),
      config: this.config,
      results,
    ...
    const junitPath = join(
      this.config.outputDir,
      `accessibility-junit-${timestamp}.xml`
    );
    writeFileSync(junitPath, junitReport);
  }
  private generateSummary(results: AccessibilityResult[]): AccessibilitySummary {
    ...
    const averageScore =
      successful.length > 0
        ? successful.reduce((sum, r) => sum + (r.score ?? 0), 0) /
          successful.length
        : 0;
    return {
      ...
      passed:
        failed.length === 0 &&
        totalViolations <= this.config.maxViolations &&
        averageScore >= this.config.minScore,
    };
  }
...
<body>
    <div class="container">
        ...
        <div class="summary">
            ...
            <div class="metric">
                <div class="metric-value">${report.summary.successful}/${
                ...
                <div class="result-item">
                    ...
                    ${
                      result.success
                        ...
                        <div>Score: <span class="score">${result.score ?? 0}/100</span></div>
                        <div>Violations: ${result.violations ?? 0}</div>
                        <div>Passes: ${result.passes ?? 0}</div>
                    `
                        : `
  ...
  private generateJUnitReport(results: AccessibilityResult[]): string {
    const testSuites = results
      .map((result) => {
        const testName = `accessibility-${result.url.replace(
          /[^a-zA-Z0-9]/g,
          "-"
        )}`;
        if (result.success) {
...
<testsuites name="accessibility" tests="${results.length}" failures="${
      results.filter((r) => !r.success).length
    }" time="1">
    <testsuite name="accessibility" tests="${results.length}" failures="${
      results.filter((r) => !r.success).length
    }" time="1">
        ${testSuites}
    </testsuite>
  ...
  private evaluateResults(results: AccessibilityResult[]): boolean {
    const failed = results.filter((r) => !r.success);
    const successful = results.filter((r) => r.success);
    if (failed.length > 0) {
    ...
    if (totalViolations > this.config.maxViolations) {
      console.log(
        `❌ Too many violations: ${totalViolations} (max: ${this.config.maxViolations})`
      );
      return false;
    }
    const averageScore =
    ...
    if (averageScore < this.config.minScore) {
      console.log(
        `❌ Score too low: ${Math.round(averageScore)} (min: ${
