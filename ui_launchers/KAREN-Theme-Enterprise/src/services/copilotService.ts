...
export class CopilotService {
  ...
  private getBackendUrl(): string {
    // Check if we're in the browser and use the current origin
    if (typeof window !== 'undefined') {
      return window.location.origin;
    }
    // Fallback for server-side rendering
    return process.env.NEXT_PUBLIC_API_URL || process.env.NEXT_PUBLIC_KAREN_BACKEND_URL || 'http://localhost:8001';
  }
