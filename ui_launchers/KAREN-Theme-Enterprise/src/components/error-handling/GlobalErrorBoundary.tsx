...
export class GlobalErrorBoundary extends React.Component<Props, State> {
  ...
  private getBreadcrumbs(): Array<{ timestamp: string; category: 'navigation' | 'user' | 'http' | 'console' | 'dom'; message: string; level: 'info' | 'warning' | 'error'; data?: Record<string, unknown> }> {
    try {
      if (typeof window === "undefined") return [];
      
      // Get breadcrumbs from the error reporting service
      const stored = window.sessionStorage.getItem('error_breadcrumbs');
      if (!stored) return [];
      
      const breadcrumbs = JSON.parse(stored);
      return Array.isArray(breadcrumbs) ? breadcrumbs.slice(-10) : [];
    } catch {
      return [];
    }
  }
