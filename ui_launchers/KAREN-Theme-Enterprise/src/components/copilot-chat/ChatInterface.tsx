...
import { ModelServiceErrorBoundary } from '../ModelServiceErrorBoundary';
import {
  MessageSquare,
  Workflow,
  FileText,
...
export const VirtualizedMessageList: React.FC<{ messages: CopilotMessage[] }> = ({ messages }) => {
  ...
  const containerRef = useRef<HTMLDivElement>(null);
  const throttledScrollHandlerRef = useRef<((event: React.UIEvent<HTMLDivElement>) => void) | null>(null);
  ...
  // Create and cleanup throttled scroll handler
  useEffect(() => {
    // Create throttled handler with proper typing
    let timeoutId: NodeJS.Timeout | null = null;
    
    const throttledHandler = (event: React.UIEvent<HTMLDivElement>) => {
      if (timeoutId) return;
    ...
    throttledScrollHandlerRef.current = throttledHandler as any;
    ...
    return () => {
      ...
      throttledScrollHandlerRef.current = null;
    };
  }, []);
  ...
  const handleScroll = useCallback((event: React.UIEvent<HTMLDivElement>) => {
    if (throttledScrollHandlerRef.current) {
      throttledScrollHandlerRef.current(event);
    }
  }, []);
          ...
          <div className="flex-1 flex flex-col">
            <Tabs value={activePanel} className="flex-1 flex flex-col">
              ...
              <TabsContent value="chat" className="flex-1 flex flex-col p-0 m-0 data-[state=active]:flex">
                <ModelServiceErrorBoundary
                  onError={(error, errorType) => {
                    console.error(`Chat Interface error (${errorType}):`, error);
