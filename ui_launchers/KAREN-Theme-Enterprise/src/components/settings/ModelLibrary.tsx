...
export default function ModelLibrary() {
  ...
  const loadModels = useCallback(async () => {
    try {
      ...
      handleError(error, { context: 'load model library' });
    } finally {
      setLoading(false);
    }
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      handleError(error, { context: 'refresh model library' });
    } finally {
      setRefreshing(false);
    }
  };
  ...
  const handleModelAction = async (modelId: string, action: ModelAction) => {
    ...
    if (action === 'delete' || action === 'cancel') {
      const dialogConfig = { title: action, message: `Are you sure you want to ${action} ${modelName}?` };
      setConfirmationDialog({
        open: true,
        title: dialogConfig.title,
        message: dialogConfig.message,
        confirmText: dialogConfig.title === 'delete' ? 'Delete' : 'Cancel',
        cancelText: dialogConfig.title === 'delete' ? 'Cancel' : 'OK',
        variant: dialogConfig.title === 'delete' ? 'destructive' : 'default',
        loading: false,
  ...
  const executeModelAction = async (modelId: string, action: ModelAction, modelName: string) => {
    ...
    try {
      ...
      handleError(error, { context: `${action} ${modelName}` });
    } finally {
