...
import { handleError, type ErrorInfo } from '@/lib/errorHandler';
...
type ExtendedErrorLike = Error & {
  detail?: ErrorInfo;
  response?: { status?: number; data?: ErrorInfo };
  code?: string;
  status?: number;
};
...
function ModelRecommendations({ provider }: ModelRecommendationsProps) {
  ...
  const fetchRecommendations = useCallback(async () => {
    ...
    try {
      ...
      const info = handleError(toExtendedError(error), { operation: 'fetchRecommendations' });
      toast({
        title: info.title,
...
export default function ProviderManagement({
  ...
  const runHealthCheck = async () => {
    try {
      ...
      const info = handleError(toExtendedError(error), { operation: 'runHealthCheck' });
      toast({
  ...
  const discoverProviderModels = async (providerName: string, forceRefresh: boolean = false) => {
    try {
      ...
      const info = handleError(toExtendedError(error), { operation: 'discoverProviderModels' });
      toast({
  ...
  const testProvider = async (providerName: string) => {
    try {
      ...
      const info = handleError(toExtendedError(error), { operation: 'testProvider' });
      toast({
