...
export default function JobCenter({
  ...
  const loadJobs = useCallback(async () => {
    try {
      ...
      if (!jobs.length) {
        const info = handleError(error, { context: "loadJobs" });
        toast({
          variant: "destructive",
          title: info.title,
  ...
  const controlJob = async (jobId: string, action: "pause" | "resume" | "cancel") => {
    try {
      ...
      const info = handleError(error, { context: `${action}Job` });
      toast({
        variant: "destructive",
        title: info.title,
        description: info.message,
  ...
  const deleteJob = async (jobId: string) => {
    try {
      ...
      const info = handleError(error, { context: "deleteJob" });
      toast({
        variant: "destructive",
        title: info.title,
        description: info.message,
  ...
  const clearCompletedJobs = async () => {
    ...
    try {
      ...
      const info = handleError(error, { context: "clearJobs" });
      toast({
        variant: "destructive",
        title: info.title,
        description: info.message,
