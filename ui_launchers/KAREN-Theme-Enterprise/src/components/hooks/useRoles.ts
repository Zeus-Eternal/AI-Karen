...
import { RoleName, RoleDefinition, UseRolesReturn, Role } from '../../lib/security/rbac/types';
...
export const useRoles = (): UseRolesReturn => {
  ...
  const [roleDefinitions, setRoleDefinitions] = useState<Record<RoleName, RoleDefinition>>({} as Record<RoleName, RoleDefinition>);
  ...
  const mappedRoles = roles.map((roleName) => ({
    id: roleName,
    name: roleName,
    displayName: roleName.charAt(0).toUpperCase() + roleName.slice(1).replace('_', ' '),
    description: `${roleName} role`,
    permissions: roleDefinitions[roleName]?.permissions || [],
    level: roleDefinitions[roleName]?.level || 0
  } as Role));
