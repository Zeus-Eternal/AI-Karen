...
const RechartsPrimitive = {
  ResponsiveContainer: ({ children }: RechartsFallbackComponentProps) => (
    <div>{children}</div>
  ),
  Tooltip: ({ children }: RechartsFallbackComponentProps) => (
    <div>{children}</div>
  ),
  Legend: ({ children }: RechartsFallbackComponentProps) => (
    <div>{children}</div>
  ),
} as const;
      ...
      >
        ...
        <div className="grid gap-1.5">
          {payload.map((item, index) => {
            ...
            const key = `${
              nameKey || itemObj.name || itemObj.dataKey || "value"
            }`;
            const itemConfig = getPayloadConfigFromPayload(config, item, key);
            const indicatorColor =
              color || itemObj.payload?.fill || itemObj.color;
