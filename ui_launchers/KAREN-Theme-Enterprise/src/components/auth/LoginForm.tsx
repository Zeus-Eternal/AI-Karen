...
import { debugLog, getErrorDetails } from '@/lib/environment-utils';
...
const TWO_FACTOR_ERROR_REGEX =
  /two[- ]?factor|2fa|mfa|one[- ]?time|otp|totp/i;
...
function isTwoFactorError(error: unknown, message?: string | null): boolean {
  ...
  // If the server message indicates an account lockout or too many attempts,
  // do not treat it as a two-factor prompt even if it contains words like
  // "authentication" — those can produce false positives.
  if (/too many|locked|lockout|attempts?/i.test(text)) {
    return false;
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isTwoFactorError(error, errorMessage)) {
        ...
        debugLog.warn('Two-factor authentication flow engaged', errorMessage ?? error);
        return;
      }
                ...
                {lockoutInfo && (
                  <div className="bg-white dark:bg-gray-800 p-3 rounded border border-red-200 dark:border-red-800 mb-2">
                    ...
                    <p className="text-xs text-gray-700 dark:text-gray-300 mt-1">
                      Wait for the lockout to expire or contact your system administrator.
                    </p>
                  </div>
                )}
                {typeof authState.error === 'string' && authState.error.includes('401') && (
                  <div className="bg-white dark:bg-gray-800 p-3 rounded border border-red-200 dark:border-red-800 mb-2">
                    ...
                    <ul className="text-xs text-gray-700 dark:text-gray-300 space-y-1 list-disc list-inside">
                      ...
                      <li>Authentication service may be unavailable</li>
                    </ul>
                  </div>
                )}
