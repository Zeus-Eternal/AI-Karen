...
// AutoSizer component for react-window integration
...
const AutoSizer: React.FC<AutoSizerProps> = ({
  ...
  const [dimensions, setDimensions] = useState<AutoSizerChildrenArgs>({
    height: defaultHeight,
    width: disableWidth ? 0 : defaultWidth
  });
    ...
    const resizeObserver = new ResizeObserver((entries) => {
      if (entries.length > 0) {
        ...
        setDimensions({
          ...
          width: disableWidth ? 0 : (width || defaultWidth)
        });
      }
    });
  ...
  }, [disableWidth, defaultHeight, defaultWidth]);
