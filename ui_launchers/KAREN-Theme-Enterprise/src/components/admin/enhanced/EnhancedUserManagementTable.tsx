...
export function EnhancedUserManagementTable({
  ...
  useKeyboardNavigation(tableRef, {
    ...
    focusableSelector: 'button, input[type="checkbox"], a, select',
    onEscape: () => {
      setEditingUser(null);
  ...
  const loadUsers = useCallback(
    async (retryCount = 0) => {
      try {
        ...
        const data: AdminApiResponse<PaginatedResponse<User>> =
          await response.json();
        if (!data.success || !data.data) {
          throw AdminErrorHandler.createError(
            "SYSTEM_SERVER_ERROR",
        ...
        announce(
          `Loaded ${data.data.data?.length || 0} users. Page ${pagination.page} of ${
        ...
        if (AdminErrorHandler.shouldRetry(adminError, retryCount + 1)) {
          const delay = AdminErrorHandler.getRetryDelay(
            adminError,
            retryCount + 1
          );
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      ...
      onUserUpdated();
      announce(
        `User ${deletingUser.email} deleted successfully`,
        "assertive"
      );
    } catch (err) {
      const adminError =
  ...
  const handleToggleUserStatus = async (user: User) => {
    try {
      ...
      setError(adminError);
      announce(
        `Failed to update user status: ${adminError.message}`,
        "assertive"
      );
    } finally {
      setOperationLoading(null);
  ...
  const renderTableRow = (user: User) => (
    ...
    >
      ...
      <td className="px-6 py-4 whitespace-nowrap" role="gridcell">
        <div className="text-sm font-medium text-gray-900 md:text-base lg:text-lg">
          {user.email}
        </div>
      </td>
      <td className="px-6 py-4 whitespace-nowrap" role="gridcell">
      ...
      <td
        className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 md:text-base lg:text-lg"
        role="gridcell"
      >
      ...
      <td
        className="px-6 py-4 whitespace-nowrap text-sm font-medium md:text-base lg:text-lg"
        role="gridcell"
      >
            ...
            {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => {
              ...
              return (
                <Button
                  ...
                  variant={pageNum === pagination.page ? "secondary" : "outline"}
                >
                  {pageNum}
                </Button>
              );
    ...
    >
      ...
      {editingUser && (
        <UserEditModal
          ...
          onUserUpdated={handleUserUpdatedLocal}
        />
      )}
