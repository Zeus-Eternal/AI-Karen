...
import DynamicMemoryGrid from '@/components/memory/DynamicMemoryGrid';
import { ModelServiceErrorBoundary } from '@/components/ModelServiceErrorBoundary';
...
export default function MemoryLabPage() {
  ...
  // Fetch real memory stats from API
  React.useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // In production, show zeros if API is unavailable
          console.warn('Memory stats API unavailable');
        }
      } catch (error) {
                    ...
                    {user && (
                      <ModelServiceErrorBoundary
                        onError={(error, errorType) => {
                          console.error(`Memory Grid error (${errorType}):`, error);
