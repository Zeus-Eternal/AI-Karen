...
import { isDevelopment } from '@/lib/environment-utils';
...
function buildSampleResponse(): NormalizedExtensionsResponse {
  if (!isDevelopment()) {
    // In production, return empty response instead of sample data
    return {
      extensions: {},
      total: 0,
      message: 'Extensions service unavailable',
    };
  }
...
function buildFallbackResponse() {
  ...
  return NextResponse.json(response, {
    ...
    headers: {
      ...
      'X-Extensions-Fallback': isDevelopment() ? 'sample-data' : 'unavailable',
    },
  });
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (response.status === 403) {
      ...
      console.warn('[GET /api/extensions] Backend returned forbidden error, using fallback', {
    ...
    if (response.status === 401) {
      // Pass through 401 errors to trigger proper authentication flow
      console.warn('[GET /api/extensions] Backend returned authentication error', {
        status: response.status,
      });
