...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Extract LNM selection result from response context
    let selectionResult = { success: false, error: 'No response from backend' };
    if (data && typeof data === 'object' && 'context' in data) {
      const context = (data as any).context;
      if (Array.isArray(context)) {
        const lnmContext = context.find((item: any) => item.id === 'lnm');
        if (lnmContext && lnmContext.text) {
          try {
            selectionResult = JSON.parse(lnmContext.text);
          } catch {
            selectionResult = { success: false, error: 'Invalid response format' };
          }
        }
      }
    }
    ...
    // Fallback response for LNM selection
    const fallbackSelection = {
      success: true,
      modelId: (body as any)?.modelId || 'gpt-4',
      message: 'LNM selected successfully (fallback mode)',
      timestamp: new Date().toISOString()
    };

    return NextResponse.json(fallbackSelection, {
      status: 200,
      headers: {
        'Cache-Control': 'no-store',
        'X-Fallback': 'true',
