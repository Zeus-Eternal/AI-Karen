...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Extract LNM list from the response context
    let lnmList = [];
    if (data && typeof data === 'object' && 'context' in data) {
      ...
      if (Array.isArray(context)) {
        ...
        if (lnmContext && lnmContext.text) {
          try {
            lnmList = JSON.parse(lnmContext.text);
          } catch {
            lnmList = [];
          }
        }
    ...
    return NextResponse.json(lnmList, {
    ...
    // Fallback response for LNM list
    const fallbackLNMs = [
      {
        id: 'gpt-4',
        name: 'GPT-4',
    ...
    return NextResponse.json(fallbackLNMs, {
      status: 200,
      headers: {
        'Cache-Control': 'no-store',
        'X-Fallback': 'true',
