...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Extract plugins from the response context
    let plugins = [];
    if (data && typeof data === 'object' && 'context' in data) {
      const context = (data as any).context;
      if (Array.isArray(context)) {
        const pluginsContext = context.find((item: any) => item.id === 'plugins');
        if (pluginsContext && pluginsContext.text) {
          try {
            plugins = JSON.parse(pluginsContext.text);
          } catch {
            plugins = [];
          }
        }
    ...
    // Fallback response for plugins list
