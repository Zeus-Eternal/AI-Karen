...
export async function POST(request: NextRequest) {
  ...
  let parsedBody: MemorySearchRequest | null = null;
  try {
    parsedBody = JSON.parse(bodyText) as MemorySearchRequest;
  } catch {
    parsedBody = null;
  }
  ...
  for (const base of candidates) {
    const url = withBackendPath('/api/memory/search', base);
  ...
  if (process.env.NODE_ENV !== 'production') {
    console.warn(
      '[api/memory/search] Falling back to empty response',
      lastError instanceof Error ? lastError.message : lastError
    );
  }

  const fallback = buildFallbackResponse(parsedBody?.query || '');
  return NextResponse.json(fallback, {
    status: 200,
    headers: {
      'Cache-Control': 'no-store',
      'X-Fallback': 'memory-search',
      'X-Fallback-Reason':
