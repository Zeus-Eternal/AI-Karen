...
export function generateStaticParams() {
  ...
  return [
    ...
    { path: ['copilot', 'start'] },
    { path: ['copilot', 'assist'] },
    { path: ['copilot', 'lnm', 'list'] },
    { path: ['copilot', 'lnm', 'select'] },
    { path: ['copilot', 'plugins', 'list'] },
    { path: ['copilot', 'plugins', 'execute'] },
    { path: ['copilot', 'security', 'context'] },
    { path: ['dev-status'] },
    { path: ['extensions'] },
    { path: ['extensions', 'background-tasks'] },
    { path: ['extensions', 'system', 'health'] },
    { path: ['files'] },
    { path: ['files', 'upload'] },
    { path: ['models', 'all'] },
...
async function handleRequest(request: NextRequest, { params }: { params: Promise<{ path: string[] }> }) {
  try {
    ...
    // Don't use fallback for authentication errors (401) - these should be passed through to frontend
    // This allows the frontend to properly handle authentication failures and redirect to login
    if (response.status === 401) {
      // Just pass through the 401 response without any fallback
      console.log(`[API Proxy] Authentication required for ${resolvedParams.path.join('/')}, status: 401`);
    }
