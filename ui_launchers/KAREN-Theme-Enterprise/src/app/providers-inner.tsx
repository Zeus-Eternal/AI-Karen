...
import { debugLog } from '@/lib/environment-utils';
import { cleanupDiagnostics } from '@/lib/diagnostics/cleanup-diagnostics';
...
const DynamicCopilotProvider = React.lazy(() =>
  import('@/ai/copilot').catch(() => {
    debugLog.error('Failed to load @/ai/copilot module');
    // Return a module object with a default export that matches CopilotChatInterfaceProps
    return {
      default: ({ initialState, backendConfig, expertiseLevel, className }: {
        initialState?: any;
        backendConfig: { baseUrl: string; userId: string; sessionId: string; };
        expertiseLevel?: string;
        className?: string;
      }) => React.createElement('div', null, 'CopilotProvider not available')
    };
  })
);

const DynamicExtensionIntegrationProvider = React.lazy(() =>
  import('@/lib/extensions/extension-initializer').then(module => {
    // The module exports ExtensionIntegrationProvider as a named export, not default
    // We need to return a module with a default export that wraps the named export
    return {
      default: module.ExtensionIntegrationProvider
    };
  }).catch(() => {
    debugLog.error('Failed to load @/lib/extensions/extension-initializer module');
    // Return a module object with a default export that is a proper React component
    return {
      default: ({ children }: { children: React.ReactNode }) => React.createElement('div', null, 'ExtensionIntegrationProvider not available', children)
    };
  })
);

const DynamicAccessibilityEnhancementsProvider = React.lazy(() =>
  import('@/components/accessibility').then(module => {
    // The module exports AccessibilityEnhancementsProvider as a named export, not default
    // We need to return a module with a default export that wraps the named export
    return {
      default: module.AccessibilityEnhancementsProvider
    };
  }).catch(() => {
    debugLog.error('Failed to load @/components/accessibility module');
    // Return a module object with a default export that is a proper React component
    return {
      default: ({ children }: { children: React.ReactNode }) => React.createElement('div', null, 'AccessibilityEnhancementsProvider not available', children)
    };
  })
);

export function Providers({ children }: { children: React.ReactNode }) {
  debugLog.log('Providers: Starting to render');
  const { toast } = useToast();
  debugLog.log('Providers: useToast hook called successfully');
  ...
  React.useEffect(() => {
    ...
    // Run cleanup diagnostics in development
    if (process.env.NODE_ENV === 'development') {
      setTimeout(() => {
        console.log('🔍 Running KAREN Theme Cleanup Diagnostics...');
        const results = cleanupDiagnostics.runFullDiagnostics();
        cleanupDiagnostics.logResults();
        
        // Log critical issues to console for immediate visibility
        ...
        <GlobalErrorBoundary
          ...
          onError={(error, errorInfo) => {
            if (process.env.NODE_ENV !== 'production') {
              debugLog.error('Global error captured', error, errorInfo);
            }
          }}
        >
          <ErrorProvider
