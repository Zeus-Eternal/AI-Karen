...
class TestAuthService:
    ...
    async def test_auth_service_initialization_with_intelligence(
        self, 
        auth_config_with_intelligence: AuthConfig
    ):
    ...
    async def test_authenticate_user_success(self, auth_config: AuthConfig):
        ...
        with patch.object(auth_service.core_auth, 'authenticate_user', new_callable=AsyncMock) as mock_auth:
            mock_auth.return_value = mock_user_data
            
            with patch.object(auth_service.security_layer, 'check_rate_limit', new_callable=AsyncMock) as mock_rate_limit:
                mock_rate_limit.return_value = True
                
                with patch.object(auth_service.security_layer, 'record_successful_attempt', new_callable=AsyncMock) as mock_record:
    ...
    async def test_authenticate_user_invalid_credentials(self, auth_config: AuthConfig):
        ...
        with patch.object(auth_service.core_auth, 'authenticate_user', new_callable=AsyncMock) as mock_auth:
            ...
            with patch.object(auth_service.security_layer, 'check_rate_limit', new_callable=AsyncMock) as mock_rate_limit:
                ...
                with patch.object(auth_service.security_layer, 'record_failed_attempt', new_callable=AsyncMock) as mock_record:
    ...
    async def test_authenticate_user_with_intelligence_block(
        ...
        with patch.object(auth_service.security_layer, 'check_rate_limit', new_callable=AsyncMock):
            with patch.object(auth_service.core_auth, 'get_user_by_email', new_callable=AsyncMock) as mock_get_user:
                mock_get_user.return_value = UserData(user_id="test", email="test@example.com")
                
                with patch.object(auth_service.intelligence_layer, 'analyze_login_attempt', new_callable=AsyncMock) as mock_analyze:
    ...
    @pytest.mark.asyncio
    async def test_create_session_success(self, auth_config: AuthConfig, sample_user_data: UserData):
        ...
        with patch.object(auth_service.core_auth, 'create_session', new_callable=AsyncMock) as mock_create:
            mock_create.return_value = mock_session_data
            
            with patch.object(auth_service.security_layer, 'validate_session_security', new_callable=AsyncMock) as mock_validate:
                ...
                with patch.object(auth_service.security_layer, 'log_session_event', new_callable=AsyncMock) as mock_log:
    ...
    @pytest.mark.asyncio
    async def test_validate_session_success(self, auth_config: AuthConfig, sample_user_data: UserData):
        ...
        with patch.object(auth_service.core_auth, 'validate_session', new_callable=AsyncMock) as mock_validate:
            ...
            with patch.object(auth_service.core_auth.session_manager.store, 'get_session', new_callable=AsyncMock) as mock_get_session:
                ...
                with patch.object(auth_service.security_layer, 'validate_session_security', new_callable=AsyncMock) as mock_security:
    ...
    @pytest.mark.asyncio
    async def test_validate_session_security_failure(self, auth_config: AuthConfig, sample_user_data: UserData):
        ...
        with patch.object(auth_service.core_auth, 'validate_session', new_callable=AsyncMock) as mock_validate:
            ...
            with patch.object(auth_service.core_auth.session_manager.store, 'get_session', new_callable=AsyncMock) as mock_get_session:
                ...
                with patch.object(auth_service.security_layer, 'validate_session_security', new_callable=AsyncMock) as mock_security:
                    ...
                    with patch.object(auth_service, 'invalidate_session', new_callable=AsyncMock) as mock_invalidate:
    ...
    async def test_create_user_success(self, auth_config: AuthConfig):
        ...
        with patch.object(auth_service.security_layer, 'check_rate_limit', new_callable=AsyncMock) as mock_rate_limit:
            ...
            with patch.object(auth_service.core_auth, 'create_user', new_callable=AsyncMock) as mock_create:
                mock_create.return_value = mock_user_data
                
                with patch.object(auth_service.security_layer, 'log_auth_event', new_callable=AsyncMock) as mock_log:
    ...
    async def test_create_user_already_exists(self, auth_config: AuthConfig):
        ...
        with patch.object(auth_service.security_layer, 'check_rate_limit', new_callable=AsyncMock) as mock_rate_limit:
            ...
            with patch.object(auth_service.core_auth, 'create_user', new_callable=AsyncMock) as mock_create:
                mock_create.side_effect = UserAlreadyExistsError(email="existing@example.com")
    ...
    async def test_update_user_password_success(self, auth_config: AuthConfig):
        ...
        with patch.object(auth_service.core_auth, 'update_user_password', new_callable=AsyncMock) as mock_update:
            mock_update.return_value = True
            
            with patch.object(auth_service.core_auth, 'get_user_by_id', new_callable=AsyncMock) as mock_get_user:
    ...
    @pytest.mark.asyncio
    async def test_get_user_by_id(self, auth_config: AuthConfig, sample_user_data: UserData):
        """Test getting user by ID."""
        auth_service = AuthService(auth_config)
        
        with patch.object(auth_service.core_auth, 'get_user_by_id', new_callable=AsyncMock) as mock_get:
    ...
    @pytest.mark.asyncio
    async def test_get_user_by_email(self, auth_config: AuthConfig, sample_user_data: UserData):
        """Test getting user by email."""
        auth_service = AuthService(auth_config)
        
        with patch.object(auth_service.core_auth, 'get_user_by_email', new_callable=AsyncMock) as mock_get:
            mock_get.return_value = sample_user_data
    ...
    @pytest.mark.asyncio
    async def test_update_user_preferences(self, auth_config: AuthConfig, sample_user_data: UserData):
        ...
        with patch.object(auth_service.core_auth, 'get_user_by_id', new_callable=AsyncMock) as mock_get:
            ...
            with patch.object(auth_service.core_auth.db_client, 'update_user', new_callable=AsyncMock) as mock_update:
    ...
    @pytest.mark.asyncio
    async def test_create_password_reset_token(self, auth_config: AuthConfig, sample_user_data: UserData):
        ...
        with patch.object(auth_service.core_auth, 'get_user_by_email', new_callable=AsyncMock) as mock_get:
            ...
            with patch.object(auth_service.security_layer, 'check_rate_limit', new_callable=AsyncMock) as mock_rate_limit:
                ...
                with patch.object(auth_service.core_auth.token_manager, 'create_password_reset_token_with_storage', new_callable=AsyncMock) as mock_create_token:
    ...
    @pytest.mark.asyncio
    async def test_create_password_reset_token_user_not_found(self, auth_config: AuthConfig):
    ...
    @pytest.mark.asyncio
    async def test_verify_password_reset_token_success(self, auth_config: AuthConfig, sample_user_data: UserData):
        """Test successful password reset token verification."""
        auth_service = AuthService(auth_config)
        
        with patch.object(auth_service.core_auth.token_manager, 'verify_password_reset_token_with_storage', new_callable=AsyncMock) as mock_verify:
            mock_verify.return_value = sample_user_data
            
            with patch.object(auth_service, 'update_user_password', new_callable=AsyncMock) as mock_update:
                mock_update.return_value = True
    ...
    async def test_get_service_stats(self, auth_config: AuthConfig):
        ...
        with patch.object(auth_service.security_layer, 'get_stats', new_callable=AsyncMock) as mock_security_stats_method:
    ...
    async def test_health_check_healthy(self, auth_config: AuthConfig):
        ...
        with patch.object(auth_service.core_auth.db_client, 'health_check', new_callable=AsyncMock) as mock_db_health:
            mock_db_health.return_value = True
            
            with patch.object(auth_service.security_layer, 'get_stats', new_callable=AsyncMock) as mock_security_stats:
