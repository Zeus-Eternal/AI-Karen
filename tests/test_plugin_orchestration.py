"""
Tests for the plugin orchestration interface.

This module tests the advanced plugin orchestration capabilities
including workflow execution, conditional logic, and MCP integration.
"""
...
from src.ai_karen_engine.extensions.orchestrator import (
    PluginOrchestrator, PluginStep, PluginCall, WorkflowResult,
    Condition, ConditionOperator
)
from src.ai_karen_engine.extensions.workflow_engine import (
    WorkflowDefinition, WorkflowStep, StepType, WorkflowEngine
)


class TestPluginOrchestrator:
    """Test the PluginOrchestrator class."""
...
class TestWorkflowEngine:
    ...
    def mock_orchestrator(self):
        ...
        orchestrator = Mock()
        orchestrator.execute_plugin = AsyncMock()
        return orchestrator
    
    @pytest.fixture
    def workflow_engine(self, mock_orchestrator):
        """Create a WorkflowEngine instance."""
        return WorkflowEngine(mock_orchestrator)
    ...
    def test_workflow_registration(self, workflow_engine):
        ...
        assert workflow_engine.workflow_definitions["test_workflow"] == workflow
    
    def test_function_registration(self, workflow_engine):
        """Test function registration."""
        def test_func(x):
            return x * 2
        
        workflow_engine.register_function("double", test_func)
        
        assert "double" in workflow_engine.function_registry
        assert workflow_engine.function_registry["double"](5) == 10
    
    def test_variable_resolution(self, workflow_engine):
        """Test variable resolution in values."""
        variables = {"name": "John", "age": 30, "data": {"key": "value"}}
        
        # Test simple variable resolution
        assert workflow_engine._resolve_variables("${name}", variables) == "John"
        assert workflow_engine._resolve_variables("${age}", variables) == 30
        
        # Test non-variable values
        assert workflow_engine._resolve_variables("static", variables) == "static"
        assert workflow_engine._resolve_variables(42, variables) == 42
        
        # Test dict resolution
        input_dict = {"user": "${name}", "years": "${age}", "static": "value"}
        resolved_dict = workflow_engine._resolve_variables(input_dict, variables)
        assert resolved_dict == {"user": "John", "years": 30, "static": "value"}
        
        # Test list resolution
        input_list = ["${name}", "${age}", "static"]
        resolved_list = workflow_engine._resolve_variables(input_list, variables)
        assert resolved_list == ["John", 30, "static"]
    
    def test_condition_evaluation(self, workflow_engine):
        """Test condition evaluation."""
        variables = {"count": 5, "status": "active", "name": "test"}
        
        # Test equality
        assert workflow_engine._evaluate_condition("${count} == 5", variables) is True
        assert workflow_engine._evaluate_condition("${count} == 10", variables) is False
        
        # Test inequality
        assert workflow_engine._evaluate_condition("${count} != 10", variables) is True
        assert workflow_engine._evaluate_condition("${count} != 5", variables) is False
        
        # Test comparison
        assert workflow_engine._evaluate_condition("${count} > 3", variables) is True
        assert workflow_engine._evaluate_condition("${count} < 3", variables) is False
        
        # Test string comparison
        assert workflow_engine._evaluate_condition('${status} == "active"', variables) is True
        assert workflow_engine._evaluate_condition('${status} == "inactive"', variables) is False
...
async def test_integration_workflow_execution():
    ...
    mock_router.dispatch.side_effect = [
        {"data": "fetched_data"},
    ...
    # Create orchestrator
    orchestrator = PluginOrchestrator(mock_router)
    ...
    # Execute workflow
    execution = await orchestrator.execute_advanced_workflow(
        workflow,
    ...
    assert mock_router.dispatch.call_count == 3
