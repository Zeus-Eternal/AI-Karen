...
def test_health_check_success_path():
    ...
    try:
        ...
        with patch(
            "ai_karen_engine.database.client.create_engine"
        ) as mock_create_engine, patch(
            "ai_karen_engine.database.client.create_async_engine"
        ) as mock_create_async_engine, patch(
            "ai_karen_engine.database.client.sessionmaker"
        ) as mock_sessionmaker, patch(
            "ai_karen_engine.database.client.async_sessionmaker"
            ...
            mock_result.__getitem__ = Mock(
                return_value="PostgreSQL 13.7 on x86_64-pc-linux-gnu"
            )
            ...
            print(
                f"Comprehensive health check: {'✓ PASS' if health_status.is_healthy else '✗ FAIL'}"
            )
            ...
            if health_status.connection_pool_metrics:
                ...
                print(
                    f"  Pool Metrics: size={metrics.pool_size}, active={metrics.checked_out}"
                )

            # Test startup health check
            startup_status = client.startup_health_check()
            print(
                f"Startup health check: {'✓ PASS' if startup_status.is_healthy else '✗ FAIL'}"
            )
...
async def test_async_health_check_success_path():
    ...
    try:
        ...
        with patch(
            ...
            print(
                f"Async comprehensive health check: {'✓ PASS' if health_status.is_healthy else '✗ FAIL'}"
            )
            print(f"  Status: {health_status.status}")
            print(f"  Message: {health_status.message}")
            print(f"  Response Time: {health_status.response_time_ms:.2f}ms")
...
def test_connection_pool_metrics():
    ...
    try:
        with patch(
            ...
            if (
                metrics.pool_size == 15
                and metrics.checked_out == 5
                and metrics.total_connections == expected_total
            ):
...
async def main():
    ...
    test_results.append(
        (
            "Async Health Check Success Path",
            await test_async_health_check_success_path(),
        )
    )
