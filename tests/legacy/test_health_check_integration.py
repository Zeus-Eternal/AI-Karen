...
def test_health_check_integration():
    ...
    try:
        ...
        try:
            ...
            print(
                f"✓ Comprehensive health check callable - Status: {health_status.status}"
            )
        except Exception as e:
            print(
                f"✓ Comprehensive health check callable - Expected error: {type(e).__name__}"
            )
        ...
        try:
            ...
            print(
                f"✓ Startup health check callable - Expected error: {type(e).__name__}"
            )
        ...
        try:
            ...
            print(
                f"✓ Connection pool metrics callable - Pool size: {metrics.pool_size}"
            )
        except Exception as e:
            print(
                f"✓ Connection pool metrics callable - Expected error: {type(e).__name__}"
            )
        ...
        print(
            f"✓ DatabaseHealthStatus structure works - Healthy: {sample_health.is_healthy}"
        )

        # Create sample metrics
        sample_metrics = ConnectionPoolMetrics(
            pool_size=10, checked_out=2, total_connections=12
        )

        print(
            f"✓ ConnectionPoolMetrics structure works - Pool size: {sample_metrics.pool_size}"
        )
            ...
            try:
                ...
                return {
                    ...
                    "details": {
                        ...
                        "pool_metrics": {
                            "pool_size": health_status.connection_pool_metrics.pool_size
                            if health_status.connection_pool_metrics
                            else 0,
                            "checked_out": health_status.connection_pool_metrics.checked_out
                            if health_status.connection_pool_metrics
                            else 0,
                        }
                        if health_status.connection_pool_metrics
...
async def test_async_integration():
    ...
    try:
        ...
        try:
            ...
            print(
                f"✓ Async health check completed - Result: {'Healthy' if is_healthy else 'Unhealthy'}"
            )
        except Exception as e:
            print(
                f"✓ Async health check completed - Expected error: {type(e).__name__}"
            )
        ...
        try:
            ...
            print(
                f"✓ Async comprehensive health check completed - Status: {health_status.status}"
            )
        except Exception as e:
            print(
                f"✓ Async comprehensive health check completed - Expected error: {type(e).__name__}"
            )
...
def test_logging_integration():
    ...
    try:
        ...
        if log_messages:
            print(
                f"✓ Logging integration works - {len(log_messages)} log messages captured"
            )
