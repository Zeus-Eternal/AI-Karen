...
class TestAuthRoutesIntegration:
    ...
    def mock_auth_service(self):
        """Mock AuthService for testing with real data models."""
        config = AuthConfig(
        ...
        real_service = AuthService(config)
        asyncio.run(real_service.initialize())

        sample_user = asyncio.run(
            real_service.create_user(
                email="test@example.com",
                password="StrongPass123!",
                tenant_id="default",
            )
        )
        sample_session = asyncio.run(
            real_service.create_session(
                sample_user,
        ...
        service.sample_user = sample_user
        service.sample_session = sample_session
        return service
    ...
    def test_get_current_user_success(self, client, mock_auth_service):
        ...
        with patch(
            "ai_karen_engine.api_routes.auth.get_current_user", return_value=user_data
        ):
    ...
    def test_update_credentials_success(self, client, mock_auth_service):
        ...
        with patch(
            "ai_karen_engine.api_routes.auth.get_session_user"
        ) as mock_get_session:
    ...
    def test_update_credentials_missing_current_password(
        ...
        with patch(
            ...
            response = client.post(
                "/api/auth/update_credentials", json={"new_password": "NewPass123!"}
            )
    ...
    def test_request_password_reset_success(self, client, mock_auth_service):
        ...
        response = client.post(
            "/api/auth/request_password_reset", json={"email": "test@example.com"}
        )
...
class TestAuthMiddlewareIntegration:
    ...
    def test_app_with_middleware(self, mock_auth_service):
        ...
        with patch(
            "ai_karen_engine.middleware.auth.get_auth_service",
    ...
    def test_middleware_invalid_token_format(
        self, middleware_client, mock_auth_service
    ):
    ...
    def test_middleware_validation_exception(
        self, middleware_client, mock_auth_service
    ):
        ...
        response = middleware_client.get(
            "/protected", headers={"Authorization": "Bearer error-token"}
        )
...
class TestAuthErrorHandling:
    ...
    def test_rate_limit_error_handling(self, client, mock_auth_service):
        ...
        mock_auth_service.authenticate_user.side_effect = RateLimitExceededError(
            "Rate limit exceeded"
        )
    ...
    def test_account_locked_error_handling(self, client, mock_auth_service):
        ...
        mock_auth_service.authenticate_user.side_effect = AccountLockedError(
            "Account locked"
        )
    ...
    def test_session_expired_error_handling(self, client, mock_auth_service):
        ...
        mock_auth_service.validate_session.side_effect = SessionExpiredError(
            "Session expired"
        )
