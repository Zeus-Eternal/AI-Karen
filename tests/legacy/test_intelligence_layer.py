...
class TestAnomalyDetector:
    ...
    @pytest.mark.asyncio
    async def test_detect_time_anomaly(
        self, mock_config, sample_login_attempt, sample_behavioral_pattern
    ):
        ...
        result = await detector.detect_anomalies(
            unusual_attempt, sample_behavioral_pattern
        )
    ...
    @pytest.mark.asyncio
    async def test_detect_location_anomaly(
        self, mock_config, sample_login_attempt, sample_behavioral_pattern
    ):
        ...
        result = await detector.detect_anomalies(
            distant_attempt, sample_behavioral_pattern
        )
    ...
    @pytest.mark.asyncio
    async def test_detect_device_anomaly(
        self, mock_config, sample_login_attempt, sample_behavioral_pattern
    ):
        ...
        result = await detector.detect_anomalies(
            unknown_device_attempt, sample_behavioral_pattern
        )
    ...
    @pytest.mark.asyncio
    async def test_detect_frequency_anomaly(
        self, mock_config, sample_behavioral_pattern
    ):
        ...
        result = await detector.detect_anomalies(
            sunday_attempt, sample_behavioral_pattern
        )
    ...
    async def test_detect_anomalies_error_handling(
        self, mock_config, sample_login_attempt
    ):
...
class TestBehavioralAnalyzer:
    ...
    @pytest.mark.asyncio
    async def test_analyze_behavior_insufficient_data(
        self, mock_config, sample_login_attempt
    ):
    ...
    @pytest.mark.asyncio
    async def test_analyze_behavior_success(
        self, mock_config, sample_login_attempt, sample_user_data, sample_auth_events
    ):
        """Test successful behavioral analysis."""
        analyzer = BehavioralAnalyzer(mock_config)

        result = await analyzer.analyze_behavior(
            sample_login_attempt, sample_user_data, sample_auth_events
        )
    ...
    async def test_build_behavioral_pattern(self, mock_config, sample_auth_events):
        ...
        pattern = await analyzer._build_behavioral_pattern(
            "test-user-123", sample_auth_events
        )
    ...
    def test_analyze_time_pattern(
        ...
        result = analyzer._analyze_time_pattern(
            typical_attempt, sample_behavioral_pattern
        )
    ...
    def test_analyze_location_pattern(
        self, mock_config, sample_login_attempt, sample_behavioral_pattern
    ):
        """Test location pattern analysis."""
        analyzer = BehavioralAnalyzer(mock_config)

        result = analyzer._analyze_location_pattern(
            sample_login_attempt, sample_behavioral_pattern
        )

        assert "current_location" in result
        assert "is_typical_location" in result
        assert "typical_locations_count" in result
        assert result["typical_locations_count"] == len(
            sample_behavioral_pattern.typical_locations
        )

    def test_analyze_device_pattern(
        self, mock_config, sample_login_attempt, sample_behavioral_pattern
    ):
        """Test device pattern analysis."""
        analyzer = BehavioralAnalyzer(mock_config)

        result = analyzer._analyze_device_pattern(
            sample_login_attempt, sample_behavioral_pattern
        )
    ...
    def test_analyze_frequency_pattern(
        self, mock_config, sample_login_attempt, sample_behavioral_pattern
    ):
        ...
        result = analyzer._analyze_frequency_pattern(
            monday_attempt, sample_behavioral_pattern
        )
    ...
    async def test_analyze_behavior_error_handling(
        self, mock_config, sample_login_attempt, sample_user_data
    ):
        ...
        result = await analyzer.analyze_behavior(
            sample_login_attempt, sample_user_data, invalid_events
        )
...
class TestRiskScorer:
    ...
    def test_is_suspicious_user_agent(self, mock_config):
        ...
        assert (
            scorer._is_suspicious_user_agent(
                "Mozilla/5.0 (Windows NT 10.0; Win64; x64)"
            )
            is False
        )

    def test_calculate_risk_score_error_handling(
        self, mock_config, sample_login_attempt
    ):
...
class TestIntelligenceEngine:
    ...
    @pytest.mark.asyncio
    async def test_analyze_login_attempt_success(
        self, mock_config, sample_login_attempt, sample_user_data, sample_auth_events
    ):
    ...
    @pytest.mark.asyncio
    async def test_analyze_login_attempt_no_data(
        self, mock_config, sample_login_attempt
    ):
    ...
    def test_generate_recommendations(self, mock_config):
        ...
        recommendations = engine._generate_recommendations(
            "medium", "medium", anomaly_result, None
        )
        assert len(recommendations) > 0
    ...
    @pytest.mark.asyncio
    async def test_calculate_risk_score_error_handling(
        self, mock_config, sample_user_data
    ):
        """Test error handling in risk score calculation."""
        engine = IntelligenceEngine(mock_config)
        ...
        risk_score = await engine.calculate_risk_score(
            sample_user_data, invalid_context
        )
...
class TestDataModels:
    ...
    def test_behavioral_pattern_to_dict(self, sample_behavioral_pattern):
        ...
        assert (
            data["typical_login_hours"] == sample_behavioral_pattern.typical_login_hours
        )
        assert (
            data["last_updated"] == sample_behavioral_pattern.last_updated.isoformat()
        )
