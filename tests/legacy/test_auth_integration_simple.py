...
async def test_auth_service_integration_in_dependencies():
    ...
    with patch(
        "ai_karen_engine.auth.service.get_auth_service", return_value=mock_auth_service
    ):
        ...
        mock_auth_service.validate_session.assert_called_once_with(
            session_token="test-token", ip_address="127.0.0.1", user_agent="test-agent"
        )
...
def test_session_cookie_configuration():
    ...
    from ai_karen_engine.api_routes.auth import COOKIE_NAME, set_session_cookie
