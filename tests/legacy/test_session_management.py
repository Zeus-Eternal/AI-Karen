...
from ai_karen_engine.auth.config import JWTConfig, SessionConfig
...
class TestMemorySessionBackend:
    ...
    async def test_store_and_get_session(self, memory_backend, sample_session_data):
        ...
        # Retrieve session
        retrieved_session = await memory_backend.get_session(
        ...
        assert (
            retrieved_session.user_data.user_id == sample_session_data.user_data.user_id
        )
    ...
    async def test_get_user_sessions(
        self, memory_backend, sample_user_data, token_manager
    ):
        ...
        assert all(
            s.user_data.user_id == sample_user_data.user_id for s in user_sessions
        )
    ...
    async def test_delete_user_sessions(
        ...
        deleted_count = await memory_backend.delete_user_sessions(
    ...
    async def test_cleanup_expired_sessions(
        ...
        expired_session = SessionData(
            ...
            - timedelta(seconds=2),  # Created 2 seconds ago
        )
...
class TestDatabaseSessionBackend:
    ...
    def db_session_config(self):
        ...
        config = SessionConfig(
            session_timeout_hours=24, max_sessions_per_user=5, storage_type="database"
        )
    ...
    @pytest.mark.asyncio
    async def test_store_session_database(
        self, database_backend, sample_session_data, sample_user_data
    ):
...
class TestRedisSessionBackend:
    ...
    @pytest.mark.asyncio
    async def test_redis_backend_initialization_without_redis(
        self, redis_session_config
    ):
...
class TestSessionManager:
    ...
    async def test_refresh_session(self, session_manager, sample_user_data):
        ...
        refreshed_session = await session_manager.refresh_session(
            original_session.session_token, original_session.refresh_token
        )
    ...
    async def test_refresh_session_invalid_token(
        self, session_manager, sample_user_data
    ):
        ...
        refreshed = await session_manager.refresh_session(
            session.session_token, "invalid-refresh-token"
        )
        assert refreshed is None

        # Try to refresh with wrong session token
        refreshed = await session_manager.refresh_session(
            "invalid-session-token", session.refresh_token
        )
        assert refreshed is None
    ...
    async def test_invalidate_session(self, session_manager, sample_user_data):
        ...
        invalidated = await session_manager.invalidate_session(
            session.session_token, "test_invalidation"
        )
        assert invalidated is True

        # Verify session is invalidated by getting it directly from store
        retrieved_session = await session_manager.store.get_session(
            session.session_token
        )
    ...
    async def test_delete_session(self, session_manager, sample_user_data):
        ...
        # Verify session is gone
        retrieved_session = await session_manager.validate_session(
            session.session_token
        )
    ...
    async def test_get_user_sessions(self, session_manager, sample_user_data):
        ...
        for i in range(3):
            session = await session_manager.create_session(
                sample_user_data, ip_address=f"192.168.1.{i+1}"
            )
            sessions.append(session)
    ...
    async def test_delete_user_sessions(self, session_manager, sample_user_data):
        ...
        # Delete all user sessions
        deleted_count = await session_manager.delete_user_sessions(
            sample_user_data.user_id
        )
        assert deleted_count == 3
    ...
    async def test_session_limit_enforcement(self, sample_user_data):
        ...
        config = SessionConfig(
            session_timeout_hours=24, max_sessions_per_user=2, storage_type="memory"
        )
