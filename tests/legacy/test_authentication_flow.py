...
def test_authentication_flow(base_url: str, origin: str) -> Dict[str, Any]:
    ...
    try:
        response = requests.post(
            f"{base_url}/api/auth/register", json=test_user, headers=headers, timeout=10
        )
    ...
    try:
        response = requests.post(
            f"{base_url}/api/auth/login", json=test_user, headers=headers, timeout=10
        )
    ...
    if "token" in results:
        ...
        try:
            auth_headers = {**headers, "Authorization": f'Bearer {results["token"]}'}

            response = requests.get(
                f"{base_url}/api/auth/me", headers=auth_headers, timeout=10
            )
    ...
    if "token" in results:
        ...
        try:
            response = requests.post(
                f"{base_url}/api/auth/logout", headers=headers, timeout=10
            )
...
def main():
    ...
    if all_tests_passed:
        ...
        print(
            "✅ Users can create accounts, login, access protected resources, and logout"
        )
